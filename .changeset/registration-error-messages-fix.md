...
Correct and shorten the registration error messages
...
param" (fused marker) and "build/meta would capture the marker into the name"
(trailing marker) were false; the trailing-marker message also claimed the gate
rejects the spelling as `name-less`, while the gate reports `trailing-marker`.

Two more were wrong about what they were describing: a splat conflict was
called "a parametric URL segment", and a duplicate name was quoted with a
hardcoded `':'` prefix even though the check covers splats, so `/:x/*x`
