...
handles (a ~10k-link page hit the emitter's listener cap). A new shared
`internal/createActiveSource` fast/slow builder (mirroring the angular helper)
now backs BOTH `<Link>`'s reactive `watch` and `useIsActiveRoute`, so a
default-options link resolves active state through ONE per-router
`createActiveNameSelector` subscription. Single source of truth for the fast/slow
