...
the `{ hash }` options object shifts to position 4. The plugin threads `search` into
core's now-search-aware `router.buildPath`, and `history.state` written by
`replaceHistoryState` / `onTransitionSuccess` carries the new `search` field
alongside `{ name, params, path }`.
