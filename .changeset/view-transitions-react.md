...
The utility lives in `shared/dom-utils/` as `createViewTransitions(router)` — same architectural pattern as `createScrollRestoration` (#497) and `createRouteAnnouncer`. It uses only the public `subscribeLeave` + `subscribe` router API.
