...
| # | Invariant | Why it must hold |
|---|-----------|-----------------|
| 1 | **Fallback to the RESOLVING door** — when `router.buildUrl` returns `undefined`, the helper falls back to `buildNavigationState` | Memory/console runtimes have no URL plugin; the fallback keeps `<Link>` usable everywhere, and resolves `forwardTo` so the href is where the click lands (#2250) |
