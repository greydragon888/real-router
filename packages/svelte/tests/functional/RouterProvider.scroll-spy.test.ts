...
// scroll-spy suite + the react/preact/vue/solid adapters, but driven via
// `<RouterProvider scrollSpy>` rendered through @testing-library/svelte + a
// host component).
interface FakeIO {
...
describe("RouterProvider — scrollSpy", () => {
  ...
  // Render `<RouterProvider scrollSpy>` through the host component — the svelte
  // analogue of react's `render(<RouterProvider …/>)`. @testing-library/svelte
  // flushes the mount `$effect` (which wires scroll-spy) synchronously, and the
  // IO callback → scroll-spy → `router.navigate` path fires under
  // `vi.runAllTimers()` (navigate is spied at the call site, not awaited), so
  // there is no further reactivity to flush before asserting. The host declares
  // `scrollSpy?: ScrollSpyOptions | undefined`, so passing `undefined` is the
  // "off" state — no exactOptionalPropertyTypes friction at the call site. The
  // whole `scrollSpy` object (incl. `scrollContainer`) is forwarded straight to
  // `<RouterProvider>`; container reactivity (#780) is driven by the utility's
  // own MutationObserver reconcile, not the Svelte effect, so the host needs no
  // extra prop for the scrollContainer scenarios. `overrideRouter` swaps in a
  // plain (URL-plugin-less) router for the detection/warning tests.
  function renderSpy(
    scrollSpy?: {
      selector: string;
      rootMargin?: string;
    ...
    return render(RouterProviderScrollSpyHost, {
      props: { router: overrideRouter ?? router, scrollSpy },
    });
  }
  ...
  // NOTE: asserted count-relative — svelte mounts once (no StrictMode
  ...
  it("SSR render never wires scroll-spy (effect is browser-only)", async () => {
    // Server-render the host through `svelte/server`. `$effect` — the client-only
    // wiring point for scroll-spy — must not run server-side, so createScrollSpy
    // is never invoked and no IntersectionObserver is instantiated. This is the
    // scroll-spy SSR contract the react original locks via
    // `expect(ioInstances).toHaveLength(0)`.
    //
    // Under this package's vitest config (`resolve.conditions: ["browser"]` — see
    // ClientOnly.test.ts + the svelte CLAUDE.md "Do not override resolve.conditions"
    // note) every `.svelte` import is compiled in CLIENT mode, so
    // `svelte/server.render()` throws `effect_orphan` when it reaches the
    // provider's `$effect` instead of cleanly skipping it. We swallow that
    // config-artifact throw; the load-bearing proof — that no IO is wired during
    // a server render — still holds (`ioInstances` stays empty). The react
    // original's `toContain("server content")` body-string assertion is dropped
    // for the same reason (no server HTML string is produced here). The real SSR
    // contract is enforced end-to-end by `examples/web/svelte/ssr-examples/*`.
    try {
      await renderToString(RouterProviderScrollSpyHost, {
        props: { router, scrollSpy: { selector: "[id]" } },
      });
    } catch {
      /* effect_orphan — client-compiled `$effect` under browser-condition vitest */
    }
