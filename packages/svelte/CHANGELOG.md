...
- [#2191](https://github.com/greydragon888/real-router/pull/2191) [`d347e0b`](https://github.com/greydragon888/real-router/commit/d347e0bb0c2997bfdf75ae45d87473089699e614) Thanks [@greydragon888](https://github.com/greydragon888)! - `<Link>` and `use:link` defer to the browser for every anchor `target` but `_self` ([#1834](https://github.com/greydragon888/real-router/issues/1834))
...
> ⚠ **Never published** — there is no `0.17.40` on npm and no git tag for it. The release run that allocated this number never reached the registry; the entries below ship in **0.17.41**.
...
> ⚠ **Never published** — there is no `0.17.26` on npm and no git tag for it. The release run that allocated this number never reached the registry; the entries below ship in **0.17.27**.
