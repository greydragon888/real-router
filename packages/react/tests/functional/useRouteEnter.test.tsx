...
import { RouterProvider, useRouteEnter } from "@real-router/react";
...
describe("useRouteEnter", () => {
  ...
  it("does not fire twice when StrictMode double-runs effects", async () => {
    const handler = vi.fn();
    ...
    // Even with StrictMode's double-effect-run, the lastHandledRouteRef
    // guard ensures the handler fires only once per real navigation.
    expect(handler).toHaveBeenCalledTimes(1);
  });
  ...
  it("skips the handler on same-route navigation by default", async () => {
    ...
    // First navigate to users.view to establish a route with params we can
    // change. This is cross-route ("test" → "users.view") so the handler
    // fires once.
    await act(async () => {
