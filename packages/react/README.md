...
| `@real-router/react/ink`                        | 19.2+         | Terminal (Ink 7+) | Hooks, `InkRouterProvider`, `InkLink` — no `Link`, no `RouteView`, no `announceNavigation`. **ESM-only** (`import` only): ink itself is ESM-only with top-level await, so a `require`-able build of this entry cannot exist |
