...
**Strictly-decoded (#1211).** The value is a DECODED fragment, encoded verbatim (`encodeURI(s).replace(/#/g, "%23")`) at the URL boundary. Do NOT pass raw `location.hash` (which is percent-encoded) — `hash="a%20b"` is the literal fragment `a%20b` and renders `#a%2520b`, not `#a%20b`. Pass `hash="a b"` for the fragment `a b`. The plugin and the adapter obey the one strict contract, so neither layer tolerates the encoded form.
...
along as app-level data; it just never reaches the URL — see the wiki's `Route.md`.) `InkLink` accepts the same prop.
...
- **ESM-only entry (#1628)** — `exports["./ink"]` declares `import` but no `require`. A CJS build of this entry is not constructible: it would `require("ink")`, and ink v7 is ESM-only with **top-level await** in its graph, so Node answers `require() cannot be used on an ESM graph with top-level await` on every version. `require("@real-router/react/ink")` therefore fails with `ERR_PACKAGE_PATH_NOT_EXPORTED`. A CJS consumer still reaches the entry with `await import("@real-router/react/ink")`, which resolves at runtime and type-checks in a CJS-mode TS file — only the `require` channel is closed. Consequence for the build: attw's `strict` profile demands a resolution in every mode, so `packages/react/tsdown.config.mts` carries `attw: { excludeEntrypoints: ["./ink"] }` — scoped to this one entry, so the six dual entries stay strictly checked. tsdown still emits `dist/cjs/ink.*` and that is deliberate: the node10 `typesVersions` fallback (`"ink": ["./dist/cjs/ink.d.ts"]`) points into it, so pruning the entry from the CJS build would break type resolution for `moduleResolution: node` consumers and leave a manifest entry pointing at a missing file. The runtime `dist/cjs/ink.js` is simply unreachable — no export condition references it.
