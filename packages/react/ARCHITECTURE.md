...
- **`anchorTargetsAnotherContext(element)`** — the same question about an element, narrowed to HTML anchors by `tagName` so an anchor from another realm still answers
