...
- **Fast path for single listener** -- when a set has exactly one listener, `emit` skips the `[...set]` array spread on **both** emit paths (`#emitFast` and `#emitWithDepthTracking` special-case `set.size === 1` → direct call). This matters for the router, which runs on the depth-tracking path (`maxEventDepth > 0` via `DEFAULT_LIMITS`): single-subscriber events get the shortcut (measured ~10% faster for one listener)
