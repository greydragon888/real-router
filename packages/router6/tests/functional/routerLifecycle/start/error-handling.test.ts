...
describe("router.start() - error handling", () => {
  ...
  describe("protectedDone callback guard", () => {
    it("should warn and ignore second callback invocation via navigateToDefault", () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith("Callback already invoked");
  ...
  describe("error handling edge cases", () => {
    describe("event listener exceptions", () => {
      ...
      it("should catch and log exception from ROUTER_START event listener", () => {
        ...
        // Error should be logged (format: "[Router] Error in listener for <event>:", Error)
        expect(errorSpy).toHaveBeenCalledWith(
          expect.stringMatching(/\[Router\].*Error in listener/),
          expect.any(Error),
        );
