...
import type { DefaultDependencies, Router } from "router6-types";
...
>(dependencies: Partial<Dependencies>) {
  ...
  const setDependencies = (deps: Partial<Dependencies>) => {
    ...
    if (overwrittenKeys.length > 0) {
      console.warn(
        `[router.setDependencies] Overwritten: ${overwrittenKeys.join(", ")}`,
      );
    }
  };
    ...
    router.setDependency = <K extends keyof Dependencies & string>(
      ...
      if (isNewKey) {
        ...
        if (isChanging && !bothAreNaN) {
          console.warn(
            `[router.setDependency] Router dependency already exists and is being overwritten: ${dependencyName}`,
          );
        }
      }
