// packages/preact/tests/property/linkUtils.properties.ts

/**
 * Property-based tests for shared dom-utils helpers.
 *
 * The DOM-utils module is symlinked from `shared/dom-utils/` and shipped to
 * every framework adapter. These invariants mirror the React adapter's
 * `linkUtils.properties.ts` — duplicated rather than imported because each
 * adapter owns its property-test surface (per `INVARIANTS.md`).
 *
 * Covers:
