...
function logBaseline(
  ...
  const line = `[memory-baseline] preact/${pattern} iters=${iterations} delta=${deltaKb}KB per-iter=${perIter}B ${notes}\n`;
...
describe("memory-mount-unmount baseline", () => {
  ...
  it("Pattern A: useRouterTransition × 1000 mount/unmount", () => {
    ...
    // After cached getTransitionSource: ~3.5KB/iter (baseline 5KB).
    expect(delta).toBeLessThan(4500 * iterations);
  });

  it("Pattern B: useRouteNode × 100 + 50 navigations", async () => {
    const NodeConsumer: FunctionComponent<{ nodeName: string }> = ({
      nodeName,
    }) => {
    ...
    // Shared cached RouteNodeSource regression gate.
    expect(delta).toBeLessThan(2100 * 10 * 100);
  });
  ...
  it("Pattern C: 500 RouterErrorBoundary with fresh routers", async () => {
    ...
    const fallback = (): ComponentChildren => <div />;
    const ErrorConsumer: FunctionComponent = () => (
      <RouterErrorBoundary fallback={fallback} onError={(): void => {}}>
        <div />
      </RouterErrorBoundary>
    );
