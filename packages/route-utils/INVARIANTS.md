...
| 5   | Curried form equivalence (string segments) | For a **string** `segment`, `f(route)(segment) === f(route, segment)` for all three testers — the curried code path has its own validation and produces identical results. **Not** for `null`: the direct form has a `(route, null) → false` overload (Null-guard Inv 4), but the curried `(segment: string) => boolean` form has no null branch, so `f(route)(null)` throws `TypeError`. The property test (`routeUtils.properties.ts`) generates only string segments, where equivalence holds. |
...
> **Note:** `getParentSegment` / `getLastSegment` (used to phrase Inv 2–3) are **test helpers**, not public API. The package's public segment testers are `startsWithSegment` / `endsWithSegment` / `includesSegment` (+ `areRoutesRelated` / `isDescendantOf`); the invariants are stated via the helpers only for readability.
...
| 2   | Empty route name     | `startsWithSegment("", seg)`, `endsWithSegment("", seg)`, and `includesSegment("", seg)` all return `false` for any non-empty segment. An empty route name matches nothing. The **curried** form (`f("")`) still returns a tester **function** per the overload contract — deferring the empty-name check does not collapse the single-arg form to a boolean — and that function returns `false` for every segment (#769).                            |
