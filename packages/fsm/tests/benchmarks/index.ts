/**
 * FSM Benchmarks Entry Point
 *
 * Run: pnpm bench
 * Results: .bench/mitata-results.json
 *
 * Categories:
 * 1. Basic Scenarios - Construction, send, getState, getContext, onTransition
 * 2. Stress Tests - High-load transitions, many listeners, large FSMs
 */
...
// Import all benchmark files (they register benchmarks via bench)
import "./fsm.bench";
...
const JIT_WARMUP_ITERATIONS = 100;

const warmupConfig: FSMConfig<"a" | "b" | "c", "X" | "Y", { n: number }> = {
  initial: "a",
  context: { n: 0 },
  transitions: {
    a: { X: "b" },
...
function warmupJIT(): void {
  ...
  for (let i = 0; i < JIT_WARMUP_ITERATIONS; i++) {
    const fsm = new FSM(warmupConfig);
