...
It drives the entire router lifecycle — all states (IDLE, STARTING, READY, TRANSITION_STARTED, DISPOSED) and transitions are managed by a single FSM instance.
...
// Example: Router FSM
{
  ...
  TRANSITION_STARTED: { NAVIGATE: "TRANSITION_STARTED", COMPLETE: "READY", CANCEL: "READY", FAIL: "READY" },
...
fsm.on("TRANSITION_STARTED", "COMPLETE", (payload) => emitter.emit("$$success", ...));
// ... etc for all events
```
