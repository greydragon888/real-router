...
- **Undeclared states are rejected at every state-entry-point** — `new FSM({ initial })`, `forceState(state)`, and `on(from, …)` all run a shared `requireDeclared` guard and throw `[FSM.<where>] state "…" is not declared in config.transitions` instead of bricking the FSM (constructor / `forceState` would leave `#currentTransitions` `undefined` → cryptic `TypeError` on the next `canSend`/`send`) or silently dead-registering an action (`on`, whose action could never fire). `forceState` runs the guard **before** any mutation, so a rejected call leaves the FSM in its prior valid state. The type forbids undeclared states for a **narrow** state union; the guard hardens `string`-typed / JS / cast callers, mirroring `send()`'s defensiveness on unknown input. Resolved in [#754](https://github.com/greydragon888/real-router/issues/754) (`forceState`) + [#885](https://github.com/greydragon888/real-router/issues/885) (constructor + `on` — the #754 guard applied symmetrically across all three).
