...
- **`send()` payload is NOT type-correlated to the event** — `send(event: TEvents, payload?: TPayloadMap[TEvents])` indexes by the full `TEvents` union, so `send("E1", payloadForE2)` compiles. `on(from, event, action)` *does* correlate the payload to the specific event. Known asymmetry — see [#753](https://github.com/greydragon888/real-router/issues/753). (Dormant for core: `RouterPayloads` is empty.)
