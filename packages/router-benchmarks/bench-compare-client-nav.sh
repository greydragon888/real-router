...
# =============================================================================
# TanStack Router vs Real-Router Client-Nav Benchmark Comparison Script
...
for arg in "$@"; do
    if [[ "$arg" == "-h" || "$arg" == "--help" ]]; then
        echo "Usage: sudo ./bench-compare-client-nav.sh"
        echo ""
        echo "Run client-nav benchmark comparison between TanStack Router and Real-Router."
...
RESULTS_DIR="${SCRIPT_DIR}/.bench-results/client-nav"
...
pnpm bench:client-nav:build:tanstack >/dev/null 2>&1
echo -e "${GREEN}TanStack Router app built${NC}"

# Build Real-Router app
echo "Building Real-Router benchmark app..."
pnpm bench:client-nav:build:real-router >/dev/null 2>&1
...
NODE_ENV=production nice -n -20 vitest bench \
    --config client-nav/tanstack/react/vite.config.ts \
    client-nav/tanstack/react/speed.bench.ts 2>&1 | tee "$RESULT_FILE_TANSTACK"
...
sync && sudo purge || true
NODE_ENV=production nice -n -20 vitest bench \
    --config client-nav/real-router/react/vite.config.ts \
    client-nav/real-router/react/speed.bench.ts 2>&1 | tee "$RESULT_FILE_REAL_ROUTER"
