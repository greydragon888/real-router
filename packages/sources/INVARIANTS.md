...
| 2   | Hash flip beats `reload=false` (branch order)                                   | When `prev.path === next.path` but the hashes differ, the stabilizer returns `next` regardless of `next.transition.reload === false`. The hash check happens before the reload check in `src/stabilizeState.ts`. |
