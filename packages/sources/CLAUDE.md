...
4. **For `createActiveRouteSource`**, `canonicalJson(params)` normalizes key order so `{a:1, b:2}` and `{b:2, a:1}` hit the same cache entry. A non-string route NAME (#1881), and `Symbol`/`BigInt`/circular refs in params, fall back to creating a fresh non-cached source.
