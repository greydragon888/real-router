...
**Non-cached paths:** `createActiveRouteSource` bypasses the cache and returns a fresh source for that call in two cases — a NON-STRING route name, because the cache key coerces it and a non-string would share a slot with a well-typed name (#1881); and `canonicalJson(params)` throwing. Throw-triggers include `BigInt` (native `JSON.stringify`), `Map`, `Set`, `WeakMap`, `WeakSet`, `RegExp` (eager `TypeError` — these would otherwise collapse to `"{}"` and cause cache-key collisions), and circular references (path-based detector in `canonicalize()`). `Symbol`-valued fields are silently dropped (standard JSON semantics) — they do not bypass the cache.
