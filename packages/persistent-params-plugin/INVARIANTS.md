...
| #   | Invariant                                              | Description                                                                                                                                                                                                                                          |
| --- | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Persisted values ride the query channel                | A value set in the query channel (`navigate(name, {}, { lang })`) is committed to `state.search`; `state.params` carries the route's own path params and never a tracked key. The plugin declares its keys as query (`setRootPath("?a&b")`) and injects them there, so core has nothing to re-route (#1563). A VALUE for a declared query name spelled in the path bag is refused by core outright (#1572), so there is no second spelling for this row to cover. |
| 2   | Removal markers are honored in either channel          | `{ key: undefined }` passed in `search` **or** in the path bag removes the key from the committed state AND from the built URL. One seam answers both: `mergeParams` applies the marker when it arrives in the query bag, the tracked-key loop when it arrives in the path bag — which core reads as a channel error only when it carries a VALUE. `search-channel-injection` owns a cell per channel (#1563). |
