...
Idempotent — multiple `invalidate()` calls between refreshes collapse to one re-run. Surgical for multi-namespace routes — only `"rsc"` re-runs. Nothing is cached across states: `state.context` is rebuilt empty for every navigation, so `state.context.data` is absent unless [`@real-router/ssr-data-plugin`](https://www.npmjs.com/package/@real-router/ssr-data-plugin)'s own `invalidate()` was also called on the same transition.
