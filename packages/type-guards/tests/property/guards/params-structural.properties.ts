...
describe("Params structural properties (DAGs, cycles, depth)", () => {
  ...
  // INV 32: the lattice ¬isParams ⇒ ¬isParamsStrict across the FULL invalid space
  // (cycles, nested class instances, nested NaN/Infinity, functions/symbols,
  // top-level arrays) — far beyond the primitives-only generator behind Inv 13.
  test.prop([richInvalidParamsArbitrary], { numRuns: 10_000 })(
    "strict never accepts what flexible rejects (rich invalid space)",
    (value) => {
