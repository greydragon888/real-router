...
describe("State Edge Cases (Uncovered Branches)", () => {
  ...
  describe("isHistoryState - Meta validation (lines 149-162)", () => {
    ...
    test.prop(
      ...
      "rejects meta with non-boolean redirected field",
      (name, path, params, redirectedValue) => {
        const state = {
          name,
          path,
          params,
          meta: { redirected: redirectedValue, id: 1, options: {} },
        };

        // Line 160: "redirected" in meta && typeof meta.redirected !== "boolean"
        expect(isHistoryState(state)).toBe(false);
    ...
    test.prop(
      ...
      "rejects meta with non-string source field",
      (name, path, params, sourceValue) => {
        const state = {
          name,
          path,
          params,
          meta: { source: sourceValue, id: 1, options: {} },
        };

        // Line 161: "source" in meta && typeof meta.source !== "string"
        expect(isHistoryState(state)).toBe(false);
  ...
  describe("isHistoryState - Valid combinations covering all branches", () => {
    ...
    test.prop(
      ...
      "accepts state with meta.redirected",
      (name, path, params, redirected) => {
        // Line 160: "redirected" in meta && typeof meta.redirected === "boolean"
        const state = {
          name,
          path,
    ...
    )("accepts state with meta.source", (name, path, params, source) => {
      // Line 161: "source" in meta && typeof meta.source === "string"
      const state = {
        name,
        path,
    ...
    test.prop(
      ...
      "accepts state with full meta",
      (name, path, params, id, metaParams, redirected, source) => {
        // All meta fields present and valid - exercises all positive branches
        const state = {
          name,
          path,
          params,
