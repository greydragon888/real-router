...
import { isParams, isStateStrict } from "type-guards";

/**
 * Robustness / DoS-resistance + anti-quadratic guards for the recursive-tree
 * validators `isParams` / `isStateStrict`.
 *
 * These run at the validation boundary on UNTRUSTED input: `history.state`
 * (validated via `isStateStrict` -> `isParams` on popstate in browser-/hash-plugin,
 * settable by any script on the page) and user-supplied params to `navigate` /
 * `makeState` (validation-plugin calls `isParams` directly). So they must stay
 * well-behaved on inputs no realistic app produces but an attacker can: chains
 * thousands of levels deep, hundreds of thousands of sibling objects.
 *
 * Coverage and small-input property tests never reach these sizes. The guards are
...
describe("S1: deep nesting validates without overflowing the call stack (#901)", () => {
  ...
  it("does not overflow on a deep params tree reached through the public isStateStrict path", () => {
    const state = {
      name: "deep",
      path: "/deep",
