...
| 3   | isParams accepts nested params                      | Objects with nested object values are valid params. `isParams` validates the full nested tree.                                                                                                                                                                                                                                            |
...
| 17  | isParams accepts arrays of nested objects           | Arrays containing plain objects are valid param values. `isParams` validates each element through the tree.                                                                                                                                                                                                                               |
| 18  | isParams accepts deeply nested structures           | Multi-level nesting is valid. Validation is iterative (explicit work-stack), so there is no depth limit and no stack overflow: `isParams` returns a boolean at any depth instead of throwing `RangeError` (#901).                                                                                                                         |
...
| 21  | isParams rejects self-referencing circular objects  | Objects with circular references (where a property points back to the object itself) are rejected via on-path cycle detection in `isSerializable`.                                                                                                                                                                                        |
| 22  | isParams rejects circular references through arrays | Objects whose arrays contain back-references to parent objects are rejected. The iterative `isSerializable` walk tracks the containers on the current DFS path, so the back-edge is flagged as a cycle.                                                                                                                                   |
...
| 24  | isParams rejects class instances at top level       | Class instances (with prototypes other than `Object.prototype` or `null`) are not valid params. The prototype check applies at the top level and throughout the nested tree.                                                                                                                                                              |
| 25  | isParamsStrict rejects class instances at top level | Class instances and custom-prototype objects (prototype other than `Object.prototype` or `null`) are rejected by the strict guard, mirroring `isParams`. Without this prototype check a class instance with no own enumerable fields yields zero `for..in` iterations and would wrongly pass, breaking Inv 12/13 (#785).                  |
| 26  | isParams accepts shared references (diamonds)       | The same object or array referenced under multiple keys, or repeated as an array element (a diamond / DAG, not a cycle), is serializable (`JSON.stringify` duplicates it) and accepted. `isSerializable` uses on-path cycle detection plus a done-set, so off-path re-encounters pass and the walk stays linear on diamond chains (#786). |
...
| #   | Invariant                                                   | Description                                                                                                                                                                                                                                                   |
| --- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | isParams accepts nested objects that isParamsStrict rejects | For params containing nested plain objects, `isParams` returns `true` (full serializable check) while `isParamsStrict` returns `false` (only primitives and arrays of primitives allowed). This is the defining behavioral difference between the two guards. |
