...
import type { Params, PluginFactory, Plugin } from "router6";
...
export function persistentParamsPluginFactory(
  ...
  if (!isValidParamsConfig(params)) {
    ...
    throw new TypeError(
      `[router6-plugin-persistent-params] Invalid params configuration. ` +
  ...
  return (router): Plugin => {
    ...
    if (PLUGIN_MARKER in router) {
      throw new Error(
        `[router6-plugin-persistent-params] Plugin already initialized on this router. ` +
    ...
    try {
      ...
      throw new Error(
        `[router6-plugin-persistent-params] Failed to update root path: ${error instanceof Error ? error.message : String(error)}`,
        { cause: error },
      );
    }
    ...
    function withPersistentParams(additionalParams: Params): Params {
      try {
        ...
        // Re-throw type errors with original message
        ...
        throw new Error(
          `[router6-plugin-persistent-params] Error merging parameters: ${error instanceof Error ? error.message : String(error)}`,
          { cause: error },
        );
      }
    }
