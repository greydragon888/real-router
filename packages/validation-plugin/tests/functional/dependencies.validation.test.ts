...
type TestDeps = { foo?: number; bar?: string };
...
describe("dependencies validation — with validationPlugin", () => {
  ...
  describe("setDependency validation", () => {
    it("should throw TypeError for non-string keys", () => {
      ...
      expect(() => raw.set(123, "value")).toThrow(TypeError);
      expect(() => raw.set(123, "value")).toThrow(
        "dependency name must be a string, got number",
      );
    });
    ...
    it("should throw TypeError for null key", () => {
      const deps = getDependenciesApi(router);
      const raw = deps as unknown as { set(k: unknown, v: unknown): void };
      expect(() => raw.set(null, "value")).toThrow(TypeError);
      expect(() => raw.set(null, "value")).toThrow(
        "dependency name must be a string, got object",
      );
    });
    ...
    it("should throw TypeError for object key", () => {
      const deps = getDependenciesApi(router);
      const raw = deps as unknown as { set(k: unknown, v: unknown): void };
      expect(() => raw.set({}, "value")).toThrow(TypeError);
    });

    it("should throw TypeError for invalid key even when value is undefined", () => {
      const deps = getDependenciesApi(router);
      const raw = deps as unknown as { set(k: unknown, v: unknown): void };
      expect(() => raw.set(null, undefined)).toThrow(TypeError);
      expect(() => raw.set(123, undefined)).toThrow(TypeError);
      expect(() => raw.set({}, undefined)).toThrow(TypeError);
    });

    it("should allow valid string key", () => {
      const deps = getDependenciesApi(router);
      expect(() => deps.set("foo", 2)).not.toThrow();
    });
  });
  ...
  describe("setDependencies (setAll) validation", () => {
    it("should reject null with TypeError", () => {
      ...
      expect(() => raw.setAll(null)).toThrow(TypeError);
      expect(() => raw.setAll(null)).toThrow(/expected plain object/);
    });

    it("should reject arrays with TypeError", () => {
      const deps = getDependenciesApi(router);
      const raw = deps as unknown as { setAll(v: unknown): void };
      expect(() => raw.setAll([])).toThrow(TypeError);
      expect(() => raw.setAll(["a", "b"])).toThrow(/expected plain object/);
    });
    ...
    it("should reject class instances with TypeError", () => {
      ...
      const deps = getDependenciesApi(router);
      const raw = deps as unknown as { setAll(v: unknown): void };
      expect(() => raw.setAll(new MyClass())).toThrow(TypeError);
    });

    it("should reject Date objects with TypeError", () => {
      const deps = getDependenciesApi(router);
      const raw = deps as unknown as { setAll(v: unknown): void };
      expect(() => raw.setAll(new Date())).toThrow(TypeError);
    });
    ...
    it("should reject objects with getters", () => {
      ...
      expect(() => raw.setAll(objWithGetter)).toThrow(TypeError);
      expect(() => raw.setAll(objWithGetter)).toThrow(/Getters not allowed/);
    });

    it("should accept valid plain object", () => {
      const deps = getDependenciesApi(router);
      expect(() => deps.setAll({ foo: 2 })).not.toThrow();
    });
  });
  ...
  describe("removeDependency validation", () => {
    it("should throw TypeError for non-string parameters", () => {
      ...
      expect(() => raw.remove(null)).toThrow(TypeError);
    });
  });
