...
| `canSend(event, payload?)` | `true` ⟺ `send(event, payload)` from the same state and context WOULD fire — i.e. an edge exists and its `when` accepts. O(1) via the cached `#currentTransitions`. The payload is optional: a payload-independent condition answers honestly without one, a payload-dependent one answers whatever it returns for `undefined` — a refusal for some predicates, an accept for others (INVARIANT 4) |
...
- **`send` / `canSend` keep the correlated rest tuple in their OVERLOAD and take the payload POSITIONALLY in the implementation.** A rest parameter materialises an array on every call, and this is the router's hottest entry point — measured at **−88 B per navigation** when it was removed, with the p90 tail collapsing from 2384 to 2133 B (alloc probe, window 200, median of 31, A/A floor 0); timing was a wash (733 vs 734 ns), so it is purely a GC-pressure win. The tuple has to stay in the overload because only a conditional rest tuple can express the payload correlation below, and the split is what lets the engine have both. `@real-router/event-emitter` made the same trade for the same reason (`emit(name, a?, b?, c?, d?)`); the engine simply had not received it. Pinned structurally in `fsm.test.ts` — "dispatch signatures allocate no rest array" — because a benchmark number would not hold it, and the second assertion there pins the overload so the first cannot be satisfied by deleting the correlation.
...
- **Undeclared states are rejected at every state-entry-point** — `new FSM({ initial })`, `on(from, …)`, and every transition **target** in the table (validated for closure once at construction, #1159) each read `config.transitions` directly and throw `Internal error (please report): state "…" is not declared in config.transitions` instead of bricking the FSM (a dangling `initial` or transition target would leave `#currentTransitions` `undefined` → cryptic `TypeError` on the next `canSend`/`send`) or silently dead-registering an action (`on`, whose action could never fire). ⚑ **That last clause was FALSE for a year and is true since #1682** — the guard checked the STATE only, so `on(declaredState, eventWithNoEdgeFromIt, …)` (the exact shape the clause names) passed and dead-registered. `on` now runs a second check and throws `Internal error (please report): event "…" has no edge from state "…"`. One check covers both dead shapes, because the normalisation below drops an explicit `undefined` target: an absent pair and a declared no-op are indistinguishable by the time `on` sees them. Measured before it landed: across 4651 tests exactly ONE registration was affected, and it existed to pin the permissiveness. The other direction — an edge that should announce but registers no action — is not knowable to the engine (muteness is legitimate), so it is held one layer up, by `tests/functional/fsm-edge-reachability.test.ts`. The closure pass is O(states×events) on the cold construction path and **skips explicit `undefined` targets** (the declared "no transition" no-op); post-construction mutation of the shared table stays a documented GIGO boundary (see the `context`/`transitions` shared-by-reference gotcha below). The type forbids undeclared states for a **narrow** state union; the guard hardens `string`-typed / JS / cast callers, mirroring `send()`'s defensiveness on unknown input. Resolved in [#754](https://github.com/greydragon888/real-router/issues/754) (originally the `forceState` guard) + [#885](https://github.com/greydragon888/real-router/issues/885) (constructor + `on`) + [#1159](https://github.com/greydragon888/real-router/issues/1159) (table targets) + [#1682](https://github.com/greydragon888/real-router/issues/1682) (the edge axis of `on`). (`forceState` itself was removed once core stopped using it — see CHANGELOG.)
