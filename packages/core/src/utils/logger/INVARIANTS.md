...
| 8   | Invalid levels throw                                     | Passing an unrecognized string as `level` — including `Object.prototype` keys like `"toString"` — throws an error matching `/Invalid logger level/`. Validation is own-property (`Object.hasOwn`), so inherited keys are never silently accepted. ⚑ True of BOTH doors only since #1814: `assertLoggerConfig` asked `"level" in obj` while `configure` asked `hasOwn`, so this row described the second door while the first REFUSED an inherited garbage level — a false rejection of a bag whose own keys are empty.                       |
