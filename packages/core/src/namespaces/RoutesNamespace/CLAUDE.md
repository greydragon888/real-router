...
| **Read-only in the TYPE, shallow at RUNTIME** | The arrays are `readonly ReadonlyRoute<D>[]`, so a write at ANY depth is a compile error. At runtime the node is frozen and so is its nested config — `event.added[0].defaultParams` is core's own snapshot, taken at registration and frozen there (#2172), not the object the caller passed. A guard FACTORY is the exception: a function is not snapshotted, so it stays the caller's |
