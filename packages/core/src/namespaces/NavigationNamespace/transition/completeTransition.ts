...
/**
 * ⛔ **`opts` here is a KNOWN defect, and snapshotting it away was tried,
 * shipped, measured and REVERTED — do not redo it without reading #1722.**
 *
 * Reading the caller's object inside the commit is what forces the ordering rule
 * below (the meta is built ABOVE the ask so a `stop()`/`dispose()` from a getter
 * is still something the verdict can see). #1719 removed it: the three flags
 * were snapshotted at the navigation's entry and read off the plan here instead,
 * which emptied the window structurally. It cost **≈15 %** on
 * `navigate/sync-baseline` and ≈12 % on `navigate/pre-commit-listener`, and the
 * cause was isolated to exactly this line — six configurations on the runner,
 * with field count, the entry reads and the call shape each ruled out:
 *
 * | where the flags come from | plan fields | `navigate/sync-baseline` |
 * | --- | --: | --- |
 * | `opts` (this form) | 17 | 8.3 ms |
 * | `opts`, plus one unused field | 18 | 8.3 ms |
 * | the PLAN | 18 (packed) | 9.8 ms |
 * | the PLAN | 20 | 9.8 ms |
 *
 * The mechanism is not understood. It is the same shape as #1704, whose first
 * form cost 13.4 % for extracting a helper while the identical code inline cost
 * nothing, with the literal, module size, call position and the runner each
 * ruled out by measurement. So the rule here is empirical, not principled:
 * **this function reads `opts`, and the ordering rule below is what pays for
 * it.**
 */
