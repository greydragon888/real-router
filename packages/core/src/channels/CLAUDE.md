...
- **`admittedSearch`'s drop branch is the only unfrozen hand-back in the subsystem, so it freezes** — `search` arrives frozen from `mergeQueryChannel` and the no-drop branch returns it untouched. ⚠ That is a description, NOT the reason the query channel keeps its merge-time freeze while the path channel lost one (#1928): measured, moving the query freeze to `materialize` leaves the suite green, so the split is perf-gated by #1598's cited figure and nothing else. Before nav-pipeline Phase 2 the gap was invisible (every consumer re-merged and re-froze downstream); `materialize` deliberately does not, and Phase 4 removed the last re-merge, so this freeze is the only one on the drop path.
