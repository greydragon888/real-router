...
- **`admittedSearch`'s drop branch is the only unfrozen hand-back in the subsystem, so it freezes** — `search` arrives frozen from `mergeQueryChannel` and the no-drop branch returns it untouched — which is exactly why the query channel keeps its merge-time freeze while the path channel lost one (#1928). Before nav-pipeline Phase 2 the gap was invisible (every consumer re-merged and re-froze downstream); `materialize` deliberately does not, and Phase 4 removed the last re-merge, so this freeze is the only one on the drop path.
