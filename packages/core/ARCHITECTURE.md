...
| Options           | Shallow-copied via spread, with two substitutions: the clone builds its own logger config from the source router's live logger, and it takes `urlParamsEncoding` already-resolved from the source store rather than re-deriving it from the raw option |
