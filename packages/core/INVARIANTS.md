...
| 6   | Hop defaults land in the TARGET's channel (#1570) | `params(①) ∩ queryNames(target) = ∅`: a hop can only SPELL a default in `defaultParams` (its single slot), but the channel belongs to the resolved target — a key the target declares with `?` is layered under `search`, everything else under `params`, and an explicit caller value beats the default in either channel. A name that also occupies a path slot (`/dst/:id?id`) stays path-owned, inheriting the #843/#1549 carve-out rather than re-deciding it. Stated over the RAW stage-① output (what a `forwardState` interceptor reads from `next(...)`): the seam's `separateChannels` moves the key one line later, so the committed state was already correct and only a producer-level assertion can see the classification. Property-verified in `tests/property/queryChannelRegistry.properties.ts`; mutation-checked (removing the split reddens this property and the four `#1570` functional cases, and nothing else). |
