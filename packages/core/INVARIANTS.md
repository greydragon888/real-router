...
| 9   | Total — no input makes it throw    | The predicate ANSWERS for every input, from inside a render (#1577). Both branches READ the caller's bags — the exact branch splits them (`separateChannels`), the descendant branch spreads them into one — so an accessor-backed key, a `Proxy` or a reactive object throws there; #1573 had wrapped only the destination arm's `forwardState`. One boundary now covers the whole walk (the shape `isParams` took for the same class, #1052): a throw yields `false` + `logger.warn`, never an exception escaping into a render. Sibling of canNavigateTo #5. |
...
## canonicalize (nav pipeline, milestone 1)

`canonicalize(port, name, params, search)` is the single producer of `Canonical` — the opaque intent that `buildURL` (⑤a) and `materialize` (⑤b) are the only consumers of (`src/pipeline/`). It runs stage ① (forwardTo resolution through the interceptor seam, via the port) and stage ③ (route defaults merged UNDER the caller's value) in one pass; there is no separating stage — channels arrive correct by the producer contract. Tested as a PURE function against a mocked port (`tests/property/canonicalize.properties.ts`), which is the only place the merge is observable without the seam, the URL build and forwardTo entangled with it.
...
| 2   | `undefined` is absence on both sides                   | No own key of either channel ever holds `undefined`; a key that is `undefined` on BOTH sides is absent entirely. Same rule as makeState #5 — one merge helper (`mergeDefined`), one source of truth, so it cannot drift between the two stage-③ callers.                                       |
...
| 4   | Channels are frozen at merge time                      | `Object.isFrozen(Canonical.path / .query)` regardless of `materialize`'s `skipFreeze`, which governs only the STATE OBJECT (the navigate path defers it so `completeTransition` can attach `transition`). The caller's own bag is never frozen — the merge copies first. Guards therefore always observe immutable bags. |
| 5   | Stage ③ is idempotent                                  | Re-canonicalizing a `Canonical`'s own channels under the same route defaults changes nothing. This is the premise behind `materialize` not calling `makeState` (a second merge would be a wasted pass, never a behaviour change) and what makes a mixed old/new path safe mid-migration.        |
