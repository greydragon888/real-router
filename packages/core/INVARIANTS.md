...
| 2a  | Every entry point can spell both channels (#1571) | `navigate` / `buildPath` / `canNavigateTo` / `isActiveRoute` / `makeState` / `buildNavigationState` all accept a query channel, and in every one an explicit `search` value beats a declared query key the caller rode in the `params` bag. `buildNavigationState` was the last without the slot, so a query intent through it was unspellable — the always-on channel guard would have made it unusable and broken its only consumer. |
