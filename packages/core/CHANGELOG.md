...
- [#2273](https://github.com/greydragon888/real-router/pull/2273) [`36392a5`](https://github.com/greydragon888/real-router/commit/36392a513c658c6781f692e41712f7c089f9b766) Thanks [@greydragon888](https://github.com/greydragon888)! - `PluginApi.makeState` stops repeating the guards its adapter runs ([#2259](https://github.com/greydragon888/real-router/issues/2259))
...
- [#2268](https://github.com/greydragon888/real-router/pull/2268) [`6639d4b`](https://github.com/greydragon888/real-router/commit/6639d4be26dda98eb2a04ae85bfc89272f72e0fc) Thanks [@greydragon888](https://github.com/greydragon888)! - `systemCommit` refuses a route name the table does not hold ([#2252](https://github.com/greydragon888/real-router/issues/2252))
...
- [#2267](https://github.com/greydragon888/real-router/pull/2267) [`8df33d8`](https://github.com/greydragon888/real-router/commit/8df33d8216df6d202e25edca6a84aaef17a16a60) Thanks [@greydragon888](https://github.com/greydragon888)! - An internal door refuses what its guarded sibling refuses ([#2259](https://github.com/greydragon888/real-router/issues/2259))
...
- [#2263](https://github.com/greydragon888/real-router/pull/2263) [`c823c78`](https://github.com/greydragon888/real-router/commit/c823c78ef51cbe884999e5cc74b3c679b9de4461) Thanks [@greydragon888](https://github.com/greydragon888)! - A guard's promise is honoured from any realm ([#2251](https://github.com/greydragon888/real-router/issues/2251))
  ...
  - **`then` is read ONCE** and the captured function is what gets called. Deciding
    on read [#1](https://github.com/greydragon888/real-router/issues/1) and letting `await` take read [#2](https://github.com/greydragon888/real-router/issues/2) is the defect [#2136](https://github.com/greydragon888/real-router/issues/2136) measured on the
...
- [#2222](https://github.com/greydragon888/real-router/pull/2222) [`be181b8`](https://github.com/greydragon888/real-router/commit/be181b876a28627aaac4d14f2ba3b4332f1623c8) Thanks [@greydragon888](https://github.com/greydragon888)! - The pending navigation target is sealed before application code sees it
  ...
  **`nextRoute` and the later `getState()` are no longer the same object.** They agree on `name`, `params`, `search` and `path`. They differ on `transition`, and always did — the meta is built from the transition's _outcome_, which does not exist when the payload is handed over. Sharing one object hid that by retro-filling the very object the subscriber was holding, so a subscriber that stored `nextRoute` and read `.transition.segments` later will now read the pending value (`DEFAULT_TRANSITION`) rather than a meta that appeared under it. Read the committed meta from the `subscribe` payload or `getState()`.
  ...
  Closes [#2144](https://github.com/greydragon888/real-router/issues/2144).

## 0.129.1

### Patch Changes

- [#2212](https://github.com/greydragon888/real-router/pull/2212) [`9169e10`](https://github.com/greydragon888/real-router/commit/9169e10d471a4b156db3ef8d7557751a6d37068f) Thanks [@greydragon888](https://github.com/greydragon888)! - `getAdoptedOrigins` hands back a frozen record

  `RouterInternals.getAdoptedOrigins()` returned `Router`'s `#adoptedOrigins` field itself — the same object on every call, and unfrozen. A single write into it silently and permanently disabled the diagnostic [#2148](https://github.com/greydragon888/real-router/issues/2148) exists to provide, for that router and for every consumer of it: `DefaultsMutationWatch.watch` skips a slot whose origin is `undefined`, so
  ...
  **The record is frozen, not the bags.** The `WeakRef` targets are the application's own `defaultParams` / `defaultSearch`, and freezing those would break the very mutation the diagnostic exists to notice.
  ...
  The rule is `packages/core/CLAUDE.md`'s **cached ⟹ frozen** ([#1805](https://github.com/greydragon888/real-router/issues/1805)) — "one instance is shared by every consumer of a router and a single member assignment rewires it for all of them".
  ...
  **It was not the only unfrozen one.** Asked of all 32 `getInternals` members rather than of a shortlist, six hand back a cached object and four of those were unfrozen: `getAdoptedOrigins`, `dependenciesGetStore`, `port` and `routeGetStore`. The last three are live stores core keeps reading, which is exactly what the `internals` carve-out in `handed-out-containers-1957` was written for. This one is not a store — `git grep '#adoptedOrigins'` returns three sites, all in `Router.ts`: the declaration, one write in the constructor, one read in the `registerInternals` closure. `getOptions` and `getTree` are cached and already frozen, and `getCloneState` mints a fresh literal per call, where a freeze would certify nothing.
  ...
  Closes [#2195](https://github.com/greydragon888/real-router/issues/2195).

- [#2212](https://github.com/greydragon888/real-router/pull/2212) [`9169e10`](https://github.com/greydragon888/real-router/commit/9169e10d471a4b156db3ef8d7557751a6d37068f) Thanks [@greydragon888](https://github.com/greydragon888)! - Remove `PluginsNamespace.has`, an internal method with no callers
  ...
  Closes [#2205](https://github.com/greydragon888/real-router/issues/2205).
...
- [#2193](https://github.com/greydragon888/real-router/pull/2193) [`2cf4754`](https://github.com/greydragon888/real-router/commit/2cf4754458284abf5b89be09f88ecf55b5c31ae9) Thanks [@greydragon888](https://github.com/greydragon888)! - `systemCommit` adopts a foreign `transition` at every level the shape declares, not just the meta

  `getInternals(router).systemCommit(state, …)` is the one door that **commits** a `State` built outside core — `navigateToState` accepts one too, but runs it through the pipeline, which constructs and freezes its own. So this door copies the shell field by field, and nothing the caller still holds is committed as core's own ([#1792](https://github.com/greydragon888/real-router/issues/1792)). The copy stopped at the meta. `Object.freeze` is shallow, so `transition.segments` and the two arrays inside it stayed the caller's objects, live and unfrozen, while core handed them out as part of its own state:
  ...
  **Behaviour change: both nullish spellings of an absent `transition` are now absence.** The conditional that guards the slot tested `!== undefined`, so `transition: null` fell through to the adoption's empty answer — the shared `EMPTY_PARAMS` singleton, committed under a type that declares `phase`, `reason` and `segments` as required. `getState().transition` was then the same object as some other state's `getState().params`. It is omitted now, exactly as `undefined` already was; `State.transition` being reachable as absent at this door is the pre-existing, pinned behaviour that `RoutesNamespace.shouldUpdateNode` reads through `?.` for.
  ...
  Closes [#2140](https://github.com/greydragon888/real-router/issues/2140).
...
- [#2179](https://github.com/greydragon888/real-router/pull/2179) [`7852b02`](https://github.com/greydragon888/real-router/commit/7852b02b2ae4179b79095a905ba8495a81452aaa) Thanks [@greydragon888](https://github.com/greydragon888)! - A navigation option the caller never supplied no longer reaches the navigation ([#2132](https://github.com/greydragon888/real-router/issues/2132))
  ...
  [#1962](https://github.com/greydragon888/real-router/issues/1962) (2026-08-30), the change that introduced the entry-door copy. That copy DID
...
- [#2173](https://github.com/greydragon888/real-router/pull/2173) [`ac17690`](https://github.com/greydragon888/real-router/commit/ac1769058a5c9f855ed520fe62d43ceff1c51cd9) Thanks [@greydragon888](https://github.com/greydragon888)! - Option sub-bags are adopted at construction, and a clone inherits its base's query strategies ([#2171](https://github.com/greydragon888/real-router/issues/2171))

  Until now core froze the options level its own spread minted and stopped there
  ([#1832](https://github.com/greydragon888/real-router/issues/1832)), so every nested bag stayed the caller's object: `defaultParams` and
  ...
  the level above does: the object is one core now mints, and [#1957](https://github.com/greydragon888/real-router/issues/1957)'s exemption for
  ...
  still doing so, after `urlParamsEncoding` ([#1877](https://github.com/greydragon888/real-router/issues/1877)), `limits` ([#1880](https://github.com/greydragon888/real-router/issues/1880) / [#1961](https://github.com/greydragon888/real-router/issues/1961)) and
  `logger` ([#1930](https://github.com/greydragon888/real-router/issues/1930)) each stopped as bug fixes. That re-read was never the harmless
  ...
  URL from its base with **no error and no warning** ([#2032](https://github.com/greydragon888/real-router/issues/2032)), against three shipped
  ...
  ([#2148](https://github.com/greydragon888/real-router/issues/2148)). An application that builds its config before constructing the router —
...
reading it threw` with the original as `cause` — matching what `queryParams`
  ...
  Decision: [#2145](https://github.com/greydragon888/real-router/issues/2145). Umbrella: [#1901](https://github.com/greydragon888/real-router/issues/1901).

- [#2173](https://github.com/greydragon888/real-router/pull/2173) [`ac17690`](https://github.com/greydragon888/real-router/commit/ac1769058a5c9f855ed520fe62d43ceff1c51cd9) Thanks [@greydragon888](https://github.com/greydragon888)! - Route configuration is adopted at registration, and the route record is frozen ([#2172](https://github.com/greydragon888/real-router/issues/2172))

  The other half of the decision [#2145](https://github.com/greydragon888/real-router/issues/2145) took, after [#2171](https://github.com/greydragon888/real-router/issues/2171) did the `Options` door.
...
...route.defaultParams, locale } })`.
  ...
  navigation-time door reads it again — the read-count table for `[#1847](https://github.com/greydragon888/real-router/issues/1847)` went from
  ...
  the caller registered, all the way down" — accurate when [#1963](https://github.com/greydragon888/real-router/issues/1963) wrote it, and false
  ...
  Decision: [#2145](https://github.com/greydragon888/real-router/issues/2145). First half: [#2171](https://github.com/greydragon888/real-router/issues/2171). Parent: [#1958](https://github.com/greydragon888/real-router/issues/1958). Umbrella: [#1901](https://github.com/greydragon888/real-router/issues/1901).
...
- [#2127](https://github.com/greydragon888/real-router/pull/2127) [`094bfbe`](https://github.com/greydragon888/real-router/commit/094bfbe0899c61e7310d56e6609d4ebed1450d91) Thanks [@greydragon888](https://github.com/greydragon888)! - Nine docblocks carried counts that had stopped being true — four of them stated the wrong thing, not a stale number ([#2126](https://github.com/greydragon888/real-router/issues/2126))
