 ...
 *
 * Covers: parseQuery, build functions with various options.
 *
...
import type { Options } from "../../../../src/engine/search-params";

describe("../../../src/search-params", () => {
  // ===========================================================================
  // parseQuery
  // ===========================================================================
  ...
  describe("parseQuery", () => {
    it("uses the same auto defaults as build when no options are given", () => {
      // build() without options uses auto strategies; parseQuery() must match so
      // parseQuery(build(x)) === x without options (no silent type loss). (#744)
      const original = { n: 5, flag: true, sort: "name" };

      expect(parseQuery(build(original))).toStrictEqual(original);
    });
    ...
    it("handles multiple values for same parameter", () => {
      expect(parseQuery("id=1&id=2&id=3")).toStrictEqual({
        id: [1, 2, 3],
      });
    });
    ...
    it("handles multiple values for same parameter with options", () => {
      ...
      expect(
        parseQuery("id=1&id=2&id=3", { booleanFormat: "auto" }),
      ).toStrictEqual({
        id: [1, 2, 3],
      });
    });
    ...
    it("handles single bracket parameter as array", () => {
      ...
      const result = parseQuery("items[]=a", { arrayFormat: "brackets" });
    ...
    it("index format: non-numeric/empty/unclosed brackets fall back to insertion order", () => {
      ...
      expect(parseQuery("a[]=x&a[]=y", o)).toStrictEqual({ a: ["x", "y"] }); // "[]"
      expect(parseQuery("a[k]=v", o)).toStrictEqual({ a: ["v"] }); // non-digit
      expect(parseQuery("a[=v", o)).toStrictEqual({ a: ["v"] }); // nothing after "["
      expect(parseQuery("a[2=v", o)).toStrictEqual({ a: ["v"] }); // no closing "]"
    });

    it("index format: a non-bracketed key stays a scalar", () => {
      expect(
        parseQuery("a=v", { arrayFormat: "index", numberFormat: "none" }),
      ).toStrictEqual({ a: "v" });
    });
    ...
    it("index format: an indexed group displaces a bare scalar for the same key (#1319)", () => {
      ...
      expect(parseQuery("a=1&a[0]=x", o)).toStrictEqual({ a: ["x"] });
      expect(parseQuery("a[0]=x&a=1", o)).toStrictEqual({ a: ["x"] });
    });

    it("handles comma-separated arrays", () => {
      expect(parseQuery("items=a,b,c", { arrayFormat: "comma" })).toStrictEqual(
        {
          items: ["a", "b", "c"],
        },
      );
    ...
    it("handles single value with comma format (no comma = scalar)", () => {
      expect(parseQuery("q=hello", { arrayFormat: "comma" })).toStrictEqual({
        q: "hello",
      });
    });
    ...
    it("comma format roundtrip preserves array values", () => {
      ...
      expect(parseQuery(build(original, opts), opts)).toStrictEqual(original);
    });

    it("comma format with numberFormat auto decodes numeric elements", () => {
      expect(
        parseQuery("ids=1,2,3", { arrayFormat: "comma", numberFormat: "auto" }),
      ).toStrictEqual({
        ids: [1, 2, 3],
      });
    });
    ...
    it("decodes URI components", () => {
      expect(
        parseQuery("name=%D0%BF%D1%80%D0%B8%D0%B2%D0%B5%D1%82"),
      ).toStrictEqual({
        name: "привет",
      });
    });
    ...
    it("handles empty-true boolean format", () => {
      expect(
        parseQuery("enabled", { booleanFormat: "empty-true" }),
      ).toStrictEqual({
        enabled: true,
      });
    });

    it("decodes empty-true false back to a boolean (build/parseQuery roundtrip)", () => {
      // build({ flag: false }, empty-true) emits "flag=false"; parseQuery must
      // round-trip it back to boolean false, not the string "false". (#743)
      expect(
        parseQuery("flag=false", { booleanFormat: "empty-true" }),
    ...
    it("decodes empty-true booleans in arrays without true/false asymmetry", () => {
      ...
      expect(
        parseQuery("a=true&a=false", { booleanFormat: "empty-true" }),
    ...
    it("empty-true reserves the bare key for true, so null is not representable", () => {
      ...
      expect(parseQuery("flag", opts)).toStrictEqual({ flag: true });
    ...
    it("handles auto boolean format", () => {
      expect(
        parseQuery("enabled=true&disabled=false", { booleanFormat: "auto" }),
      ).toStrictEqual({
        enabled: true,
        disabled: false,
      });
    ...
    it("handles auto number format", () => {
      expect(
        parseQuery("page=1&limit=20&sort=name", { numberFormat: "auto" }),
      ).toStrictEqual({
        page: 1,
        limit: 20,
        sort: "name",
    ...
    it("handles auto number format with decimals and non-numeric values", () => {
      expect(
        parseQuery("id=42&name=abc&price=12.5", { numberFormat: "auto" }),
      ).toStrictEqual({
        id: 42,
        name: "abc",
        price: 12.5,
    ...
    it("decodes negative numbers as numbers under auto (matches navigate/build roundtrip)", () => {
      // build({ n: -5 }) emits "n=-5"; parseQuery must round-trip it back to a number,
      // mirroring the type a programmatic navigate({ n: -5 }) keeps. (#742)
      expect(
        parseQuery("n=-5&d=-5.5&z=0", { numberFormat: "auto" }),
      ).toStrictEqual({
        n: -5,
        d: -5.5,
        z: 0,
    ...
    it("keeps non-canonical negatives as strings under auto", () => {
      ...
      expect(
        parseQuery("a=-007&b=-9007199254740992", { numberFormat: "auto" }),
      ).toStrictEqual({
        a: "-007",
        b: "-9007199254740992",
      });
    ...
    it("keeps negative zero as a string under auto (not round-trippable)", () => {
      ...
      expect(parseQuery("a=-0&b=-0.0", { numberFormat: "auto" })).toStrictEqual(
        {
          a: "-0",
          b: "-0.0",
        },
    ...
    it("treats keys shadowing Object.prototype members as plain params", () => {
      ...
      expect(
        parseQuery("valueOf=1&constructor=2&toString=3&hasOwnProperty=4", {
          numberFormat: "none",
        }),
      ).toStrictEqual({
        valueOf: "1",
    ...
    it("decodes a literal __proto__ key as an own property (no prototype pollution)", () => {
      const result = parseQuery("__proto__=x", { numberFormat: "none" });
    ...
    it("accumulates repeated Object.prototype-named keys into an array", () => {
      expect(
        parseQuery("toString=a&toString=b", { numberFormat: "none" }),
      ).toStrictEqual({ toString: ["a", "b"] });
    });

    it("parses key with empty value after = as empty string", () => {
      expect(parseQuery("key=")).toStrictEqual({ key: "" });
    });

    it("parses multiple keys with empty values", () => {
      expect(parseQuery("a=&b=&c=value")).toStrictEqual({
        a: "",
        b: "",
        c: "value",
      });
    ...
    it("throws URIError for incomplete percent-encoding (%2)", () => {
      expect(() => parseQuery("key=%2")).toThrow(URIError);
    });

    it("throws URIError for invalid hex in percent-encoding (%GG)", () => {
      expect(() => parseQuery("key=%GG")).toThrow(URIError);
    });

    it("handles combined booleanFormat and numberFormat", () => {
      expect(
        parseQuery("enabled=true&disabled=false&count=1&price=12.5&name=abc", {
          booleanFormat: "auto",
          numberFormat: "auto",
        }),
      ).toStrictEqual({
  ...
  // ===========================================================================
  // parseQuery — does NOT split its input at "?" (#1292 characterization)
  // ===========================================================================

  describe("parseQuery (no input splitting, #1292)", () => {
    it("keeps a '?' inside the input — the whole string is the query, never re-split", () => {
      // The input is already the extracted query, so a "?" is data, not a
      // delimiter. A path-accepting wrapper re-running a leading-"?" split would
      // break here and drop the param — the #1292 bug shape, now unrepresentable
      // since that wrapper is gone.
  ...
  describe("build", () => {
    ...
    it("throws TypeError for object elements in arrays", () => {
      ...
      expect(() => build({ items: [{ nested: "value" }] })).toThrow(
        "[router] Invalid query value: an array element must be a string, number, or boolean — received object",
      );
    });
    ...
    it("encodes null elements in arrays as the format's bare-key form (#1155)", () => {
      ...
      // format under nullFormat "default", so parseQuery round-trips it back.
  ...
  // ===========================================================================
  // Value decoding (decode.ts decodeValue) — through parseQuery
  //
  // `decodeValue` is exercised on every parsed VALUE (and name). Migrated off a
  // direct `decodeValue` import to the public `parseQuery`: each branch of the
  ...
  describe("value decoding (%/+ fast-path ladder)", () => {
    it("fast path returns a value unchanged when it has neither % nor +", () => {
      ...
      expect(
        parseQuery("k=simplevalue123", { numberFormat: "none" }),
      ).toStrictEqual({ k: "simplevalue123" });
    });
    ...
    it("replaces + with space when only + is present (no decodeURIComponent)", () => {
      ...
      expect(parseQuery("k=a+b+c", { numberFormat: "none" })).toStrictEqual({
        k: "a b c",
      });
      // A bare "+" value decodes to a single space.
      expect(parseQuery("k=+", { numberFormat: "none" })).toStrictEqual({
        k: " ",
      });
    });

    it("percent-decodes when only % is present (no + replacement)", () => {
      expect(parseQuery("name=hello%20world")).toStrictEqual({
        name: "hello world",
      });
      expect(
        parseQuery("name=%D0%BF%D1%80%D0%B8%D0%B2%D0%B5%D1%82"),
      ...
      expect(parseQuery("k=%21", { numberFormat: "none" })).toStrictEqual({
        k: "!",
      });
    });
    ...
    it("processes both + and % together, replacing + BEFORE decoding %", () => {
      ...
      expect(
        parseQuery("k=hello+world%21", { numberFormat: "none" }),
      ).toStrictEqual({ k: "hello world!" });
      expect(
        parseQuery("name=%D0%BF%D1%80%D0%B8%D0%B2%D0%B5%D1%82+world"),
      ).toStrictEqual({ name: "привет world" });
    });
  });

  // ===========================================================================
  // Option resolution (encode.ts makeOptions) — through parseQuery/build
  //
  // Migrated off a direct `makeOptions` import. The RESOLVED defaults and the
  // partial-override precedence are all observable in parseQuery/build behavior; each
  ...
  describe("option resolution (defaults + partial overrides)", () => {
    it("no options resolves to the documented defaults (none/auto/default/auto)", () => {
      ...
      expect(parseQuery("flag=true")).toStrictEqual({ flag: true });
      ...
      expect(parseQuery("n=5")).toStrictEqual({ n: 5 });
    });
    ...
    it("only arrayFormat set — other three fall back to defaults", () => {
      ...
      expect(
        parseQuery("flag=true&n=5&x", { arrayFormat: "brackets" }),
      ).toStrictEqual(
    ...
    it("only booleanFormat set — other three fall back to defaults", () => {
      ...
      expect(
        parseQuery("a=1&a=2&flag&n=5", { booleanFormat: "empty-true" }),
      ).toStrictEqual({
    ...
    it("only nullFormat set — other three fall back to defaults", () => {
      ...
      expect(parseQuery("flag=true", { nullFormat: "hidden" })).toStrictEqual({
    ...
    it("only numberFormat set — other three fall back to defaults", () => {
      ...
      expect(
        parseQuery("n=5&flag=true&x", { numberFormat: "none" }),
      ).toStrictEqual({
        n: "5", // number none
    ...
    it("all four options set — every field takes the provided value", () => {
      ...
      expect(parseQuery("a[0]=1&a[1]=2&flag", opts)).toStrictEqual({
        a: [1, 2],
        flag: true,
      });
    });
  ...
  describe("inverse-pair domain closure (#1155/#1156)", () => {
    it('skips empty chunks instead of injecting a junk "" param (#1156)', () => {
      expect(parseQuery("&a=1")).toStrictEqual({ a: 1 }); // leading
      expect(parseQuery("a=1&&b=2")).toStrictEqual({ a: 1, b: 2 }); // interior
      expect(parseQuery("a=1&")).toStrictEqual({ a: 1 }); // trailing
      expect(parseQuery("x=1&&&x=2")).toStrictEqual({ x: [1, 2] }); // double empty
    });
    ...
    it("preserves an intentional empty-key chunk that carries a value (#1156)", () => {
      ...
      expect(parseQuery("=1")).toStrictEqual({ "": 1 });
    });

    it("round-trips null array elements produced by parseQuery (#1155)", () => {
      // none: bare key mixed with valued keys
      expect(parseQuery("a&a=1")).toStrictEqual({ a: [null, 1] });
      expect(build(parseQuery("a&a=1"))).toBe("a&a=1");

      // brackets
      const br = { arrayFormat: "brackets" } as const;

      expect(parseQuery("a[]&a[]=x", br)).toStrictEqual({ a: [null, "x"] });
      expect(build(parseQuery("a[]&a[]=x", br), br)).toBe("a[]&a[]=x");

      // index
      const ix = { arrayFormat: "index" } as const;

      expect(parseQuery("a[0]&a[1]=x", ix)).toStrictEqual({ a: [null, "x"] });
      expect(build(parseQuery("a[0]&a[1]=x", ix), ix)).toBe("a[0]&a[1]=x");
    });

    it("sanitizes a lone surrogate to U+FFFD instead of throwing (safeEncode, #1314)", () => {
      // `parseQuery` accepts a literal lone surrogate (identity decode), so `build` must
      // stay total on it. Both encode sites — scalar/key and array element — route
      // through `safeEncode`.
      expect(build({ a: "\uD800" })).toBe("a=%EF%BF%BD");
      expect(build({ a: ["\uD800"] })).toBe("a=%EF%BF%BD");
      expect(build(parseQuery("a=\uD800"))).toBe("a=%EF%BF%BD");
    });
...
// did not pin. Each kills a specific survivor via the public parseQuery/build.
// =============================================================================
...
describe("mutation guards (observable-behavior kills)", () => {
  ...
  it("auto number keeps exponent / hex forms as strings", () => {
    expect(parseQuery("n=1e5")).toStrictEqual({ n: "1e5" });
    expect(parseQuery("n=0x1f")).toStrictEqual({ n: "0x1f" });
  });
  ...
  it("builds arrays of valid primitive elements without throwing", () => {
    ...
    expect(parseQuery(built, { arrayFormat: "comma" })).toStrictEqual({
      x: ["a", 1, true],
    });
  });
  ...
  // searchParams.ts parseQuery loop bound: a trailing "&" must NOT spawn an
  ...
  it("a literal __proto__ key is an enumerable, redefinable own property", () => {
    const single = parseQuery("__proto__=x", { numberFormat: "none" });
  ...
  it("a key-only param before a valued one stays key-only", () => {
    expect(parseQuery("a&b=1")).toStrictEqual({ a: null, b: 1 });
  });
  ...
  it("comma format does not split a key-only chunk", () => {
    expect(parseQuery("x,y", { arrayFormat: "comma" })).toStrictEqual({
      "x,y": null,
    });
  });
