...
| `consumers.test.ts`    | Who actually REACHES for a member? Three call idioms, plus what a shipped consumer reads one level down.                                                                         |
...
## Four of these are repo-wide scans

`application`, `consumers`, `core-config` and `reachability` read beyond their
own workspace, so turbo's per-package cache cannot answer for them. They are
registered in `scripts/repo-wide-scans.json` and run by `pnpm lint:repo-scans`;
