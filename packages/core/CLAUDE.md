...
| **Errors** | A throwing handler is isolated via `onListenerError`; other handlers still run and the CRUD caller does not see a re-throw — **except** `RecursionDepthError`, which propagates. After the #1032 ban, `RecursionDepthError` no longer comes from reentrant CRUD; it bounds a **runaway listener-driven nested same-event emit** — e.g. a `router.subscribe` listener that calls `replace()` unconditionally, whose revalidation re-emits `TRANSITION_SUCCESS` (#950) and re-enters the listener past `maxEventDepth = 5`. Re-exported from `@real-router/core` for `instanceof` checks. |
