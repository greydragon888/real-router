...
- **unsafe-key guard** (#1792) — an own `__proto__` in the caller's `params` or `search` bag. `__proto__` is the only ACCESSOR among `Object.prototype`'s twelve own members, so `bag[key] = value` for it reaches the inherited setter, creates no own key, and the value is gone with no error and no log. Sits at the same three producers as the channel guard and throws for the same reason: an ARGUMENT-SHAPE defect at the API boundary, caught before any transition exists.
  ...
  ⚑ **Where each half sits is a perf decision, and `canonicalize` is deliberately NOT one of the sites.** `canonicalize` is shared by all seven producers, so a check there is paid by six that cannot reach the case; measured, hosting it there cost `navigate` +5.3 % against `origin/master` versus +2.4 % now (40 paired rounds, sign holding 36 of 40). The drop lives at the ONE wire entry, `RoutesNamespace.matchPath`, where `search` was parsed out of a URL. The refusal lives at two places: each caller-facing producer's own entry, and the `forwardState` seam for anything an interceptor, a `decodeParams` or a dynamic `forwardTo` hands back — gated on the chain having produced NEW bags, so the common path pays two reference compares. Same always-on-fixes / opt-in-diagnoses split.
