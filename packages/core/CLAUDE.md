...
**Stage ② (channel separation) did NOT leave with the migration.** It lives in the `forwardState` SEAM, which the port itself calls, so the three class-① points that reach the seam still get it; only `isActiveRoute`, which owned its own `separateChannels` call, is genuinely free of it. Phase 4 removes the seam's wrapper.
