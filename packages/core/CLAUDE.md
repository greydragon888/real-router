...
**A route's own defaults are routed by the same registry, from both slots (#1549).** `defaultParams` and `defaultSearch` do not decide the channel — the route's `?`-declaration does, exactly as a `forwardTo` hop's defaults are routed by the *destination*'s declaration (#1570). `defaultSearch` is layered last and so outranks the query half of `defaultParams`; the caller outranks both. The split is applied wherever a route's defaults are merged — `pipeline/canonicalize`, `StateNamespace.makeState`, `RoutesNamespace.buildPath` and the `matchPath` rebuild — because the URL builders run BEFORE the state builders; splitting only in the latter publishes a `state.search` its own `state.path` contradicts. `buildPath` skips the split only for a v1 **single-bag** call — one where a route-declared query name actually rides in the `params` bag. Such a caller depends on the matcher's `search ?? params` fallback to print the query out of the one bag it passed, and a defined `search` would switch that fallback off. The test is the caller's BAG, not the absence of the `search` argument (#1578): keyed on `search === undefined`, it also skipped the defaults for a caller who named no query channel at all — the arm every adapter's `<Link>` takes — so the href dropped the route's `defaultSearch` while the click kept it, leaving `buildPath` the only producer disagreeing with `navigate` / `makeState` / the `matchPath` rebuild. Until #1549 each slot worked in exactly one position — a hop's `defaultSearch` was read by nobody, and a terminal's `defaultParams` for a declared query name stayed in the path bag, which made the P3 guard reject core's OWN state on `start()`.
...
**The same registry decides the channel of a `forwardTo` chain's defaults (#1570).** A forwarding hop can only spell a default in `defaultParams` — the single slot a route config gives it — but the channel belongs to the **resolved target**: `forwardState` folds the chain's defaults, then splits them through the same `separateChannels` / `getQueryParams` pair, layering the query half under `search` and the path half under `params`. **The caller beats the default whichever bag they named it in** — splitting alone did not give that: the default and a caller's params-twin ended up in different channels, where no merge ranks them, and the seam (spreading `search` last) let the DEFAULT win, so `navigate("src", { lang: "de" })` on a chain default `{ lang: "fr" }` committed `?lang=fr`. The fix withholds the default for a slot the caller already filled rather than rerouting the caller's key — channel correctness stays the producer's contract. `undefined` is absence on both sides (#1550/#1551), so a removal marker does not count as "filled" and the default survives it in either channel. Before this, a hop default for a target-declared `?key` rode out of stage ① in the path bag and was only rescued by channel separation at the seam one line later — invisible in the committed state, but wrong for every `forwardState` interceptor reading `next(...)`, and unfixable by the producer contract because the producer was core itself.
