...
src/channels/ (channel correctness — one rule, three mechanisms; `guard` is core's fifth always-on invariant guard and `unsafeKeys` its sixth, `modeGate` is deliberately NOT a guard)
...
Core contains six invariant guards that run regardless of whether validation-plugin is installed:
...
- **unsafe-key guard** (#1792) — an own `__proto__` in the caller's `params` or `search` bag. `__proto__` is the only ACCESSOR among `Object.prototype`'s twelve own members, so `bag[key] = value` for it reaches the inherited setter, creates no own key, and the value is gone with no error and no log. Throws for the same reason as the channel guard: an ARGUMENT-SHAPE defect at the API boundary, caught before any transition exists. Sits at the FOUR producers that build from a caller's bag — `navigate`, `makeState`, `buildNavigationState`, `buildPath` — and at the `forwardState` seam for whatever the chain returns. `buildPath` carries its own because the literal form (`resolveForward: false`) never reaches that seam; the two predicates carry none, answering `false` instead, since a predicate must not throw.

  ⚑ **The answer is keyed on the SOURCE of the data, not on the name** — that is the whole design, and carrying the key everywhere was measured and rejected first. Writing it as data does not keep the hazard in core, it EXPORTS it: once it survives into `state.params` / `state.search`, every consumer meets it, and `Object.assign` — how application code merges bags — drops it exactly as core did. Measured downstream, `logger-plugin`'s diff accumulator had its PROTOTYPE replaced by caller data and logged a blank line, `persistent-params-plugin` lost the key on every navigation, and `search-schema-plugin` corrupted the prototype of the bag it returns AS the state. So:

  | source                                                                | answer                                           | why                                                                                                                         |
  | --------------------------------------------------------------------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
  | the caller's per-navigation bag                                       | **refuse**, `TypeError`                          | the caller wrote the name; telling them beats any silent handling                                                           |
  | a URL's QUERY                                                         | **drop**, and report through `validation-plugin` | not the caller's code, and `match()` must never throw on input (#737) — a link from anywhere would crash a popstate handler |
  | an interceptor / `decodeParams` / dynamic `forwardTo` return          | **refuse**, `TypeError`                          | a plugin or a route's own code wrote that name; it is the caller case with one more hop                                     |
  | a URL's PATH segment                                                  | **nothing** — already normalised, silently       | `normalizeParams` plain-assigns, so the key cannot survive whatever we do; a check there buys a diagnostic, not a fix       |
  | a route's static config (#1788), a plugin's context namespace (#1191) | **preserve**, untouched                          | the author typed the name deliberately; no outside payload is involved                                                      |

  ⚑ **Where each half sits follows the SOURCE, and `canonicalize` is deliberately NOT one of the sites.** It is shared by all seven producers, so a drop there would silently swallow a key that six of them were handed by a CALLER, who should be told instead. The drop lives at the one wire entry, `RoutesNamespace.matchPath`, where `search` was parsed out of a URL. The refusal lives at each caller-facing producer's entry and at the `forwardState` seam, which covers anything an interceptor, a `decodeParams` or a dynamic `forwardTo` hands back — unconditionally, like the channel check beside it. Same always-on-fixes / opt-in-diagnoses split.

  ⚠ **The path row is a deliberate exception to this section's own discriminator**, so it is written down rather than left to look like an oversight. "Report where information was destroyed" would have the path channel report too: the key IS destroyed there. It does not, because reporting it is not free — the matcher would have to stop losing the key before `canonicalize` could see it (six decode and junction-merge writes), plus a second membership test per navigation, and that measured a real cost on the hottest path while changing no observable result. The rule the exception follows: when the fix and the diagnostic are the same code, `loss` decides; when the loss is already unavoidable and only the diagnostic is on offer, the hot path decides.
...
| **Immutability is shallow**                                   | The payload route object (and `update`'s `patch` envelope) is `Object.freeze`d, but **nested config is by reference and aliases the live store** — `event.added[0].defaultParams` is the same object the router reads on every navigation (same aliasing as `get()`), and it is NOT frozen. **Treat payloads as read-only**: mutating a nested field (`event.added[0].defaultParams.x = …`, a `patch.defaultParams`, an `encodeParams`/guard closure) corrupts router config. Core does not deep-freeze (that would freeze the caller's own input, see H-1) or deep-clone (circular refs / class instances). ⚑ The record is written as ordinary DATA for every field name: a `"__proto__"` patch key goes through `Object.defineProperty` (#1788), mirroring `claim.write` (#1191) and `assignParam` (#855) — plain assignment there dispatched into the inherited accessor and swapped the record's prototype, which plugins read by key. The route-name-keyed containers of the ROUTE STORE never had that exposure: `routeCustomFields` and every `RouteConfig` map is an `Object.create(null)` dictionary. ⚠ That was stated as covering the whole layer and did not: `RouteLifecycleNamespace`'s six guard-factory records are keyed by a route name too and were plain `{}` until #1801 — read by key they reported a `canDeactivate` nobody registered, enumerated they dropped a `"__proto__"` route's guard on `cloneRouter`. They are `Object.create(null)` now, so the claim holds for the layer as a whole. |
