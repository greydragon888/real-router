...
4. **Clear definition guards** — `clearDefinitionGuards()` preserves external guards; for a **both-slot** name (definition + external) it recompiles the compiled function from the surviving external factory (registration is last-add-wins, so the compiled slot may be the very definition guard being cleared — #1192)
...
**Guard origin tracking**: `RouteLifecycleNamespace` tracks guard origins with four Maps split by origin (`#definitionActivateFactories` / `#externalActivateFactories` / `#definitionDeactivateFactories` / `#externalDeactivateFactories`), populated via the `isFromDefinition` parameter on `addCanActivate()`/`addCanDeactivate()`. Registration is **last-add-wins regardless of origin** (`#registerHandler`) — NOT "external wins at compile". `clearDefinitionGuards()` clears the two definition Maps and, for a name that _also_ holds an external guard, **recompiles** the compiled-function slot from that surviving external factory (#1192) — so external guards survive `replace()` in behavior, not merely in the Map. External "wins" only where the code recompiles from it explicitly (`clearDefinitionGuards`, `#recompileSlot`).
