...
| Options                                                                                       | Shallow spread, with two substitutions: the clone's own resolved logger config, and the base's already-snapshotted `urlParamsEncoding` key (#1877) — so a non-string option is not coerced per clone, and the clone's `getOptions()` reports the key rather than the caller's value. Everything else is ref-shared, which is safe because options are deep-frozen |
