...
**On `forwardState`, `next()` hands back a core-owned SNAPSHOT (#1986).** The value an interceptor receives is not the object the link below it returned: both channel bags are stripped of `"__proto__"` and copied into a fresh literal at every hop. The reason is the HAND-OUT rule — merging the result is this seam's documented idiom, so what one interceptor gives the next is a hand-out in the #1957 sense, and an own `"__proto__"` riding through swaps the merging plugin's own prototype. The snapshot rather than a conditional copy, because a hop result backed by accessors can read clean once and poisoned the next time. Spread it as before (`{ ...next(...), params: mine }`); do not rely on its identity. `buildPath` and `start` are unaffected — they return a string and a `Promise<State>`, and the wrapper is passed only where the seam asks for it.
...
⚑ **The freeze is not limited to those three, nor to core (#1960 / #1964).** Every `RouterError` core throws is frozen at the throw, and so are the ones raised by the three doors outside it — `usePlugin` after start, an unmatched popstate, a Navigation API rollback. The primitive is published as `freezeThrownError` on `@real-router/core/utils`, because an inline `Object.freeze` per site puts the reason in as many places as there are sites. ⚠ It applies to an error the package CONSTRUCTED; a re-thrown foreign error stays untouched.
...
| **`update` filter**                                           | Emits only when the patch has a structural field (`forwardTo` / `defaultParams` / `defaultSearch` / `encodeParams` / `decodeParams`). Guard-only and empty patches are silent — and so is a patch whose only structural field carries a type-invalid FALSY value, because `update` adopts a field on the same terms registration does and therefore changed nothing (#1797).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
⚠ **`areStatesEqual(…, false)` and `isActiveRoute(…, false)` answer different questions.** This one is state IDENTITY — its `false` polarity compares the WHOLE `params` bag, which is what `canSkipPopstateHistoryWrite` needs and what #515 / #478 pin. `isActiveRoute` asks about the LOCATION; borrowing this polarity for that question is #1978. Its own reach is derived, not restated — INVARIANTS isActiveRoute #10 names the test.
...
- **Cached ⟹ frozen (#1805).** The three cached factories — `getNavigator`, `getRoutesApi`, `getPluginApi` — hand back a frozen object, because one instance is shared by every consumer of a router and a single member assignment rewires it for all of them. The two UNCACHED ones (`getLifecycleApi`, `getDependenciesApi`) are not frozen and need not be: a write to a per-call object reaches nobody. The classification is DERIVED — `tests/functional/factory-surface-freeze-authority-1805.test.ts` asks every factory both questions, so a sixth one has to answer them. ⚠ A test that stubbed a member of these surfaces belongs on `getInternals(router)` instead: every member delegates there, so a spy one layer down still intercepts
