...
- **Freeze once, at the origin** — there is NO traversal (this line claimed "consolidated into one recursive traversal" long after the traversal was gone, #1599). `freezeStateShell` — renamed from `freezeStateInPlace`, which promised a depth it never delivered — freezes the state object's own level; the depth comes from each producer freezing its own output exactly once: `params` in `canonicalize` (fast path) / `mergeWithDefault` (slow), `search` from the `EMPTY_SEARCH` singleton or `admittedSearch`'s drop branch, `transition` + nested in `buildTransitionMeta`. Measured reason not to centralise: re-freezing an already-frozen object costs ~8 ns, so a walk would pay per node for work already done. Owned and pinned per producer — INVARIANTS "State immutability (who freezes what)"
