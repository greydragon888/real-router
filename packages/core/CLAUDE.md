...
Cancellation: Pass `{ signal }` via `NavigationOptions` for external `AbortController` cancellation. `router.stop()`, `router.dispose()`, and concurrent navigation abort the internal controller automatically. **Every cancellation path returns the FSM to `READY` and emits `TRANSITION_CANCEL`** — including an external `opts.signal` abort during an async guard, which routes through `cancelNavigation` symmetric with `stop`/`dispose`/supersede (`onExternalAbort`, #1030). Before #1030 the external-signal path only aborted the controller and left the FSM stuck in `TRANSITION_STARTED`/`LEAVE_APPROVED` — `isTransitioning()` stayed true (route-CRUD silently blocked) and `isLeaveApproved()` was falsely true until the next navigation. 
