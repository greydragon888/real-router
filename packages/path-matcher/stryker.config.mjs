/**
 * Stryker mutation testing configuration for path-matcher
 *
...
export default {
  ...
  // Mutate path-matcher source files
  mutate: [
    "src/**/*.ts",
  ...
  // Mutation score thresholds (foundation primitive - moderate thresholds).
  // Note: stryker runs only the unit `.test.ts` suite (tests/unit/); the
  // property/stress suites are excluded by convention. Deep matcher invariants
  // live there, so measure the real score on first run and tune accordingly.
  thresholds: {
    high: 90,
    low: 70,
    break: 60,
