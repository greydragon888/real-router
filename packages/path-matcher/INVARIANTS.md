...
| 9   | Constraint satisfaction              | Params extracted from a matched constrained route always satisfy the constraint regex. A route declared as `/:id<\d+>` only matches when `id` is numeric. The constraint is checked on the **decoded** value (the value the consumer receives), not the raw URL segment (#857). |
...
| 23  | Constraint filtering at match        | `match()` returns `undefined` when a captured (decoded) value violates the route's constraint regex (route filtered out); a satisfying value is admitted and captured. Complements #9 (satisfaction); validation runs on the decoded value (#857). |
