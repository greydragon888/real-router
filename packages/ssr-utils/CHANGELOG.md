...
- [#2346](https://github.com/greydragon888/real-router/pull/2346) [`70c8f18`](https://github.com/greydragon888/real-router/commit/70c8f18fad5c6663d0de118c18376d40c0c8a939) Thanks [@greydragon888](https://github.com/greydragon888)! - An enumerated leaf that forwards must land on a URL the manifest produced ([#2256](https://github.com/greydragon888/real-router/issues/2256))

  Since [#2250](https://github.com/greydragon888/real-router/issues/2250) an href RESOLVES the `forwardTo` chain, so `<Link routeName="old">` renders the TARGET's URL. `getStaticPaths` prints the LITERAL form — the one that answers about the route it was NAMED (INVARIANTS [#8](https://github.com/greydragon888/real-router/issues/8)) — so an entry supplied for a forwarding source wrote a file at the SOURCE's URL while the href that reaches users named one the manifest never produced. On a static host that is a silent 404, and nothing in the build failed.
...
- [#2147](https://github.com/greydragon888/real-router/pull/2147) [`e30f07e`](https://github.com/greydragon888/real-router/commit/e30f07e1859aa2d39efd1613b531c42b5eea51f7) Thanks [@greydragon888](https://github.com/greydragon888)! - Stop `createRequestScope` from installing `dispose` under the string `"undefined"` on Node 22 ([#2117](https://github.com/greydragon888/real-router/issues/2117))
