...
// Track every mounted provider so leftover instances (their pagehide listener +
// router subscription) are torn down in afterEach — @vue/test-utils has no
// automatic per-test cleanup like @testing-library/react's `render`.
const mountedWrappers: { unmount: () => void }[] = [];

// Mount `<RouterProvider>` through a render-function component — the vue
// analogue of react's `render(<RouterProvider …/>)`. No `act` wrapper: with the
// synchronous rAF shim the capture / scrollTo spies fire synchronously inside
// `router.navigate`, so there is no vue reactivity to flush before asserting.
...
function unmountAll(): void {
  for (const wrapper of mountedWrappers) {
    try {
      ...
      // Already unmounted by the test itself — idempotent teardown.
    }
  }
...
describe("RouterProvider — scrollRestoration", () => {
  ...
  it("scrollRestoration provided — flips history.scrollRestoration to 'manual'", () => {
    mountRouterProvider(router, { scrollRestoration: { mode: "restore" } });

    expect(history.scrollRestoration).toBe("manual");
  });

  it("unmount restores history.scrollRestoration", () => {
    const wrapper = mountRouterProvider(router, {
  ...
  it("replacing the options ref with same fields — does NOT re-create the utility", async () => {
    ...
    // reference-based this would destroy + re-create, and instance #2's
    // prevScrollRestoration would capture "manual".
  ...
  it("mode 'native' — no flip, no scroll on navigation", async () => {
    ...
    mountRouterProvider(router, { scrollRestoration: { mode: "native" } });
  ...
  it("mode 'top' — scrolls to top on navigation", async () => {
    ...
    mountRouterProvider(router, { scrollRestoration: { mode: "top" } });
  ...
  it("scrolls to the location.hash anchor when no URL plugin is installed", async () => {
    ...
    mountRouterProvider(plain, { scrollRestoration: { mode: "restore" } });
  ...
  it("restores the saved position on a back navigation", async () => {
    ...
    mountRouterProvider(navRouter, { scrollRestoration: { mode: "restore" } });
  ...
  // ⚠ Inverted by #1923, not deleted. These pinned a DEGRADATION: the key went
  // through `JSON.stringify`, so a `BigInt` param threw, capture and restore
  // were skipped for that route and the provider warned once. The key is
  // `state.path` now — a string that cannot reach a serializer — so the failure
  // mode does not exist and these assert the working behaviour instead.
  ...
  it("defaults to mode 'restore' when the mode field is omitted", async () => {
    ...
    mountRouterProvider(router, { scrollRestoration: {} });
  ...
  // never reaches the URL and both navigations describe one location.
  ...
  it("reload of a route with a BigInt param runs the restore arm", async () => {
    ...
    // ⚠ #1923: what this measures is that the arm RUNS. Under the old key a
    // BigInt made `keyOf` throw, the key was null and the arm restored 0
    // unconditionally; the key is the location now, so the arm consults the
    // store like any other route. The stored VALUE is not pinned here — this
    // route shares the `/about` bucket with every other test that visited it,
    // and pinning a number would pin the file's execution order.
  ...
  it("pagehide captures a route with a BigInt param like any other", async () => {
    ...
    // onPageHide keys by path, so an unserializable param no longer skips
    // the capture.
...
describe("RouterProvider — scrollRestoration (destroyed guard)", () => {
  ...
  it("restorePos retry bails when the provider is destroyed mid-budget", async () => {
    ...
    const wrapper = mountRouterProvider(navRouter, {
      scrollRestoration: {
        mode: "restore",
        scrollContainer: () => container,
      },
    ...
    wrapper.unmount(); // destroy mid-budget
