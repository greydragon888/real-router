...
- **Angular** — uses `injectDeferred()` (signal-based, asymmetric — see [packages/angular/CLAUDE.md](../angular/CLAUDE.md)) when paired with `defer()`. The streaming example uses native `@defer` blocks + incremental hydration instead, since Angular's chunk loading and per-block hydration cover the same ground without needing the wire-format bridge.
