...
| `injectIsActiveRoute(name, params?, search?, options?)`          | Function       | Subscribe to active route state                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `RealLinkActive`                                                 | Directive      | Active route class directive for any element. **Note**: does NOT accept a `hash` or `routeSearch` input — hash-aware active state and the query (search) channel are `RealLink`-only                                                                                                                                                                                                                             |
...
`injectRoute<P>()` and `RouteSignals<P>` accept an optional generic so `routeState().route.params` is typed without `as` casts. Runtime is unchanged — the cast happens once inside the function. The generic types the **path** channel only — the query channel (`route.search`) keeps its own `SearchParams` shape from `@real-router/core` regardless of `P` (RFC-4 M2, #1548).

```typescript
type RouteParams = { slug: string } & Params;

const route = injectRoute<RouteParams>();
const slug = route.routeState().route.params.slug; // typed as string
const q = route.routeState().route.search.q; // query channel — SearchParamValue | undefined, not generic-typed
```
...
- **Client pass** — the same `provideAppInitializer` callback reads `TransferState`, finds the seeded JSON, and calls `hydrateRouter(router, ssrJson)` instead of `router.start(path)`. `hydrateRouter` deposits the parsed state into a one-shot scratchpad `@real-router/ssr-utils` owns, and `ssr-data-plugin`'s start interceptor reads it through `getHydrationState(router)` — reusing the server-resolved `state.context.data` without re-invoking the loader on first paint.
...
| [`examples/web/angular/ssr-examples/ssr-streaming/`](../../examples/web/angular/ssr-examples/ssr-streaming) | Streaming SSR — `@defer (on viewport)` for Reviews, `@defer (on hover)` for RelatedItems, per-block incremental hydration                                                        | 28        |
