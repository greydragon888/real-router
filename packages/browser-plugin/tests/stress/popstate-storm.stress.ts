...
describe("B1 — Popstate Storm", () => {
  ...
  it("1.1 — 50 rapid-fire sync popstate events: final state = last event, intermediates skipped", async () => {
  ...
  it("1.2 — 50 popstate events with 100ms async guards: only first and last transition", async () => {
    const result = createStressRouter();
  ...
  it("1.3 — 200 alternating popstate events: final state correct, not stuck", async () => {
    const result = createStressRouter();
  ...
  it("1.6 — 50 null-state events, allowNotFound=false: navigateToDefault called, replaceState used", async () => {
    ...
    const { browser, dispatchPopstate } = result;
