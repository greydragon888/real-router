...
- **F5 / cold-load: lazy read in `onTransitionSuccess`**. On the first transition (`!fromState`), `getDecodedHash(browser)` reads the current URL fragment. browser-plugin has no Navigation-API `getActivationType()` analogue, but doesn't need one — `popstate` doesn't fire for the initial document load anyway. By the time `onTransitionSuccess` runs, `location.hash` already reflects the destination URL.
