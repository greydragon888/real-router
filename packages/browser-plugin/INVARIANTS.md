...
| 5   | The param survives `safelyEncodePath` too           | `matchPath(safelyEncodePath(buildPath(p))).id === p.id`. ⚠ Invariant 4 above names a pipeline that OMITS `safelyEncodePath`, and stayed true while that function corrupted every reserved escape (#1920) — the reload path runs it and the invariant did not. This one puts it in the chain.                                                                                                                                                                                                                                          |
