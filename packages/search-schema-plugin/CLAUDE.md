...
> **Caveat — the recommended order covers ONE direction, and never `state.path`.** (#1231, #1563)
>
> - **This plugin reads one channel per direction.** The interceptor picks the channel from its `routeSearch` argument: `state.search` on the URL→State direction (`matchPath` / `start()`), the **params bag** on the State→URL direction (`navigate`, where a v1 single-bag caller's query still rides there). Since #1563 `persistent-params-plugin` injects into `search` on both directions, so in the recommended order the schema sees the persisted values on the URL→State direction and **misses them on a plain `navigate`** (before #1563 it was the other way round). Closing both directions means validating the query wherever it lives — a change here (#1564), not a registration order.
