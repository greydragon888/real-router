...
- **`validateRoutePath` rejects static text fused to a constraint (#1150)** -- `hasFusedConstraintSuffix` (run after the balance check, so every `>` is a constraint closer) flags a `>` not followed by `/`, `?`, or end-of-input — `/:year<\d+>-archive`, `/:id<\d+>.html`. build re-extracts the name greedily and fuses the suffix (name `year-archive`) while meta ends at `<` (name `year`), compiling to a silent dead route. The mirror of the fused-marker (#1050) rejection on the OTHER side of the param; path-matcher backstops at `registerTree`. A char-scan (constraints may contain `/`)
...
- **Gate ↔ backstop parity is property-locked (#1320)** -- every reject check above has a path-matcher `registerTree` backstop twin. For the scans the two layers implement SEPARATELY (gate whole-path char-scan vs backstop per-segment regex/embedded fork — fused-marker #1050, non-ASCII #1154, optional-splat #1149, unconstrained-opt→splat #1264, dup-param #1151), `tests/property/gate-backstop-parity.properties.ts` proves they agree (both reject a malformed shape / both accept a valid one) across the generated input space, so a future edit that makes one layer stricter than the other fails the property. The whole-path scans the two layers can literally share (`isConstraintBalanced`, `hasConstraintInStaticSegment`, `hasFusedConstraintSuffix`) are instead single-sourced in path-matcher's `constraint-grammar.ts` (#1320 Tier 1). Known excluded divergence: a `*` fused to the END of a param (`/:y*`) — gate rejects (name-less marker), backstop accepts (param name `y*`), gate-masked in production. See INVARIANTS.md "Gate ↔ Backstop Parity"
