...
| 2   | Tree immutability               | Every node in the tree — plus its `children` Map, `paramTypeMap`, `nonAbsoluteChildren` array, nested `paramMeta` object, and its `urlParams`/`queryParams`/`spatParams` arrays — passes `Object.isFrozen` (#747). Verified across all 6 fixture trees (PARAM, QUERY, MIXED, DEEP, ABSOLUTE, MIXED_ABSOLUTE). **Exception:** `paramMeta.constraintPatterns` is a `Map`, which `Object.freeze` cannot make read-only; it is protected at the type level via `ReadonlyMap` and intentionally not runtime-frozen.                                                               |
