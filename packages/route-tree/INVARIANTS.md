...
| #   | Invariant                       | Description                                                                                                                                                                                                                                                                                                                            |
| --- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Tree immutability               | Every node in the tree — plus its `children` Map, `paramTypeMap`, `nonAbsoluteChildren` array, nested `paramMeta` object, and its `urlParams`/`queryParams`/`spatParams` arrays — passes `Object.isFrozen` (#747). Verified across all 6 fixture trees (PARAM, QUERY, MIXED, DEEP, ABSOLUTE, MIXED_ABSOLUTE). **Exception:** `paramMeta.constraintPatterns` is a `Map`, which `Object.freeze` cannot make read-only; it is protected at the type level via `ReadonlyMap` and intentionally not runtime-frozen.                                                               |
| 2   | Non-absolute children filtering | For every node, `nonAbsoluteChildren` contains exactly those children from the `children` Map where `absolute === false`, preserving definition order. Absolute children appear in `children` but not in `nonAbsoluteChildren`.                                                                                                        |
...
| `tests/property/tree.properties.ts`        | N1–N4, CC1–CC3, ND1 | createRouteTree normalization + caches + nodeToDef |
