...
| `channels/` | The rule that `params` is the path channel and `search` the query one: the always-on guard detecting a query-declared key in the path bag, the registration-time check on a route's own defaults, and the gate dropping what the active mode will not print. It also owns the one key that cannot be carried as data — an own `__proto__`, the sole accessor on `Object.prototype` — answered by the SOURCE of the bag: refused where someone wrote it, dropped where a URL supplied it | Declared query names arrive as DATA — a `readonly string[]` or an accessor, never a matcher. A subsystem rather than a namespace method because the rule has no owning module: it runs from the facade, from `internals`, from two namespaces and from the pipeline |
...
3. Attach and freeze the `TransitionMeta`, freeze the state. Its three flags come off the navigation's PLAN, where the entry point put them before anything was announced — so this step, like the rest of the function, runs no application code and needs no position relative to the verdict.
