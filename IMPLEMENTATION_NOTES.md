...
### Adapter benches PoC — react under jsdom, pure tinybench + vite prebuild (2026-07-16)

The research slot the doctrine reserved for adapter benches is now proven end-to-end by `benchmarks/adapter-bench-poc/` + `.github/workflows/codspeed-adapters.yml` (dispatch-only, deliberately a **separate workflow** so adapter failures never block the core-gate signal). Shape: vite prebuilds the app (react-dom production + adapter + memory-plugin, resolved from live `src` via the `@real-router/internal-source` condition) into ONE self-contained `app.mjs` **outside** the CodSpeed action; the measured step is a single direct `node` that installs jsdom globals and runs tinybench + the tinybench plugin — the exact process shape whose V8-flag injection the core gate already proved (the vitest-worker question never arises on this path). Three benches (`react/navigate-param-swap`, `navigate-route-swap`, `back-forward` via memory-plugin), each iteration wrapped in **`flushSync`** so the synchronous React commit lands inside the measure window — a pre-measure self-check asserts the DOM actually mutates (otherwise the bench silently measures router-only work while React batches outside). Empirics: a jsdom React commit costs **~3.5–4.2 ms/op simulated** (~×20 its 200 µs local wall-clock — local numbers say nothing about callgrind cost, as the calibration rule predicted), so K=2, masses 9.6–18.4 ms, bench step ~15–20 s, job ~1 min; first same-sha pair **3/3 unchanged, +0.017 %**. Two gotchas for the next five adapters: never shim `globalThis.Event`/`CustomEvent` from jsdom (tinybench's `Bench` extends Node's native `EventTarget`, which rejects foreign Event instances), and invoke vite in workflows via a workspace *script* (`prebuild:adapter-poc`) so knip resolves the binary against `benchmarks/package.json`.
