...
**Solution.** `OSV_SCANNER_FLOOR="2.6.0"` in `scripts/check-deps-audit.sh`. The script reads `osv-scanner --version` and exits `3`, "the audit did NOT run", when the version is below the floor or cannot be read. The comparison is numeric per field. A missing scanner still skips, as before.

**Measured (2026-09-15).** This tree's lockfile, as it is and with a pnpm 12 environment document prepended, scanned without the repository config. On the single-document file both versions read 1762 packages and report three advisories. On the two-document file 2.3.8 reads 1 package, the environment document, and prints `No issues found` with exit 0; 2.6.0 reads 1763, the extra one being pnpm, and reports the same three. With `scripts/osv-scanner.toml`, which ignores those three, every run exits 0 — so the exit code cannot tell the two scanners apart, and only the version can. The reproduction from google/osv-scalibr#2351 agrees: 2.6.0 finds `minimist` and its two advisories, and 2.3.8 does not see the package.

⚠ **`--version` does not say whether the fix is in.** 2.6.0 prints `osv-scalibr version: 0.5.2`, the release before the fix, because it builds on an untagged scalibr commit, and its release notes do not mention the fix. The `go.mod` of the release is what answers the question.

**Why a floor, not an exact pin.** The failure is below the floor. Homebrew installs the current release and `brew upgrade` moves it, so an exact pin would fail every push after an upgrade.

**Validated mutationally.** The test stub answers `--version` with the floor itself, so every existing test runs at the boundary. Two new tests cover the refusal and the numeric comparison, since `2.16.0` and `12.0.0` sort below `2.6.0` as strings. Six of seven mutations are killed: dropping the comparison, a string sort, a patch-first sort, an unreadable version exiting 0, exit 2 instead of 3, and moving the check after the scan. Deleting the explicit unreadable-version branch survives because an empty version also sorts below the floor, and that branch exists only for its message.
...
**Solution.** A `react` group, listed first in the npm entry's `groups`, holding `react`, `react-dom`, `@types/react` and `@types/react-dom`. Dependabot puts a dependency in the first group that matches, so the four arrive in one PR instead of `dev-dependencies` or a lone update. Patch updates stay ignored for `react` and `react-dom` (the patch-float set), so for those two the group carries minors and majors.
...
**Problem.** A Dependabot PR ran the same reporting surface as a feature PR, and its merge re-ran CodSpeed on the single self-hosted slot. None of those reports could say anything about the bump: every `dependencies` entry of the 23 published manifests is `workspace:`, and third-party packages appear only as dev or peer dependencies, so a bump changes nothing a consumer installs and no line of shipped source. One of them was also leaking. `sonar-trusted.yml` recognised Dependabot by the CI run's actor, and any push to a Dependabot branch that Dependabot did not make changes the actor: `dependabot-dedupe.yml` pushes its dedupe commit with a maintainer's token, and `resolve:dependabot` rebases as the maintainer. #2313 and #2317 (dedupe) and #2287 (rebase) each carried a real `Quality gate: success`. CodSpeed's push run after an npm Dependabot merge took 3–18 minutes from trigger to finish; of the 19 such merges since 2026-07-10, the 9 runs that finished all reported "Performance Gate Passed", 5 were cancelled, and 5 merges have no run.
...
⚠ **The predicates read who opened the PR, not what it changes, and that holds only because code never lands on a Dependabot branch (owner rule).** Besides Dependabot's own commits, the branch only ever receives lockfile work — the dedupe fixer's commit, or `resolve:dependabot`'s rebase that re-resolves the lockfile. A code commit on that branch would get the same skips, the changeset gate included. (`sonar-trusted.yml` reads the branch rather than the author, with the same effect.)

## The two benchmark suites are one workflow, because CodSpeed only aggregates within one (#1691)

**Problem.** `codspeed.yml` (core) and `codspeed-adapters.yml` (six adapters) were deliberately separate files, so that "an adapter-suite failure never blocks the core gate signal (and vice versa)". The cost of that split was invisible until a PR reported **−19.71 %** and the number would not reproduce: four local A/B configurations — JIT on, JIT off single-file, JIT off with all six suites in one process, each swapping `packages/core/src` for the base version and alternating processes — put eleven of the twelve flagged `navigate/*` arms between **+0.6 % and +3.4 %**, against **11–28 %** in CI. Only `navigate/external-signal` reproduced everywhere (+49 % with JIT), and it was a real cost of #1684.

Three properties of the CI report, all measured:

1. **The aggregate oscillated ±20 % on trees whose runtime code was byte-identical.** Three consecutive commits of the #1689 branch changed only `.claude/commands/*.md` and the prose of a changeset, and the sidebar read `−19.71 % → +19.72 % → −20.23 %` — the middle pair cancelling to 0.01 pp.
2. **Base and head were never measured in the same run.** Every core benchmark's base value carried an asterisk and the tooltip _"Skipped result on the base run, the displayed result ran 1 day ago"_.
3. **Per-benchmark the instrument was fine.** Across the three same-tree core runs all 66 benchmarks agreed to within **3.5 %**, most of them exactly. So the ±20 % was never measurement noise — the 10 % threshold is meaningful.
...
**Two things that moved rather than vanished.**

- **Concurrency stays workflow-level and now covers both suites**, which is the correct scope: they belong to one run and must be superseded together, or half a superseded run would upload into the new run's report. (The old comment's fear — "so the two suites never cancel each other" — was about two runs racing; there is one run now.)
- **The path-ignore twin is gone.** Two byte-identical lists that had to be synchronised by hand ("GitHub Actions has no YAML anchors") collapsed into one, removing a standing drift hazard.

⚠ **Serialisation is still the runner's, not the workflow's.** The two jobs queue because the self-hosted runner has a single job slot — exactly as they did as two workflows. Add a second slot and they could run concurrently and perturb each other's counts; the remedy then is `needs: core` **plus** `if: always() && …` on `adapters`, which restores ordering without giving up independence. A bare `needs` would trade the property this whole note exists to preserve.

⚑ **What this does NOT fix, and must not be claimed to.** The 11–28 % CI-vs-local gap is untouched by it: comparing the two CORE runs directly to each other gives the same numbers as the fallback did (`navigate/sync-baseline` −27.4 % against −28.3 %). The open half of #1691 is the instrument — CPU simulation _"does not include system calls in the measurement"_, and these arms report ~70 excluded syscalls each, so its percentage is a share of user-space work rather than of what a user experiences. Until those arms are measured under `walltime`, do not quote a simulation delta as a user-visible slowdown.
...
**Why not jscpd too:** the hard 2% duplication threshold deliberately stays in the hook; CI keeps the **informational** jscpd SARIF job (`-t 100`, exit 0) — a conscious choice from the prior audit (see "jscpd …SARIF" below), so duplication is visible as PR annotations without gating merges. ⚠ It was not visible for the channel's first year — the paths did not resolve and every upload produced zero alerts (#2154, below).
...
**Sequel — the same package, one major later (`GHSA-5p4m-2wfm-xmqj`, `js-yaml@4.3.0`, 2026-08-07):** the line the fix above moved the tree onto got its own High advisory — `resolveYamlOmap()` does an `indexOf` scan per element, so `!!omap` resolution is O(n²) and `yaml.load()` on a modest document burns CPU (`!!omap` is in the **default** schema, so plain `load(input)` is affected). It is the 5.x `CVE-2026-59870` never backported, finally patched on the legacy lines (`4.3.1` / `3.15.1`). This one needed none of the ancestor-walking the original did: exactly one version was in the tree, its parents are all dev/build tooling parsing **our own** yaml (`@changesets/parse`, `cosmiconfig`, `read-yaml-file`, `electron-builder`/`app-builder-lib`), and the fix is a same-line patch — so a plain floor `js-yaml: '>=4.3.1 <5'` in `pnpm-workspace.yaml#overrides` clears it with no allowlist. The `<5` ceiling is deliberate and matches the `<3.0.0` reasoning above: 5.x is a major none of those consumers declare, and an open floor would let `pnpm dedupe` (pre-commit runs it) jump the whole tree across it. Lesson: an override that fixes today's advisory is not a permanent answer for that package — the intermediary pin bought a fixed line, not immunity, and the next advisory on the new line is an ordinary floor bump.

**Third shape — the override WAS the vulnerability (`GHSA-px8p-9vwx-vf98`, `fflate@0.8.2`, 2026-09-04):** `unzipSync` loops forever on a malformed ZIP64 archive. Unlike the two above, no ancestor needed walking and no floor needed inventing: both parents — `@arethetypeswrong/core` (via its CLI and `tsdown`) and `@vitest/ui` — already declare `^0.8.3`, so the tree would have resolved to the fix on its own. What held it back was an entry ALREADY in `pnpm-workspace.yaml#overrides`: a bare `fflate: 0.8.2`, undated and uncommented, sitting among the dedupe pins. An exact override added to collapse two copies keeps collapsing them onto the version it names, and when an advisory lands on that version the override becomes the thing enforcing it. Deleting the entry is not the fix either — measured, the tree then carries 0.8.2 **and** 0.8.3 and `lint:dedupe` refuses it; the pin was load-bearing for deduplication. The answer is the same shape as the entries above, `fflate: '>=0.8.3 <0.9'`, which collapses to one version AND clears the scan. **Lesson: an uncommented exact override is a latent advisory floor.** When `lint:audit` names a package that already has an entry, read the entry before adding a second one — and every override in that block earns a comment saying which of the two jobs it does, pinning for dedupe or flooring for security.

**Fourth shape — the advisory was WITHDRAWN, and the removal condition could not see it (`RUSTSEC-2024-0411` … `-0420`, 2026-09-10):** ten gtk-rs unmaintained-crate entries (gdk, gtk, atk, their `-sys` crates, gtk3-macros) carried an explicit removal condition — drop them "when Tauri 2.x ships a new GTK stack". Tauri has not, and all ten crates are still in both `examples/desktop/tauri/*/src-tauri/Cargo.lock`; read literally, the condition would have held the entries for as long as Tauri keeps GTK. What actually happened is upstream: RustSec **withdrew** all ten on 2026-08-14, confirmed per id at `api.osv.dev` (`withdrawn` is set) rather than inferred from the scanner. **Lesson: a removal condition written against the dependency GRAPH is blind to the advisory being retracted, and an allowlist entry whose advisory no longer exists is indistinguishable from one still doing work.** The tripwire that catches it already exists and is easy to miss — osv-scanner prints `has unused ignores:` and lists them, and does **not** fail the run. Read that line. Its positive control is free: the other Tauri-chain entries were absent from the same list, which is what proved the scanner still read `Cargo.lock` and the ten were genuinely unmatched rather than unscanned.
...
`codspeed.yml` (then also its twin `codspeed-adapters.yml`, folded into it
as a second job by #1691), plus
...
- **`svelte` coverage:** Migrated (#2303) — see the subsection below.
...
**Why it is safe, measured against a control on each gate:** `svelte-check` reports the same 725 files / 0 errors / 36 warnings with the condition and without; `lint` is clean; the suite is 35 files / 389 tests green; `publint` emits the same seven complaints either way, all of them "`./dist/...` does not exist" in an unbuilt worktree and none of them about the new entry. Nothing in `packages/` imports the adapter, and the example apps' `tsconfig.json` does not extend the root, so it never activates `customConditions` — the two resolvers that do are monorepo `tsc` and `workspaceSourceAliases()`, and neither had a consumer to break.

**What it costs, and it is a silent cost rather than a loud one.** Under plain `tsc` the condition now resolves the package to `src/index.ts`, whose component re-exports are `.svelte` specifiers. Those do not fail: `svelte` ships an ambient `declare module '*.svelte'` in `svelte/types`, so each component types as the generic `LegacyComponentType` instead of the exact props `svelte-package` writes into `dist/*.svelte.d.ts`. Everything declared in `.ts` — `useRouter`, `createLinkAction`, every exported type — stays exact (`traceResolution` plus an assign-to-`number` probe confirmed both halves). A consumer using `svelte-check` rather than bare `tsc` is unaffected in either direction, because `svelte2tsx` generates real types from the same source. ⚠ So the first package to import this adapter under plain `tsc` gets components typed loosely and no diagnostic saying so — add `svelte-check` to that package's gate, or point its own resolution at `dist`.
...
- `Router` constructor registers the entry point on the `RouterInternals` WeakMap (alongside `start`, also there) — not on the `Router` class facade. `getPluginApi.navigateToState` calls through `ctx.navigateToState`, which gets the same unhandled-rejection suppression `Router.navigate` does, so plugin call-sites can fire-and-forget the returned promise (popstate handlers do). Since #1588 that suppression lives in `NavigationNamespace.#settle` (one checkpoint per public method, skipping its pre-suppressed singletons by identity) and the facade closure only owes callers the `Promise` shape; the former `lastSyncResolved`/`lastSyncRejected` side channel the facade used to read is gone.
...
- **`build.lib` mode does NOT replace `process.env.NODE_ENV`** (2026-07-21) — a library must adapt to its host, so Vite deliberately leaves the check un-replaced. Combined with the bench process (`node … codspeed.mts`) running with `NODE_ENV` **unset**, every framework fell back to its **DEVELOPMENT runtime**: React profiler timers (`startUpdateTimerByLane`) + act-warnings, Vue dev warnings + devtools hooks, Angular `ngDevMode` assertions. Two harms: (1) the benches measured **dev overhead, not the shipped hot path** (react same-session A/B: dev **3.6–12× slower** — `navigate-param-swap` 867 µs → 72 µs); (2) those dev paths hammer `clock_gettime`/`write` **syscalls the callgrind/Simulation instrument cannot measure consistently** — the CodSpeed "N system calls, totalling …s" warning (react/route-swap carried `sysCount` 711). Fix: `base.mts` now `define`s `process.env.NODE_ENV: "production"` (+ `mode: "production"`), so rollup constant-folds and tree-shakes the dev branches out entirely (react bundle **1.9 MB → 851 kB**, `jsxDEV`/profiler/act-warning fns gone, self-check still commits). NODE_ENV covers react/preact/vue/solid/svelte runtimes; the non-NODE_ENV dev switches get per-config `define`s — **vue** `__VUE_PROD_DEVTOOLS__`/`__VUE_OPTIONS_API__`/`__VUE_PROD_HYDRATION_MISMATCH_DETAILS__`, **angular** `ngDevMode: false` (Angular gates dev on the `ngDevMode` global, not NODE_ENV). `minify` stays `false` — the constant-fold, not minification, is what strips dev code, and readable names keep the flamegraph legible.
...
> ⚠ Superseded on 2026-09-14: the push side is skipped too. See the 2026-09-14 entry "Dependabot PRs skip every metric".

**Problem.** Dependabot opens npm + github-actions PRs in weekly batches, and each one triggers BOTH CodSpeed workflows (core + adapters) — 2 slow jobs on the single self-hosted VPS that processes one job at a time (see the release-skip note above). The queue those jobs form delays the runs that actually matter: feature-PR gates and the master-push baseline. The CodSpeed verdict is NOT a required check and adds little on a dep bump — most bumps (eslint / prettier / types / testing-libs / UI-frameworks-in-examples / GitHub Actions) never touch the core/adapter hot paths the benches measure.

**Solution.** Extended both jobs' `if` — inside the existing same-repo pull_request guard — with `github.event.pull_request.user.login != 'dependabot[bot]'`. A Dependabot pull_request now fails the guard and is skipped; push / workflow_dispatch / collaborator PRs are untouched (they either carry no pull_request payload, so the guard's first clause is true, or a non-bot author).
...
**Retired 2026-09-06.** `@changesets/changelog-github@1.0.1` pulls `get-github-info@1.0.1`, which uses the platform `fetch`; `node-fetch@2` has no other parent in this tree and `pnpm why -r node-fetch` now returns nothing. With the library gone the pin has no subject, so `changesets.yml` reads `node-version-file: .nvmrc` like every other workflow. See "Changesets 2 → 3" below.
...
## Cross-router deck — print/PDF edition, one source two outputs (2026-07-19)

**Problem.** The interactive deck (`deck.html`) is a scrolling web page with hover tooltips, click-to-isolate, and in-page anchors — right for screen, wrong for a downloadable/printable artifact. A viewer wanted a static PDF to grab from the page: paginated (a cohort per page), no interactivity, laid out for print.

**Solution.** A SECOND shell built from the SAME data + render — one source, two outputs. The pure render primitives (`chart`/`barCard`/`boardHTML`/`fmt`/`smoothPath`/`NT`/`mk`/`stampHTML`/`fieldHTML`) were extracted from `deck-shell.html` into a shared `deck-render.js` that `build-deck` inlines (CSP: no `<script src>`) into BOTH shells at a `//__RENDER__` marker; `chart()` gained an `interactive` flag (default true) so the print edition drops the hover-target pass. `print-shell.html` composes a paginated **A4-landscape** document — cover (title + stamp + versions) → summary board split **one row-group per page** → each cohort on its own page with each row-group (Per navigation / Memory & startup / How it scales) breaking to a fresh page (`.grp-lab ~ .grp-lab{break-before:page}`); static charts (`interactive=false`), no isolate/tooltip/anchors, `break-inside:avoid` on cards. `build-deck` now builds `deck.html` AND `print.html` from the same config splice (`buildOne(shell,out)`). `render-pdf.mjs` loads `print.html` in Playwright's Chromium and `page.pdf({format:'A4',landscape:true,printBackground:true})` → `cross-router-report.pdf`. The deck gains an "Open the full report — PDF" button (`target=_blank`). CI renders the PDF after the deck rebuild (non-fatal `continue-on-error`) and publishes it to Pages next to `index.html` so the button resolves. `print.html` + the PDF are gitignored generated artifacts; the shells + `deck-render.js` + `render-pdf.mjs` are tracked source.

**Why.** One render source means the two editions can't visually drift — the yellow tie-diamonds, colours, and verdicts are identical (36 tie diamonds in both). Playwright was already in the stack (the bench drives Chromium), so a build-time **vector** PDF was near-free and higher-fidelity than any client-side html2pdf (which would bloat the CSP-strict deck and rasterize) — verified: fonts embedded, **0 raster images**, A4 landscape. The `deck-shell.html` inline render was deduped into `deck-render.js` and re-verified equivalent via jsdom (36 tie diamonds, 13 board rows, 5 cohorts, 504 hover targets + 81 isolate-legend items + anchors intact). A CSS gotcha cost several iterations: the print cohort-head badge looked top-aligned because the `h2` rule's `margin:0 0 5mm` (for the Summary headings) made `align-items:center` centre the badge against `text+5mm` — zeroed via `.cohort-head h2{margin:0}`, matching the deck. Infra-only (`benchmarks/` + workflow) → straight to master, no changeset.

## Core dts unbundled (two-phase tsdown) — dist augmentation declaration-site invariant (#1540, 2026-07-20)

**Problem.** Seven plugins augment `@real-router/core/types` (`declare module … { interface StateContext { navigation?: … } }`) to type `state.context.<namespace>` and their `NavigationOptions` extensions. TS merges such an augmentation **only when the module the specifier resolves to lexically declares the interface** — a re-export of ANY form is a silent no-op (#1519; re-verified empirically here: single-statement `export type { X } from`, renamed and non-renamed, all fail identically to the import+re-export pair). In the monorepo (internal-source → `src/types/index.ts`) the merge worked, but for external consumers resolving `dist`, the bundled dts hoisted `StateContext`/`NavigationOptions` into a shared hashed chunk (`rolldown-plugin-dts` does this even when the declarations live lexically in the entry SOURCE module — verified: moving them into `src/types/index.ts` alone did NOT keep them in the entry chunk), leaving `dist/esm/types.d.mts` a re-export barrel → every plugin's context/options typing degraded to `{}` for dist consumers. Regressed by the #1520 foundation fold (pre-fold the types entry happened to be self-contained); caught only by the twice-weekly examples workflow (`react/navigation-api` → TS2339), not by PR CI — the src path structurally cannot see it (#431 class).

**Solution.** Three coordinated pieces. (1) The augment-target interfaces (`StateContext`, `NavigationOptions`) moved from `src/types/base.ts` into `src/types/index.ts` — the entry itself is now the lexical declaration-site; `base`/`router`/`api`/`internal` import them back from `./index` (type-only cycle with the barrel — deliberate; `import-x/no-cycle` ignores type-only edges, so no disables needed). (2) Core's tsdown config became **two-phase**: a JS pair (`dts: false`) keeps the bundled output byte-identical (same chunks, same package shape/weight — size-limit unaffected), and a dts-only pair (`unbundle: true`, `dts: { emitDtsOnly: true }`) emits the declaration tree preserving modules, so `dist/*/types.d.*` IS `src/types/index.ts` with the lexical declarations, and `types/base.d.mts` imports them from the entry. Both phases run with `clean: false` (they share outDirs and tsdown may run configs concurrently — one phase's clean would race the other's output); the `bundle` script does `rm -rf dist` upfront instead. The dts tree's `.mjs` specifiers (e.g. `./types/base.mjs`) have no JS twin on disk — TS resolves them to `.d.mts` and runtime never loads dts, attw confirms all four resolution modes clean. (3) A post-bundle tripwire `packages/core/scripts/check-dts-augment-targets.mjs` (wired into the `bundle` script) fails the build when a `types` entry dts loses a lexical `interface StateContext`/`NavigationOptions` (re-export barrel again) or when any other dist dts declares one (a duplicate splits the type into two symbols — consumers would silently reference the un-augmented twin). Mutation-validated: healthy dist passes, deleting the declaration from the entry fails the check.

**Why.** The two prior bundler-level attempts died on structural grounds (`codeSplitting: false` rejects multi-entry; plain `unbundle` was tried before the declarations moved, so the entry re-exported from `types/base.d.mts` and still didn't merge) — the fix requires BOTH the source-level move AND unbundled dts; each alone fails. Full unbundle (JS too) worked but cost +713 B brotli on the root entry (lost cross-module minification) and 247 published files; the two-phase split keeps the JS identical (verified: same chunk hashes) at zero size cost. Reproduction went red→green: `react/navigation-api` example TS2339 ×2 on baseline, clean after the fix, with core tests (100% coverage), lint, attw/publint and all seven plugins' + react/sources/validation type-checks green. Note: `pnpm size` shows core ESM 20.63 kB vs a 20 kB limit — pre-existing on master (the CI size gate is informational, `|| true`) and untouched by this change (JS byte-identical).

**Root-family follow-up.** The sibling class — `declare module "@real-router/core"` (root, not `/types`) augmenting the `Route` / `RouteConfigUpdate` **interfaces** (lifecycle `onEnter`/`onStay`/`onLeave`/`onNavigate`, preload `preload`, search-schema `searchSchema`) — was probed on the fixed dist and **merges without an entry move**: the main entry re-exports them from the real `./types/router` module (unbundled) AND carries value bindings (`Router` class / `createRouter`), and interface-augmentation propagates through a re-export when the module has a value binding (#1519) — unlike the pure-type `/types` entry, which needed the lexical move. But a regression here would be **silent**: `Route` carries `[key: string]: unknown`, so a broken merge degrades to `unknown` (not a missing property) — neither ordinary example code nor #1539's PR type-check would catch it. Guarded by `examples/web/react/combined/src/root-augment.guard.ts` — a discriminating type-level probe (resolves dist + all three plugins, `IsUnknown`-based assert over all nine `Route`/`RouteConfigUpdate` fields) that turns the silent degradation into a hard `tsc` error; mutation-validated (healthy passes, an un-augmented field fails). It is the **root-family twin** of the `check-dts-augment-targets.mjs` tripwire: that covers the types-family **structurally** (entry must lexically declare), the guard covers the root-family **semantically** (root merges via re-export, so "entry declares" does not apply). Runs in the Examples workflow (schedule) today; on PR once #1539 lands the examples PR gate.

## SSR/SSG helpers extracted from core into `@real-router/ssr-utils` (#1543, 2026-07-20)
...
**Gotcha: `pnpm doctor` reports Node 26, and that is not your Node.** The newly available `pnpm doctor` (absent on 11.9 — `ERR_PNPM_RECURSIVE_EXEC_FIRST_FAIL: Command "doctor" not found`) prints `✓ Versions: pnpm 11.18.0, Node.js 26.0.0`. That is the runtime embedded in the `@pnpm/exe` standalone binary, not the one scripts execute — a real hazard to misread here, because Node ≥ 26 is exactly what shadows jsdom's `sessionStorage` under vitest (see "Adapter scroll tests mock `sessionStorage`"). Confirmed the separation holds: `pnpm exec node --version` → `v24.18.1`, and `globalThis.sessionStorage` inside a pnpm-spawned script is `undefined`. Read `pnpm doctor`'s version line as diagnostics about pnpm itself, never as the project's Node.

**Not taken: pnpm 12.** `12.0.0-beta.0` is already on the `next-12` dist-tag. Staying on 11.x until it ships stable and the same due diligence can be repeated.
...
**Verified on a fixture, not by reading.** A throwaway `packages/` tree with a 404 name, a deliberately-ahead `@real-router/core@99.0.0`, and a `private: true` package: the three are classified `never-published` / `behind` / skipped respectively, the `::error::` fires, and all four `GITHUB_OUTPUT` keys come out in the format the workflow consumes. The post-mortem's bash block was then run against the same fixture to confirm `MISSING` keeps its historical shape (leading space, `name@version`).
...
**Solution — sequencing, and nothing else.** The two passes stayed in ONE `tsdown.config.mts`, which now exports the dts pass or the JS pass depending on `RR_DTS_PASS`; the `bundle` script runs them in order and deletes pass 1's stray JS in between:

```
RR_DTS_PASS=1 tsdown     # declarations, unbundled (its stray CJS JS deleted in `build:done`)
  && tsdown              # the real bundled JS, then publint/attw
  && node scripts/check-dts-augment-targets.mjs
```

Everything else lives in `tsdown.config.mts`, not in the script. Pass 1 discards its own stray JS from a `build:done` hook (`dropStrayJs`, ~15 lines over `node:fs`) scoped to **that config's own `outDir`** — so the concurrent ESM and CJS builds of pass 1 never touch each other's output, and the ESM one is simply a no-op. Matching on extension is safe by construction rather than heuristic: pass 1 emits **no ESM JS at all**, so the only `.js` under `dist` at that moment is the CJS graph being discarded, and declarations are `.d.ts` / `.d.mts`. Emptied directories go with it. Hook timing is load-bearing and was read off the source, not assumed: `buildSingle` does `await build(configs)` over BOTH rolldown builds of a CJS pass (the ordinary one and the extra `cjsDts` one) before calling `postBuild()`, which fires `build:done` — so nothing is still being written when the hook runs. Pass 1 also carries `clean: true` and owns the cleanup for the whole build, which is why there is no `rm -rf dist` (checked by planting a stale `dist/cjs/STALE.d.ts` — a name no extension rule would match — and watching it disappear); pass 2 runs `clean: false` so it cannot wipe the declarations. `publint`/`attw` are off in pass 1 (nothing to validate yet) and inherited from the base in pass 2, which runs last against the finished package.

**The hook is mutation-validated**, since "no stray files" is exactly what a silently-not-running hook also looks like: deleting it brings back 89 CJS JS files / 334 total, restoring it returns 6 / 168.

**Why two passes exist at all — re-derived by measurement, because the original reason no longer holds.** Each link was checked, not assumed:

1. **Bundled dts really does break the augmentation.** Built with `dts: { sourcemap: true }` and no `unbundle`: `StateContext` / `NavigationOptions` land in a shared chunk (`index-DCSOG7_g.d.mts`) and `types.d.mts` is a pure re-export barrel — exactly the #1540 shape.
...
5. **So the only single-pass option is unbundling EVERYTHING — and the cost is not what #1540 recorded.** That entry rejected full unbundle at "+713 B brotli on the root entry"; today the same experiment measures **+60 B** on `@real-router/core` and **+110 B** on `/api` (22.60 → 22.66 kB, 26.30 → 26.41 kB), and the tarball is actually _smaller_ (535 KB vs 623 KB — many small sourcemaps compress better than two huge ones). Judged on size alone the two passes would no longer be worth it.
   ...
   Anyone revisiting this: the size argument is dead, the load-time one is why the two passes stay. Re-measure both before changing it.

**The first version of this fix was more elaborate and got cut.** It staged pass 1 in `.tsdown-dts/` outside `dist`, moved the declarations in with a 120-line fail-closed `scripts/collect-dts.mjs`, split the passes into `tsdown.config.dts.mts` + `tsdown.config.mts` with a shared `tsdown.entry.mts`, and added an `outDirBase` option to `tsdown.base.ts` — five build files where there had been two, plus a knob on the shared base that only one package would ever pass. Deleting instead of moving needs none of it: no staging directory, no `.gitignore` entry, no `outDirBase`, no map-`sources` revalidation (nothing moves, so the relative paths cannot break), and no second copy of the `entry` map to keep in sync. Both versions produce a **byte-identical** `dist` (same sha over every file+size) — kept the simpler one.

**Verified against the previous output, not just against itself.** Consecutive clean builds now produce a byte-identical `dist` (one sha over every file+size — the old config produced two different ones). The declaration tree is **unchanged**: same 76 files, all 76 byte-identical to the old build; `check-dts-augment-targets.mjs` still passes, so the #1540 invariant this whole two-pass design exists for is untouched. All 6 ESM JS files are byte-identical too, and the 4 CJS entries match the bundled variant the old build produced only when it happened to win its own race. `attw` clean, `publint` unchanged (still only the pre-existing `engines.node` note), `smoke-test-packages.sh` resolves 35/35 exports from a real tarball install, and the full `turbo run build lint:package lint:types` is green across 117 tasks. Build time 3.42 s → 3.56 s — the passes no longer overlap, which is the whole point.
...
## Upstream caught up: `@changesets/changelog-github` 0.7.0 → 1.0.1 retires the `maxBatchSize` patch (2026-09-06)

**Problem.** The 2026-07-30 entry above shipped `patches/@changesets__get-github-info@0.8.0.patch` — `{ maxBatchSize: 20 }` on the library's DataLoader — and attached its own retirement condition: _"the patch must be re-validated on any `@changesets/*` upgrade"_. That upgrade exists now. `@changesets/changelog-github@1.0.0` (2026-08-11) and `1.0.1` (2026-09-04) carry `@changesets/get-github-info@1.0.x`, whose changelog names exactly the same defect the patch was cut for: _"Improve data fetching by using GraphQL fragments to reduce the query size, and set a maximum batch size of 50"_ (changesets#2121).

**Solution.** Bump the root devDep to `1.0.1`, delete the `patchedDependencies` entry from `pnpm-workspace.yaml` and the `patches/` directory with it. No port of the patch: upstream's bound is strictly tighter than ours, which is measured rather than assumed.

**Measured, on the incident's own shape.** A sandbox running `@changesets/cli@2.31.1` against 109 changesets in 109 commits, with `GITHUB_GRAPHQL_URL` pointed at a local stub that logs every document:

| build                                       | requests | largest document | total bytes |
| ------------------------------------------- | -------- | ---------------- | ----------- |
| 0.8.0 unpatched (what failed on 2026-07-30) | 1        | 59 236 B         | 59 236 B    |
| 0.8.0 + our patch (batch 20)                | 6        | 10 998 B         | 60 026 B    |
| 1.0.1 (batch 50 + fragments)                | 3        | **9 396 B**      | 20 767 B    |

The fragments are what make 50 cheaper than our 20: 0.8.0 inlined the whole commit selector per changeset, 1.0.x emits `...CommitFragment` per changeset and one fragment definition per document. So the failure mode the patch existed to bound — document complexity at validation time — is bounded harder without it.

**Why a MAJOR of the changelog generator is safe under `@changesets/cli@2.31.1`.** 1.0.0 is ESM-only, drops `dotenv`, and moves to `@changesets/types@^7` while the CLI stays on `^6` — none of which the loader cares about: `apply-release-plan@7.1.1` resolves the module by name and loads it with `await import(...)`, and `@changesets/types` is types-only. Verified by running `changeset version` end-to-end in the sandbox on both versions: the generated CHANGELOG is identical once commit SHAs are normalised — at one changeset, at 109, and for the `Updated dependencies [...]` block a dependent package gets. Issue linkification (`#1938` → issue URL) and `Thanks @user!` attribution are unchanged.

**Two workarounds fall, not one.** `node-fetch@2` had exactly one parent in this tree — `@changesets/get-github-info@0.8.0` — and 1.0.x uses the platform `fetch`. `pnpm why -r node-fetch` now returns nothing, which removes the _subject_ of the Node pin in `changesets.yml` (2026-06-27 entry: 24.17.0's keep-alive change surfacing a false `ERR_STREAM_PREMATURE_CLOSE` in node-fetch@2). The pin is released in the same batch: `changesets.yml` reads `node-version-file: .nvmrc` like every other workflow, and CLAUDE.md's toolchain table no longer carries an exception.

**Three overrides answered to Changesets 2, and two of them were wrong afterwards.** `read-yaml-file` left the tree entirely: its only parent was `@manypkg/get-packages@1`, the workspace reader the v2 CLI depended on, and v3 moved to `@manypkg/get-packages@3`. Its override is deleted rather than left as a floor over nothing. `js-yaml`'s note listed `@changesets/parse` among the parents it protects; `parse@1` reads `yaml` now, so the list names its real remaining parents. ⚠ **`yaml@2` was pinned BELOW what a consumer declares, and an override outranks semver, so nothing said so.** `@changesets/parse@1.0.0` asks for `^2.9.0` while the entry read `2.8.3`; pnpm resolved the override and the release path's changeset parser ran one minor behind its own contract. Raised to `2.9.0`, which every other consumer on that line accepts — `pnpm dedupe` and `lint:dedupe` still resolve one 2.x copy. The lesson generalises past this line: an exact override is a claim about EVERY consumer's range, and a bump can invalidate it silently from the other side.

**Fix that arrives for free.** 1.0.1's `cacheKeyFn` (changesets#2264) keys the DataLoader cache on `repo`+`sha` instead of the request object. Measured in the two-package sandbox: 0.7.0 fetched the same commit twice (once for `getReleaseLine`, once for `getDependencyReleaseLine`), 1.0.1 fetches it once. Across 23 packages that duplication scaled with the dependency fan-out.

**What did not change, deliberately.** The `template` option (changesets#2059) that 1.0.0 adds is opt-in, experimental, and its authors reserve the right to change token syntax in a patch — the default line format is what this repo's CHANGELOGs and `.changeset/aggregate-changelog.mjs` already expect, so it stays unconfigured. The `GITHUB_REPOSITORY` fallback is likewise unused: `.changeset/config.json` names the repo explicitly. Dropping `dotenv` is a no-op here — the local `.env` holds `SONAR_TOKEN` and `CONTEXT7_API_KEY`, never `GITHUB_TOKEN`, and CI passes the token through the step env.

## Changesets 2 → 3: the release job stopped being able to read its own tags (2026-09-06)

**Problem.** `@changesets/cli` 2.31.1 → 3.0.2 is a whole-ecosystem major (`config@4`, `apply-release-plan@8`, `assemble-release-plan@7`, `@clack/prompts` for `enquirer`, `tinyexec` for `spawndamnit`, `cac` for `mri`), and the one thing in it that breaks this repo is invisible from the changelog: **the string `New tag: ` no longer exists in the CLI**. `changesets.yml` parsed exactly that out of `changeset publish`'s stdout to build `new_tags`, and the "Push git tags" step is gated on `new_tags != ''` — so on v3 the gate would evaluate false, the step would be skipped whole, and with it the fallback that creates tags the CLI silently failed to create (changesets#1621). npm would carry the release; git and the Releases page would not. Nothing fails; nothing is red.

**Solution.** Read the tags from the **event stream** instead of the console. v3 writes NDJSON to the path in `CHANGESETS_OUTPUT` (or `--output`), one `{"type":"git-tag","tag":…,"packageName":…}` per created tag, colour-free and line-delimited. The publish step sets `CHANGESETS_OUTPUT="$RUNNER_TEMP/changesets-publish.ndjson"` and extracts `tag` with a five-line `node -e`. Everything downstream is untouched: `publish_status`, the push loop, the manual-tag fallback and the reconcile step all keep their contracts.

⚠ **stdout is not merely reformatted — for our case it is incomplete.** `publish` prints `Created git tags:` from `formatGitTagResults(tool, { tagged: results.tagged.filter(r => tagOnlyReleases.has(r)) })`, and `tagOnlyReleases` holds only the releases the plan marked `tag-only` — i.e. private packages. A _published_ package's tag never appears in that block. Measured against a local **verdaccio** with two publishable packages and one private one: three tags were created, the NDJSON carried all three, and stdout's `Created git tags:` listed **only the private one**. A regex retuned to the new wording would therefore pass a private-only rehearsal — where every release IS tag-only, and v2's `New tag:` and v3's list agree — and drop every real release tag in production.

⚠ **`pnpm install` before `publish` is load-bearing in a way it was not before.** v3 routes publishing through the package manager's own CLI (changesets commit `8c88f6a`), so `pnpm publish` — not Changesets — expands `workspace:` ranges, and it refuses to do that without installed links: the same sandbox published cleanly after `pnpm install`, and died with `ERR_PNPM_CANNOT_RESOLVE_WORKSPACE_PROTOCOL` on the copy whose dependencies came from npm. The release job already installs (`--frozen-lockfile --ignore-scripts`) before bundling, so this holds today; dropping or narrowing that install would now break publishing, not just the build. What lands on the registry is unchanged — `workspace:^` still publishes as `^0.127.0`.
...
**`changesets/action` moves with it — 1.9.0 → 2.1.1 — and merging Dependabot's PR alone would have broken the release.** The action's v1 line is, by its own README, "the v1 code compatible with Changesets v2"; v2.0.0 is "Update to Changesets v3 packages", so this half is not optional. What makes the Dependabot PR (#1890) unsafe is that it edits only `uses:`, while v2 renamed **every input this step passes** — `version` → `version-script`, `title` → `pr-title`, `commit` → `commit-message` — and removed `setupGitUser`. Unknown inputs are ignored rather than rejected, so the step would have kept running: a bare `changeset version` with no `version-script`, skipping `cap-major-bumps` → `sync-version` → `aggregate-changelog`, under the default `Version Packages` title. v2 also stopped reading the `GITHUB_TOKEN` env var (changesets#674), so the PAT — whose whole purpose is that a PAT-authored PR triggers CI — has to move into the `github-token` input; left in `env:` it would silently degrade to the job token.

⚠ **`push-with-git-cli: true` is a deliberate hold, not a leftover.** v2's default commits through the GitHub API, and that path builds the release commit as a diff of the working tree against `github.sha` — the default-branch tip — while this job deliberately checks out `workflow_run.head_sha`. When a second push lands between the two, the API path would commit a revert of it. v1's `prepareBranch` did `git reset --hard github.sha` before versioning, and the CLI path in v2 still does; keeping it is what makes this bump behaviour-preserving. The `GIT_AUTHOR_*`/`GIT_COMMITTER_*` env stays for the same reason, and it also keeps v2's `ensureGitUser()` a no-op — it returns early when `git var GIT_AUTHOR_IDENT` resolves, which those variables guarantee, reproducing v1's `setupGitUser: false`.

The v3 `.changeset/pre/` restructure (changesets#2190) is out of reach either way: it only applies in prerelease mode, which this repo does not use.

## `danger` 14 hands the undici version back to us, and our override was already holding it (2026-09-06)

**Problem.** Danger 13 declared `undici` as an ordinary dependency pinned to `6.21.1`, so its HTTP client was whatever Danger shipped — and a CVE on that line meant waiting for a Danger release. This repo had already stepped around that: `pnpm-workspace.yaml` forces `undici: '>=7.29.0 <8.0.0'` tree-wide, which quietly overrode Danger's own pin. The bump makes that supported rather than accidental.

**Solution.** `danger` 13.0.10 → 14.0.7. The major's single breaking change is that `undici` becomes an **optional peer dependency** (`^6.28.0 || ^7`) defaulted to 7, raising Danger's effective Node floor to 20.18.1 — which costs nothing here, since `.nvmrc` is 24 and the override already had the tree on `undici@7.29.0`. With `autoInstallPeers: true` the peer resolves to that same copy, and pnpm records the override's range as Danger's peer range in the lockfile.

**What the bump actually removes.** Signing in as a GitHub App no longer goes through `jsonwebtoken` (it uses node's own crypto), so that package and everything under it leave the tree: `jsonwebtoken`, `jwa`, `jws`, `ecdsa-sig-formatter`, `buffer-equal-constant-time`, `safe-buffer` and six `lodash.*` singles. The crash it fixes was never ours — this repo authenticates Danger with `GITHUB_TOKEN`, not an App — but the dependency surface is real.

**Verified by output, not by reading.** `danger pr` was run against two merged PRs of different shapes (one 43-file fix, one 83-file docs pass) on both versions with the same dangerfile: the reports are **byte-identical**, down to the warning threshold and the statistics table. ⚠ The first probe was a one-line Dependabot PR and returned "passed review, received no feedback" on both versions — a comparison that would have held even if the DSL had broken, which is why the substantive PRs replaced it. `dangerfile.ts` also still type-checks clean against the 14.x DSL types, and both invocations this repo ships — `pnpm danger ci` in `danger.yml` and `danger local` behind `danger:local` — start and behave as before.

**Not relevant, checked rather than assumed.** The `github.reviews` state fix and review pagination touch an API this dangerfile never reads (its whole surface is `danger.git.*`, `danger.github.pr.{body,user,additions,deletions}` and `danger.systems`). 14.0.6 and 14.0.7 are about the Homebrew binary and the Docker image behind Danger's GitHub Action; this repo runs the CLI from `node_modules`, so neither path is ours.

## The stranded-release watcher never ran: `setup-node`'s automatic cache went looking for pnpm (2026-09-06)

**Problem.** `Stranded Release Watch` (#2057) is the detection half of a failure that is silent and permanent — a release commit whose `Post-Merge Build` was cancelled allocates versions that exist only in CHANGELOG sections, with no npm release and no tag. It ran once, on its first schedule, and failed: `##[error]Unable to locate executable file: pnpm`, from `actions/setup-node` itself, before `scripts/check-published-versions.mjs` was reached. So the guard against a silent failure was itself failing silently — a red daily cron nobody is paged about.

**Cause, which is a default rather than a mistake in the script.** The step's log shows `package-manager-cache: true`, an input nobody set: `setup-node` enables it by default, reads the root `package.json`, finds `packageManager: pnpm@…` and tries to invoke pnpm to locate its store. This job installs nothing — the script imports only node builtins and shells out to `git` and `npm view` — so pnpm is not on the runner and never needs to be.

**Solution.** `actions/checkout` and `actions/setup-node` move v5 → v7, and the step sets `package-manager-cache: false`. The version bump alone would have been enough: v5 auto-caches for **any** `packageManager` field, while v7 does so only when the field names npm (`/^(\^)?npm(@.*)?$/` in its `main.ts`), which `pnpm@…` does not match. The explicit `false` is kept because the default has already moved once in this direction and can move back, and because it states what is true of this job — it installs nothing.
...
⚠ **The manual step survives on purpose, and the CLI has no flag that removes it.** A hash is refreshed AFTER someone re-reads the claim; a `--all` that rewrote the table would delete the only thing the ledger asserts. What the tool removes is the arithmetic — computing sha1s, finding the row, editing a data file by hand — which is what made the ritual feel like the mechanism.

**Validated by mutation, both directions.** Editing one claim line in `packages/core/src/limits.ts` reds the drift cell (and the remainder cell, since the rewritten paragraph is also unrecorded — the pair is what identifies an edit); appending a new claim reds the remainder cell ALONE, with drift staying green; `--update --yes` then records it and the suite returns to green. The `#2120` control keeps its arms and gains a sibling that asserts the two events land in different cells, so a regression to whole-file keying cannot pass quietly.

**Migration was a re-shaping, not a re-verification.** Every file in the old table was passing its whole-file hash at the time of the move, so its claims were already recorded as read; splitting one hash into its paragraphs asserts nothing new about them.

## tsdown 0.23 quietly relaxes `attw`, and the typings still say otherwise (2026-09-06)

**Problem.** `tsdown` 0.22.13 → 0.23.0 is a 0.x minor, i.e. a major. Its migration guide renames eight options and none of them are ours — `bundle`, `outExtension`, `publicDir`, `removeNodeProtocol`, `injectStyle`, `inlineOnly`, `skipNodeModulesBundle`, `dts.cjsReexport` appear in none of the 21 `tsdown.config.mts` files, and core already used the new `unbundle: true`. The two changes that land without touching a config are the ones that matter, and one of them is invisible from the type: the `attw` default profile moved from `strict` to `esm-only`, which ignores CJS resolution failures — while `AttwOptions.profile` still carries `@default 'strict'` in the typings of BOTH versions. Read from the implementation instead: `const { profile = "strict" … }` in 0.22.13 against `const { profile = "esm-only" … }` in 0.23.0.

**Why that matters here specifically.** `tsdown.base.ts` states the coverage it wants — "node10 / node16-cjs / node16-esm / bundler (same coverage as the old `attw --pack .`)" — and this repo publishes dual ESM/CJS, where `dist/cjs/**` is exactly the half that has broken unnoticed before (see the smoke-test entry that found CJS had never been loaded by any gate). Under the new default that check would have gone on passing while testing less. `attw` is now `{ profile: "strict" }`, explicitly, with the reason inline.

**The other change rewrites every `.d.ts` without changing what they say.** `rolldown-plugin-dts` 0.27 → 0.28 emits inline `export interface` / `export declare function` instead of a trailing `export { … }` list. Every artefact moves — all 182 files in core — so a hash comparison says "everything changed". The contract does not: the exported NAME SETS are identical (71 in `@real-router/core/types`, 17 in `ssr-utils`), and the augment targets stay lexical declarations, now as `export interface StateContext {}`. `check-dts-augment-targets.mjs` passes on the new output ("no duplicates in 80 dts files"), which is the guard that exists because a re-export of any form would make module augmentation a silent no-op.

**Measured rather than reasoned.** All 23 packages bundle clean under `CI=1` (so `failOnWarn` is armed). `size-limit` moves 14 of 23 checks, every one of them between −1.16 % and +0.53 % (largest absolute delta 35 B) with no budget exceeded — the signature of a new code generator, not of changed resolution. That distinction is worth stating, because `resolveDepSubpath` also flipped to `false` by default in this release: had it changed what gets bundled, the deltas would be kilobytes, and the built externals confirm it — `@real-router/core`, `@real-router/core/api`, `@real-router/route-utils`, `@real-router/sources` and the framework peers all remain external, subpaths included.

⚠ **Two size checks moved that tsdown does not build.** `solid` (rollup + babel-preset-solid) and `angular` (ng-packagr) shifted by +9 B and +5 B. Their builders did not change, so the cause is elsewhere in the rebuild, and it is unexplained rather than understood — worth remembering if a future bump shows a larger unexplained move in a package tsdown never touches.

## pnpm 11.26 and the action pin that has to move first (2026-09-07)

**Problem.** A pnpm 12 study (`.claude/pnpm-12-migration-research-2026-09-07-ru.md`) found the repo already compatible with the next major — pnpm 12.3.4 accepts every setting in `pnpm-workspace.yaml`, which is that major's headline breaking change — but blocked by something outside it: once `packageManager` names a pnpm 12 version, `pnpm-lock.yaml` becomes a two-document YAML stream, and **no released osv-scanner reads the second document**. Measured on this tree: the pnpm 11 lockfile gives `found 1817 packages` and two known High advisories; the pnpm 12 one gives `found 9 packages` and `No issues found`, exit 0. The fix exists in `osv-scalibr` (PR #2385, merged 2026-08-28) and in no shipped scanner — the last release, v2.5.1, predates it. `lint:audit` would go green over 0.5 % of the tree with nothing printed to say so.

**Solution: take the three preparatory steps that stand on their own, and leave the major.**

- **`pnpm/action-setup@v6` → `@v6.1.0`** at all eight call sites. pnpm 12 support landed in v6.1.0 and upstream has not moved the floating `v6` tag to it. This is the step that must come FIRST, because its failure mode is the whole CI dying at its setup step on the day the pin moves — see the `pnpm/action-setup` section above.
- **`packageManager` 11.18.0 → 11.26.0.** Eight minors of the same major; `namedRegistries` (registry-qualified lockfile keys, so a package cannot be substituted across registries) arrived in 11.20.0. The `+sha512.<hex>` is derived by decoding `pnpm view pnpm@11.26.0 dist.integrity`, and — as in the 11.9 → 11.18 bump — the derivation was validated against the OUTGOING pin first: decoding 11.18.0's SRI reproduces the old field byte-for-byte, so a bad hash could not have passed as a working one.
- **`loglevel` `error` → `warn` — tried, and reverted the same day.** See below.

⚠ **A minor still re-keys peer variants, so the bump is two commands, not one.** `pnpm install` left the lockfile untouched, and `pnpm dedupe --check` then failed: 11.26 resolves `@volar/typescript@2.4.28` with its `(typescript@6.0.3)` peer suffix, which propagates to `rolldown-plugin-dts` and `tsdown`. Controlled against 11.18, where the same lockfile passes. One `pnpm dedupe` settles it (+7 lines net), after which `dedupe --check`, `lint:deps` and `osv-scanner` (still `found 1817 packages`, still one document) are all clean, and `core` + `sources` bundle under `CI=1`.

⚠ **`loglevel: error` had been hiding pnpm's own deprecation notices, and one was already standing** — the `$` version reference in the `@types/node` override, drawing a warning since before 11.18. A deprecation is the version before an `ERR_`, and pnpm 12 is the release that turns unrecognized workspace settings into exactly that.

⚠ **Lowering the level to `warn` was nevertheless wrong, and the revert is the entry worth reading.** pnpm writes `[WARN]` to STDOUT, and this repo parses pnpm's stdout as JSON: `scripts/build-matrix.mjs` runs `execSync("pnpm exec turbo query affected … --packages")` and hands the result straight to `JSON.parse`. The first warning line therefore killed the CI job that computes the build matrix — `SyntaxError: Unexpected token 'W'`, on the first PR opened after the change. The cost measured before the change — four extra lines on a full `turbo run lint:package` — counted the humans reading the output and not the MACHINES parsing it, which is the whole defect in that measurement. `error` is restored; deprecation notices are read deliberately (`pnpm --loglevel=warn install` during a bump) rather than by lowering the level for every invocation, including the ones a script is parsing. The same symptom had already appeared locally an hour earlier, on `knip --reporter json`, and was worked around instead of generalized.

**The `@types/node` override stays on the `$` syntax, deliberately.** pnpm asks for a catalog instead, and neither available shape is an improvement: a catalog entry plus an exact version in `package.json` duplicates the version with nothing to gate the drift — syncpack has no notion of catalogs — while putting `package.json` itself on `catalog:` lands in Dependabot's known-broken catalog handling (dependabot-core #14339, #16049, #15515, all open). Removing the override is not the third option: measured, it drops the pin from 20 `peerDependencies` entries, which fall back to `>=18`, `*` and similar open ranges. They all still resolve to 24.13.2 today, which is the point — the override is what makes that true rather than a coincidence.

**Why not the major, restated as a condition.** The unblocking event is a released osv-scanner carrying the scalibr fix. Until then the check to run is not "is CI green" but `osv-scanner … --lockfile pnpm-lock.yaml` printing `found 1817 packages` rather than `found 9` — every other gate is green either way, which is what makes this failure mode worth writing down. Target ≥ 12.3 when it comes: on 12.0–12.2 `pnpm dedupe` did not converge in one pass over a pnpm 11 lockfile (pnpm#14455), and this repo auto-dedupes in pre-commit while checking dedupe in pre-push, so the two hooks would have pulled against each other.

**Update (2026-09-15): the osv condition is met; two conditions on GitHub's side are not.** osv-scanner 2.6.0 reads the second document, and the script refuses anything older (the entry "osv-scanner has a version floor" at the top of this file). What still holds the major:

- **Dependabot cannot run pnpm 12 until dependabot-core #16170 reaches it.** pnpm 12's launcher downloads its native binary with a `fetch()` that ignores the updater's proxy, so every npm job in a repository pinning pnpm 12 fails at `Could not download the pnpm … binary` (dependabot-core #16095). #16170 merged on 2026-09-15. The job log names the updater image as `dependabot-updater-npm:<commit>`, and across this repository's npm jobs from 09-01 to 09-14 that commit was 0.1 to 2.6 days older than the job; the fix has arrived once that commit contains `2a997d2c`. The other open pnpm 12 PR, #16169, gates nothing here: Dependabot reads `SUPPORTED_VERSIONS` to resolve an `engines` range and to reject versions older than all of them, and this repository pins an exact `packageManager` with no `engines`.
- ⚠ **GitHub's dependency graph reads only the first document.** For pnpm the graph comes from static parsing — GitHub's supported-ecosystems table marks pnpm "not supported" for Dependabot graph jobs — so the dependabot-core grapher fix (#15968, deployed 2026-09-02) does not reach it (dependabot-core #15904, open). On seven public repositories with a two-document lockfile the graph holds 0 to 11 % of the package names in the project document, and lockfile commits that add real packages produce a dependency-graph diff with no change outside pnpm's own. That diff is what the required `Dependency Review` check reads, and the graph is what Dependabot alerts are raised against. The seven lockfile commits date from 09-01 and 09-02; a report on #15904 shows the same for a lockfile pushed on 09-07, after the grapher fix. This repository's graph, the control, holds all 1479 package names of its lockfile.

**A workaround exists, and it costs version enforcement.** With `pmOnFail: ignore` in `pnpm-workspace.yaml`, pnpm 12 writes a single-document lockfile, which every reader above handles: measured with pnpm 12.3.4 on a probe project, two documents by default and one with the setting. The setting also stops pnpm from switching to the pinned version — pnpm 12.2.1 run in the probe reports 12.3.4 by default and 12.2.1 with the setting, without a warning. CI would not notice, because `pnpm/action-setup` installs the pinned version itself; a local pnpm of another version would write the lockfile.

## Coverage globs were repo-relative, and Vitest 4 was the only reason that worked (2026-09-07)

**Problem.** `vitest.config.unit.mts` declared `coverage.include: ["packages/*/src/**/*.ts", …]`, but every package runs `vitest` from its own directory, so the coverage root is `packages/<pkg>` and the real paths are `src/**`. Vitest 4 matched anyway through loose "contains" matching. Vitest 5 matches strictly against the root, where `packages/*/src/**` matches nothing: measured on the whole workspace, **16 of 23 packages dropped to 0 statements**, `lcov.info` went to 0 bytes, and the 100 % thresholds passed **vacuously** — exit code 0, CI green, SonarCloud fed 0 % on new code. The `.claude/pnpm-12-…` study found the same shape one layer down; this is the class, not the incident.

**Solution.** The root include becomes `["src/**/*.ts", "src/**/*.tsx"]`, and `packages/svelte` — the one package whose own override was repo-relative without `allowExternal` — becomes `src/**/*.{ts,svelte,svelte.ts}`. Both were verified on BOTH majors: on Vitest 4 all 23 packages reproduce their baseline statement counts and percentages exactly (46/46 turbo tasks green), and on Vitest 5 no package reports zero.

⚠ **A root-relative include walks into the symlinked shared trees, so it needs a matching exclusion — and the exclusion cannot be a literal list.** `src/browser-env`, `src/dom-utils` and `src/shared-ssr` are symlinks into `shared/` in ten packages, and `src/**` measures them in every consumer rather than in the one owner that is supposed to: measured, `navigation-plugin` went from 268 statements to **687 at 39 %** and failed its own 100 % threshold on files belonging to `browser-plugin`. But `packages/angular/src/dom-utils` is a git-tracked COPY, not a symlink (ng-packagr does not follow them), so a hard-coded `"src/dom-utils/**"` deletes 606 statements of Angular's OWN source — measured, 1027 statements at 99.02 % became 421 at 97.86 %, thresholds red. `symlinkedSharedDirs()` therefore derives the exclusion from `readdirSync(…, { withFileTypes: true })` + `isSymbolicLink()`, which is right for both shapes and for any consumer added later.

⚠ **`browser-plugin`, `react` and `ssr-data-plugin` keep their repo-relative globs deliberately — the obvious fix breaks Vitest 4.** All three set `coverage.allowExternal` so they can measure their `shared/<dir>` by its real path. Under Vitest 4's loose matching a bare `src/**` then matches every collected file in the workspace, not just their own: measured, `browser-plugin` went from 493 statements to **4458 at 57.8 %** and `ssr-data-plugin` from 338 to **4392 at 52.8 %**. There is no literal that is exact under both majors here — v4 matches loosely against the full path, v5 strictly against the root — so these three are a migration-time task. Their failure mode under v5 is under-reporting (419/606/266 of 493/884/338, all still at 100 %), not the silent zero the root glob caused.

**Correction to the Vitest 5 study.** That report recorded a divergence in `packages/react` between `vitest` and `vitest run` under v5. Re-measured on a repaired tree, that is **not the shape of it**: under v5 the full suite passes 861/861 and `tests/performance` ALONE fails 16 of 110; under v4 both pass. The original comparison was tainted by a stale bin shim — `packages/react/node_modules/.bin/vitest` still pointed into the `vitest@5.0.0` store entry after the tree had been reverted to 4.1.10, so runs believed to be v4 were v5. `pnpm install` does not rewrite such a shim; `pnpm install --force` does, and `grep -oE 'vitest@[0-9.]+' packages/*/node_modules/.bin/vitest` is what makes it visible. Same class as the earlier stale-shim incident. Angular additionally fails under v5 with `NG0303: Can't bind to 'errorTemplate'`, which is the Analog/Angular chain, not coverage.

**`cacheDir` moves to `.vitest-cache`.** From Vitest 5 the plain `.vitest/` is the conventional home for generated reports (`index.html`, `json/output.json`, `junit/output.xml`, `attachments/`, `blob/`), and Vite's cache was pointed at exactly that path. Everything that treats `.vitest` as disposable — `clean-all.sh`, the Stryker sandbox ignore lists — would then have been deleting reports; `.gitignore` and those three lists move with it.

## knip 6.28 stopped hiding re-exports, and four of the 58 it found were alive (2026-09-07)

**Problem.** `knip.json` sets `ignoreExportsUsedInFile`, and until 6.28.0 that option silenced unused RE-EXPORTS as well as unused exports. 6.28.0 ships one line — _"Report unused re-exports when `ignoreExportsUsedInFile` is set"_ ([knip#1895](https://github.com/webpro-nl/knip/pull/1895)) — and 58 dead re-exports appear at once across 18 intermediate barrels, turning `lint:unused` (a pre-push gate) red. Bisected across the ten releases in range: every finding arrives in 6.28.0 and none before. The other candidate, 6.29.0's _"do not follow symbolic links in async glob"_, changes no finding at all — it only turns the ten `ignore` patterns for the symlinked shared sources into "Remove from ignore" hints.

**Solution.** The findings are cleared, not suppressed: each flagged symbol is reachable by the path consumers actually use (`createRouteTree` from `../../engine`, `EVENTS_MAP` from `./constants`, `ensureRegistryPromise` straight from `deferRegistryClient`), so the intermediate barrel's copy was dead weight. `@real-router/core`'s published entry emits the same export list afterwards, `Unsubscribe` included. The bump then lands with the 13 config entries the release train made obsolete — `playwright` from `ignoreDependencies`, `open` and `view` from `ignoreBinaries`, and the ten per-workspace `ignore` entries — each of which knip 6.34 names as a hint. Suppressing the findings in `knip.json` was the one move not on the table: it silences a real gap, and the gap is the reason the delta was measured at all.

⚠ **Four of the 58 were not dead, and the gate could not have known.** `FSMConfig`, `NullFormat`, `QueryParamPrimitive` and `QueryParamValue` are imported by property-test helpers, while core's knip `project` is `src/**` plus `tests/benchmarks` — the property tiers sit outside its scan. They surfaced as four `TS2305` on the first `type-check` after the removal, not from knip. Restoring the re-exports is not the fix, because knip would flag them again for a use it cannot see; the two helpers now import each type from the module that declares it, as every other consumer already does. The general form: **a knip finding is a claim about the files knip scans, and this repo deliberately scans `src/**` — so `type-check` over the full graph, tests included, is the control arm for every removal it suggests.**
...
**Solution, in two halves.** The paths are already fixed: jscpd 5.1.1+ emits `uriBaseId` per result plus `originalUriBaseIds` naming each scan root as an absolute `file://` URI, which is exactly the shape GitHub documents ("Code scanning interprets results that are reported with relative paths as relative to the root of the repository analyzed. If a result contains an absolute URI, the URI is converted to a relative URI."). Applying that rule to the two real reports: **5.0.12 leaves 16 of 16 locations unresolvable, 5.1.2 leaves 0** — `packages/browser-plugin/src/index.ts`, `packages/core/src/namespaces/…`, files that exist.

The half that is new is `scripts/check-sarif-paths.mjs`, run between the scan and the upload. It resolves every `physicalLocation` — primary and `relatedLocations` — by GitHub's rule and exits 1 if any names a file this repository does not have. It is pure stdlib because that job deliberately skips `pnpm install`, and it carries **no `continue-on-error`**: the job sits outside the `ci` gate's `needs` (0 occurrences), so a red step there is loud and blocks nothing, which is the right trade for a guard against silence.

⚠ **The guard refuses shapes it cannot read rather than skipping them** — an undefined or chained `uriBaseId`, an `artifactLocation` without a `uri`, an empty `uri`, a location with no `artifactLocation`. A skipped location is a location silently declared fine, which is this defect one level down; the first draft skipped one of these cases and mutation testing caught it. It also separates the two ways of checking nothing: no results at all is a quiet pass, while results carrying no resolvable location is an error, because that is a report read to no effect.

**Sibling, measured and left alone.** `upload-sarif` has exactly one call site (positive control: `actions/checkout` has 27). The same class lives one channel over — `scripts/fix-lcov-paths.sh` rewrites vitest's package-relative `SF:` paths to repo-root-relative for Codecov and SonarCloud, and nothing verifies the result either. Simulating that rewrite over the 23 lcov files gives **427 SF entries, 0 unresolvable**, so it is healthy today and unguarded tomorrow. ⚠ That simulation omits the script's `../../shared/` collapse for shared-source owners, so it is evidence and not proof; the assertion belongs in `fix-lcov-paths.sh` itself, where the collapse already lives, and was not added here on a partial measurement of a path three coverage jobs depend on.

## tinybench 6.1 ships timer diagnostics the CodSpeed gate cannot execute (2026-09-07)

**Problem.** The `6.0.2 → 6.1.6` bump carries exactly one feature across its eight releases, and it reads like the guard this repo keeps writing by hand: [tinybench#571](https://github.com/tinylibs/tinybench/pull/571) adds timer-overhead correction, a `'warning'` event when a task's samples are dominated by the timer grain, and `Task.detectedResolution`. A bench whose samples are timer noise measures nothing and passes green — the same silence `check-sarif-paths.mjs` exists for, one layer over.

**Measured, and this is why none of it is wired.** `withCodSpeed` **replaces** `bench.run()` and `bench.runSync()` with its own loop over `bench.tasks`, so under the runner tinybench's own measurement path — and every diagnostic living inside it — never executes. Off the runner the plugin returns the bench untouched (`codspeedRunnerMode === "disabled"`), so the diagnostics do run locally, where the wall-clock numbers are explicitly not the gate. A saturation listener would therefore fire only where nothing is decided.

`subtractTimerOverhead` stays off for a second, independent reason: it is opt-in (`@default false`, confirmed in the installed dist as `subtractTimerOverhead = i.subtractTimerOverhead === !0`), it targets sub-microsecond work, and it is rejected outright with `concurrency: 'task'`. The batching doctrine already puts the tasks far out of its range — the core suite measures medians of **242–357 µs** against a timer grain measured at **~41 ns**, a margin of ~6000×. Control arm for that number: a bare `() => {}` task fires `'warning'` with `reason: 'zero-dominated'` while a batched task stays silent, so the detector discriminates rather than being uniformly quiet.

⚠ **The type-level half of the bump is gated, and only by an inclusion that is easy to lose.** [tinybench#633](https://github.com/tinylibs/tinybench/pull/633) turns on `exactOptionalPropertyTypes` in tinybench's own build (upstream records that it broke Vitest CI). This repo sets that flag too, and the benches sit inside the gate only because `packages/core/tsconfig.json` declares `include: ["src", "tests"]` — narrow that to `src` and every `tests/benchmarks/*.bench.ts` leaves type-check without a single failure to announce it.

⚠ **`benchmarks/` carries no `type-check` task by decision, and the manual recipe standing in for it names the one config that cannot see the typed apps.** The workspace is deliberately outside the turbo graph and `bench:type-check` was dropped on purpose ("Retire the router5/6 mitata + vs-tanstack jsdom benchmark suites"), so what remains is `benchmarks/CLAUDE.md`'s instruction to run `tsc --noEmit -p benchmarks/tsconfig.json` "for the typed apps" — a config whose `include` is `["audit-probes"]`, the single tree that same decision declared intentionally non-strict. Measured: 249 errors across 104 files, every one of them a probe, and `adapter-bench` / `plugin-seam` / `cross-router` absent from the program entirely. This bump was validated against `benchmarks/adapter-bench/tsconfig.json` instead, which the recipe never names. **Fixed the same day — see "The benchmarks type-check recipe named the one config that could not see the typed apps" below (#2159).**

## The benchmarks type-check recipe named the one config that could not see the typed apps (#2159, 2026-09-07)

**Problem.** `benchmarks` is deliberately outside the turbo graph and `bench:type-check` / `bench:lint` were dropped on purpose (see "Retire the router5/6 mitata + vs-tanstack jsdom benchmark suites"), so all that stood between the typed harness and no checking at all was one hand recipe in `benchmarks/CLAUDE.md`: `tsc --noEmit -p benchmarks/tsconfig.json` "for the typed apps". That config's `include` is `["audit-probes"]` — the one tree the same decision declared intentionally non-strict. Measured: **249 errors across 104 files, every one a probe, and not one typed app in the program.** The recipe was never right: `git blame` puts it in `8019b112d` (2026-08-18), a commit titled "correct two stale paths in the benchmarks guide", by which time the `include` already read `["audit-probes"]` and both `adapter-bench` and `cross-router` already existed.

⚠ **The guide carried a second, contradicting recipe.** The directory-tree line labelled `cross-router/tsconfig.json` "(react)" and stated that svelte apps have "NO tsc step" and angular apps "NO separate tsc — the AOT build type-checks". Measured, that config holds **94 angular + 78 react + 68 svelte** files, and the AOT claim is refuted by the defect it hid: `deep/level.component.ts` and `nested/app.component.ts` both **build clean** through `@analogjs/vite-plugin-angular` while `tsc` reports `TS2449` on each. A build proves an app runs; it is not a type-check.

**Solution.** Three standing errors cleared — the two recursive Angular components now self-reference through `forwardRef(() => …)` (Angular's documented mechanism for a class whose declaration follows its own decorator; runtime verified by Playwright: the deep app still stamps 90 nested levels, the nested app still keeps its chain mounted across an a↔b swap), and `plugin-seam/bench.mts` annotates its `searchSchema` literal `satisfies StandardSchemaV1` so `version: 1` stops widening to `number`. The recipe now names the **four** configs that actually hold the typed harness, in one place instead of two, and says plainly that `benchmarks/tsconfig.json` is the probe config. `adapter-bench/tsconfig.json` additionally pulls in `./vite/*.mts` (with `allowImportingTsExtensions`), taking that program from 10 of 23 files to 17 and surfacing one more real defect: `adapterBuild` declared `ReturnType<typeof defineConfig>`, but `defineConfig` is overloaded, so the alias resolved to the widest overload and `svelte.config.mts` could not reach `.resolve` on it.

⚠ **What is fixed is not pinned, and the gap is measurable.** Mutation over the six data terms this fix introduces: five red, one green — deleting `./vite/*.mts` from the `include` drops the program back to 10 files and every command still exits 0. Narrowing an `include` makes these checks _greener_, so coverage is the one property they cannot self-report, and nothing in the repo runs them anyway. The guide now carries the `--listFiles | grep -c` re-derivation instead of a coverage claim to be trusted; a real guard would have to run somewhere, and giving `benchmarks` a gate is the decision above, not a bug fix.

## adapter-bench's six app entries join the type-check, two of them in their own configs (#2167, 2026-09-07)

**Problem.** #2159 took `benchmarks/adapter-bench`'s program from 10 of its 23 TypeScript files to 17. The six that stayed out were `apps/**` — the entries the CodSpeed adapter job actually measures — and nothing checked them. Vite builds them, and a build is not a type-check: #2159 proved that on the angular side, where two components compiled clean through `@analogjs/vite-plugin-angular` while `tsc` reported `TS2449` on each.

**Solution.** Four of the six needed nothing but inclusion — `apps/react.tsx`, `apps/vue.ts`, `apps/angular/main.ts` and `apps/svelte/index.ts` are green under the config's existing `jsx: react-jsx`. The other two get their own configs, because one `jsx` setting cannot serve six frameworks: folded into the main program they raise **31** errors, every one from `packages/preact` (14) or `packages/solid` (17) and none from an app's own code. `tsconfig.preact.json` and `tsconfig.solid.json` mirror what each adapter package already declares for itself — preact `jsx: react-jsx` + `jsxImportSource: preact`, solid `jsx: preserve` + `jsxImportSource: solid-js` — rather than copying `cross-router`'s carve-outs, which use `preserve` for both. Coverage of `adapter-bench` is now **23 of 23**, measured as the union of the three programs' `--listFiles` against `git ls-files`.

⚠ **The carve-outs are load-bearing, not decorative.** Dropping `jsxImportSource` from the preact config raises 5 errors and from the solid config 8; dropping the main config's `exclude` raises the full 31. A config whose `include` matches nothing also exits 0, so each program was checked against `--listFiles` for the file it exists to cover before any of this was believed.

⚠ **`apps/svelte/index.ts` is checked; its templates are not.** The `./Host.svelte` import resolves to Svelte's ambient `*.svelte` declaration and types as `LegacyComponentType`, not `any` — verified by assignment probe, alongside a control showing `TS2307` still fires for an ordinary unresolved specifier in the same file. The component's own props remain the svelte plugin's business at build time.

## publint and attw never ran on angular or svelte, and one of the findings was real (#2155, 2026-09-07)

**Problem.** `tsdown.base.ts` validates every package it builds, and its comment said the three non-tsdown packages "keep their own validation". Only `solid` did. `angular` (ng-packagr) and `svelte` (svelte-package) declared neither `lint:package` nor `lint:types`, so `turbo run lint:package lint:types` — the pre-push step and CI's "Validate published artifacts" alike — ran **zero** tasks for them. Run by hand, both exited 1.

**The control arm did the work here, twice, and it flipped once.** Reading the shapes suggested svelte's four `InternalResolutionError` rows were a defect and angular's `Masquerading as CJS` was the ecosystem's price. Both readings were wrong-ended. `@angular/router` — `"type": "module"` — is 🟢 on node16-from-ESM where this package was 🎭, so the masquerade was **ours**. And `@mateothegreat/svelte5-router`, which re-exports 27 `.svelte` specifiers from its declarations exactly as this package does, reproduces the 🥴 table cell for cell, so those rows are **the ecosystem's**. ⚠ The first svelte control tried, `sv-router`, is green there and proves nothing: it ships **zero** `.svelte` specifiers, so it is not the same shape — a control has to match the shape being judged, not merely the framework.

**Solution, in the order the issue required.** First the one real defect: ng-packagr copies the source manifest into `dist/package.json`, and Node reads a file's module format from the NEAREST manifest above it, so `dist/types/*.d.ts` read as CommonJS beside ESM `dist/fesm2022/*.mjs`. `packages/angular/scripts/finalize-dist-manifest.mjs` now sets `"type": "module"` on the generated manifest after every build and drops the `@real-router/internal-source` condition from its `exports`. (It dropped the whole `exports` field at first, on publint's advice; that broke the secondary entry point's cross-entry import and was corrected — see the 2026-09-09 entry below.) attw's two node16 rows went 🟢 and publint went from one warning to "All good!". Only then were the gates wired: `angular` and `svelte` declare `lint:package` (publint) and `lint:types` (attw), taking the census from 21 covered public packages to **23 of 23**, `turbo run lint:package lint:types` at 29 tasks, RC 0.
...
## Node ignores a nested `exports` from outside and honours it from within, and ng-packagr's cross-entry import rides on the second half (2026-09-09)

**Problem.** The scheduled `Examples` run went red on `ssr-angular-example:test:e2e`: `ng build` could not resolve `@real-router/angular`, three times, from `packages/angular/dist/fesm2022/real-router-angular-ssr.mjs`. The specifier is not a mistake — `ssr/functions/injectDeferred.ts` imports `injectRoute` from the primary entry point by package name, which is the form ng-packagr documents for cross-entry imports (it wires tsconfig `paths` during compilation and leaves the bare specifier in the FESM).

Nothing above `packages/angular/dist` can answer that specifier. The walk goes `dist/node_modules` → `packages/angular/node_modules/@real-router/` (core, route-utils, sources, ssr-utils — no self-link) → `packages/node_modules` → the repo root, which has no `node_modules/@real-router` at all. An **installed** copy is fine: the walk reaches `node_modules/@real-router/angular` and stops. The gap is the workspace layout, not the published artifact.

**Why it started.** #2155 deleted `exports` from the generated `dist/package.json` on publint's advice — "ignored by Node.js … only works in root package.json files". That sentence is half a measurement. Node ignores a nested `exports` when resolving INTO the package from outside, and **honours it for a self-reference from within the package scope**, which is the only route left here. Measured both directions from `packages/angular/dist/fesm2022`:

```bash
node --input-type=module -e "console.log(import.meta.resolve('@real-router/angular'))"
# with the field:    file:///…/dist/fesm2022/real-router-angular.mjs
# without the field: ERR_MODULE_NOT_FOUND
```

**Solution.** `finalize-dist-manifest.mjs` keeps `exports` and strips only its `@real-router/internal-source` condition, whose targets (`./src/index.ts`, `./ssr/public_api.ts`) do not exist under `dist/`. `type: "module"` is untouched — that half of #2155 was right and is what attw's node16 rows ride on. The script then **executes** the claim rather than asserting it: `createRequire(dist/fesm2022/real-router-angular-ssr.mjs).resolve("@real-router/angular")`, which throws with the reason if the field ever goes away again.
