...
- **`loglevel` `error` → `warn`.** See below; this is the one that changes what the repo can see.
...
⚠ **`loglevel: error` had been hiding pnpm's own deprecation notices, and one was already standing.** The `$` version reference in the `@types/node` override has been drawing a deprecation warning since before 11.18 — invisible for as long as the level has been `error`. A deprecation is the version before an `ERR_`, and pnpm 12 is the release that turns unrecognized workspace settings into exactly that, so the level that hid the first notice is the level that would have turned the migration into a surprise. At `warn` a full `turbo run lint:package` prints four such lines, which is the measured cost.
