...
**Two workarounds fall, not one.** `node-fetch@2` had exactly one parent in this tree — `@changesets/get-github-info@0.8.0` — and 1.0.x uses the platform `fetch`. `pnpm why -r node-fetch` now returns nothing, which removes the _cause_ of the Node pin in `changesets.yml` (2026-06-27 entry: 24.17.0's keep-alive change surfacing a false `ERR_STREAM_PREMATURE_CLOSE` in node-fetch@2). The pin's stated relax condition was runner-toolcache drift; the stronger one — the library is gone — now holds too.
