...
### Batched measurements + heap settling — single-iteration GC lottery on the self-hosted runner (2026-07-16)

**Problem.** The #984 Phase 1 verification (two `workflow_dispatch` runs on the SAME sha `71ae8e57`, self-hosted VPS, 2026-07-15) still produced 3/56 phantom swings: `state/areStatesEqual-ignoreQuery` ×5.1, `navigate/sync-baseline` +82%, `navigate/same-state-reject` +41% (CodSpeed runs `6a57dc24…` vs `6a57df44…`). Consistent hardware had fixed the #984 *cross-CPU* cause, so this was a new, third variance class. Root cause (proven via CodSpeed's per-benchmark valgrind metrics + flamegraph diff): in `simulation` mode `@codspeed/tinybench-plugin` instruments a **single task invocation** (7 deterministic warmup calls → `global.gc()` → one measured call). The runner's determinism flags (`--predictable --predictable-gc-schedule --hash-seed=1 --random-seed=1 --no-opt --expose-gc --no-concurrent-sweeping`, from `@codspeed/core`'s `getV8Flags`) do NOT virtualize wall-clock, and V8/Node retain time-driven heuristics plus full-GC tails (page unmapping, finalization) that land on later macrotask boundaries — so GC work occasionally drops into the one measured iteration. Dirty measurements carry the signature: 11–24 syscalls with `sysSeconds` up to **1.6 s** (`madvise`/`munmap`/`futex`), inflated instruction+memory components (marking/copying), and `callgraphGenerationFailure: InternalsRootGapTooHigh` (no flamegraph). The victims: tasks **#1/#2 of the whole process** (`sync-baseline`, `same-state-reject` — the process's first full GC is its heaviest: tsx compilation + suite-setup garbage) and the lightest benches (7.7 µs of honest instructions for `ignoreQuery` — any event is a multiple). With a ~0.13–0.22 ms event against a ~10% gate threshold, **every bench under ~2 ms was in the risk class** — the other 53 merely won that run's lottery.

**Solution.** Two `fixtures.ts` helpers, applied across all 6 suites (all 56 tasks, zero task-name/URI changes):

- `addBatched(bench, name, iterations, fn)` — each measured call runs `fn` K times, K picked per current op cost so a batch costs **~3–6 ms** of simulated CPU (`<60 µs → 96 | 60–120 → 48 | 120–250 → 24 | 250–500 → 12 | ≥500 → 4`). A stray GC event is now ≤ ~5% noise, and GC becomes a steady, near-deterministic fraction of the batch instead of a lottery.
- `settleHeap()` — two `gc()` → `setImmediate`-drain rounds before every `bench.run()`, retiring the heaviest (first) full GC and its unmapper/finalizer tails outside any measurement window. No-op without `--expose-gc` (local wall-clock runs).

**Why this and not alternatives.** The tests were correct for local wall-clock tinybench (`time: 100` → thousands of statistically-averaged iterations); the fragility is specific to the plugin's single-shot instrumentation, which we don't control — measurement mass is the lever we do. Rejected: hand-adding `--max-semi-space-size` to the workflow's node invocation (hides GC cost from honest measurements too, and depends on the runner's flag-merge order); ignoring the 3 benches (the risk class is all 56, not the 3 that happened to flake). Consequences: absolute values scale ×K on the same URIs — a **one-time baseline re-seed** (already due: the self-hosted baseline is brand-new), and local `pnpm bench` numbers are per **batch** (divide by K when eyeballing; the numbers are not the gate — CodSpeed is). Gate re-enable order stays: batched suite green → **two more same-sha dispatch runs with 0 phantoms** → only then Phase 2 (push/PR triggers).

**Iteration 2 (same day) — two corrections from the first batched run pair (`6a588535`/`6a5887c4`, both on `c6ebc9cb`):**

1. **URI attribution.** The first cut wrapped `bench.add` inside a fixtures helper (`addBatched`) — and the CodSpeed plugin attributes the benchmark URI to the file that **calls `bench.add`** (`getCallingFile(1)` in its patched `add`), so all 56 URIs re-homed to `fixtures.ts::…`. Fixed by inverting the helper: `batched(K, fn)` returns the task **body** and the `bench.add(name, batched(K, fn))` call stays in the suite file.
2. **K must be sized from BATCHED per-op costs, not single-shot numbers.** The batched run exposed that the old single-shot values were dominated by the plugin's **fixed per-measure harness cost** (~50–60 µs simulated: async root frame + microtask drain) — e.g. `areStatesEqual-ignoreQuery` "54 µs" was ~99% harness, the op itself is ~0.08 µs; `matchPath/strict-query` "1.3 ms" is really ~41 µs/op. K derived from those inflated numbers landed far below the 3 ms budget (batch masses 67 µs–1.9 ms), and the pair still showed 3 phantoms — now shrunk to +14–20% (each delta ≈ one ~130–215 µs GC event, confirming the model). All 56 K re-derived as `K = ceil(4000 µs / per-op)` with `per-op = (batched-total − 60 µs) / K_old`, rounded up the {…, 96, 128, 192, 256, 384, 512, 768, 1024, …} series — now spanning 48…65 536 (operations mass ~4–6 ms each). Bonus finding from the pair: `matchPath/encoding-uri` at ~99 µs/op is **stable across runs** — genuinely ×7 its encoding siblings (a real cost question, parked; not a GC artifact).

**Verdict — phantoms eliminated.** Three independent same-measured-code run pairs now show **0/56** changed benches: `6a588c4b`↔`6a588fbc` (impact −0.0017 %), `6a588fbc`↔`6a589908` (−0.0006 %). The progression is `3/56` (#984 P1, ×5.1/+82 %/+41 %) → `3/56` (batched, K undersized, +14–20 %) → `0/56` (batched, K correct). The measurement-stability precondition for Phase 2 is met.

**Later the same day, the P1/P2 coverage additions (`navigate/params`, `navigate/query-params`, `matchPath/no-match` — commit `40f24030`) surfaced two more durable rules:**

1. **Suite-composition changes step a few baselines deterministically — expect it, don't chase it.** `--predictable-gc-schedule` is deterministic over the *allocation sequence*, so adding tasks (or changing any K) realigns where GC lands across every subsequent batch: a handful of *untouched* benches step once by ±≈1 GC event (observed −12.7 %/+12.9 % and one larger step) and then hold. Same-composition pairs stay 0-phantom. Treat these as a one-time re-baseline, not regressions to investigate.
2. **Size K only from ms-scale batched runs; the first honest run recalibrates everything.** The first CI run with real per-op numbers (`6a58a87f`) showed 8 benches sized off GC-polluted single-shot anchors sat *under* the 3 ms floor (sync-baseline's honest per-op is ~14 µs, not the polluted ~68), and one sub-µs op overshot to 34 ms because subtracting the ~60 µs harness from a single-shot total is noise-dominated for tiny ops. All 9 re-derived in one commit (`c58ba435`) — one realignment instead of a drip. **Task #1 of the process additionally needs ~double mass** (K=512, ≈7 ms for `navigate/sync-baseline`): it sits closest to startup noise (`--predictable` does not virtualize wall-clock), and at 3.5 ms it straddled the 10 % threshold run-to-run (`f4c7702e`). Final state: 59 benches, same-sha pair **59/59 unchanged** (impact +0.008 %).

### CodSpeed CLI installer unreachable from the RF-hosted runner — cargo-install pin (2026-07-16)

**Problem.** With batching done, the verification pairs kept losing runs to a *second*, unrelated failure: `curl: (28) SSL connection timeout` after 300 s, thrown by `CodSpeedHQ/action@v4`'s install step while downloading `https://codspeed.io/v4.18.4/install.sh`. The self-hosted VPS is in RF and reaches codspeed.io only intermittently (~50 % of dispatches died here, before any bench ran). This is **not** the sudoers/`NOPASSWD:SETENV` runner fix (that governs `sudo` for setup, not outbound HTTPS), and it blocks Phase 2 outright — auto-triggered PRs would flake on it.

**Solution.** `runner-version: rev:v4.18.4` in `codspeed.yml`. The action's install script routes any `rev:`/`branch:` value to a `cargo install --locked --git https://github.com/CodSpeedHQ/codspeed --rev <v> codspeed-runner` branch instead of the codspeed.io download — so it fetches from **github.com + crates.io only**. The tag `v4.18.4` exists in that repo and is the *same* runner the release path fetched, so instruction counts are unchanged (proven: `6a588fbc` downloaded-runner ↔ `6a589908` cargo-runner = 0/56, impact −0.0006 %). Getting to green took three follow-ups, each an empirically-surfaced VPS prerequisite the cargo path assumes but the release path did not:

1. **rustup/cargo for `gh-runner`.** The action does `source $HOME/.cargo/env` and calls `cargo`; the runner user had neither → `cargo: command not found`. One-time: `sudo -u gh-runner -H bash -c 'curl --proto "=https" --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --profile minimal'` (rustup ships from sh.rustup.rs / static.rust-lang.org — reachable). This also confirmed **crates.io is reachable** from the VPS: the first `cargo install` compiled the full dep tree in 5 m 05 s.
2. **A stale, cargo-*untracked* `~/.cargo/bin/codspeed`** made `cargo install` abort with `binary 'codspeed' already exists in destination` (cargo's pre-install collision guard; a *tracked* same-rev install would instead no-op with "already installed"). One-time `rm -f ~/.cargo/bin/codspeed ~/.cargo/bin/codspeed-runner`; the reinstall then records the binary, so future installs of the same rev skip cleanly.
3. **`~/.cargo/bin` off PATH.** Actions runs each step in a non-login shell (`bash --noprofile --norc`) that never reads the `~/.profile` line rustup adds, so the successfully-installed `codspeed` was invisible to the action's runner step → `exit 127`. Fixed **in the workflow** (the only durable, VPS-independent piece): a step before the action does `echo "$HOME/.cargo/bin" >> "$GITHUB_PATH"`.

**Why this and not alternatives.** git-rev is a durable fix, not just a retry: on the *persistent* runner cargo records the rev in `~/.cargo/.crates2.json`, so after the one-time 5-minute compile every later dispatch sees "already installed" and skips reinstall **with zero network** — the ~3-minute green run above did exactly that. Rejected: (a) a **prebuilt-binary cache** manually placed in `~/.cargo/bin` — redundant with cargo's own self-cache and would *re-trigger* the untracked-collision guard from step 2; (b) a **retry wrapper** around the codspeed.io curl — can't wrap the action's internal download from outside without forking it, and moot once that host leaves the code path entirely; (c) `--max-semi-space-size` (unrelated — that was a GC-hiding idea for the batching problem). Cold-cache risk (a `~/.cargo` wipe forces a recompile needing crates.io) is real but rare on a persistent runner and self-healing; warm PR runs under Phase 2 touch no network at all. Bump the pinned rev in lockstep with any `CodSpeedHQ/action` major upgrade.

**Persistent-runner speedups (same day): ~3 min → 2m32s.** Two ephemeral-runner defaults were pure overhead on the dedicated VPS and were opted out per-workflow (measured step timings before → after):

1. `actions/checkout` defaults to `git clean -ffdx`, wiping the gitignored `node_modules` every run and forcing `pnpm install --frozen-lockfile` to re-link all 30 workspace packages. `clean: false` in `codspeed.yml` keeps them (checkout still fetches + hard-resets tracked files); install becomes a ~1 s no-op. Safe for this job specifically: the suite runs from `src` via `tsx --conditions=@real-router/internal-source` (no vitest, no dist build), so stale artifacts can't perturb resolution or the counts.
2. With install a no-op, the remaining Setup cost turned out to be `actions/setup-node`'s `cache: pnpm` **re-downloading a ~200 MB pnpm-store tarball from GitHub's cache backend every run** (~20 s at ~13 MB/s) and unpacking it over an identical local store — the restore exists for ephemeral GitHub-hosted runners. The shared setup composite gained an optional `node-cache` input (default `"pnpm"`, so every other workflow is byte-identical); `codspeed.yml` passes `""` to skip the restore/save pair. Setup: 36 s → 30 s (clean:false) → **8 s** (no cache restore).

The **~2m06s callgrind bench step is the deliberate floor** — that is the batched measurement mass that buys the 0/56 phantom-freeness (reducing K re-opens the GC lottery; parallelizing under callgrind would corrupt instruction counts). Auto-comparisons across all these workflow-only commits stayed 0/56 (impact within ±0.003 %), extending the same-code verification chain.
