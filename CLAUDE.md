...
- Runtime validation is opt-in via `@real-router/validation-plugin` — core ships with structural guards and five invariant guards only (subscribe, navigateToNotFound, start, claimContextNamespace, channel guard #1572); separately, an always-on **mode gate** (#1575) normalises rather than detects — a query key the active `queryParamsMode` will not print never enters `state.search`, so `state.search` ⊆ what `state.path` shows, in every mode
