...
- Runtime validation is opt-in via `@real-router/validation-plugin` — core ships with structural guards and six invariant guards only (subscribe, navigateToNotFound, start, claimContextNamespace, channel guard #1572, unsafe-key guard #1792 — an own `__proto__` in a caller's bag, refused at the four producers that build from one plus the `forwardState` seam, and dropped at the wire entry instead because `match()` must never throw on a URL); separately, an always-on **mode gate** (#1575) normalises rather than detects — a query key the active `queryParamsMode` will not print never enters `state.search`, so `state.search` ⊆ what `state.path` shows, in every mode
