...
| --------------- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Node.js         | 24                                               | **`.nvmrc`** — single source of truth, with no exceptions: every workflow reads it via `node-version-file`. Minor floats UPWARD only: `jsdom` 30 declares `^22.22.2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |     | ^24.15.0 |     | >=26.0.0`, so **24.15 is a floor** on the 24 line. Nothing enforces it — there is no root `engines`, `engineStrict`is unset, and the pre-commit check below compares the MAJOR — so a host on 24.0–24.14 installs a jsdom that does not support it and hears only a pnpm warning.`.husky/pre-commit`warns when the local major differs. No root`engines`— deliberately, see IMPLEMENTATION_NOTES. Do NOT dev on another major: host globals differ (Node ≥ 26 ships Web Storage unflagged and shadows jsdom's`sessionStorage`under vitest — IMPLEMENTATION_NOTES "Adapter scroll tests mock`sessionStorage`"). On macOS/Homebrew use `node@24`+`brew pin node`; the unversioned `node` formula tracks the latest major and will silently upgrade you |
...
| pnpm            | 11.26                                            | `packageManager` field, root `package.json`; behavioral config in `pnpm-workspace.yaml` (overrides/allowBuilds/settings — pnpm 11 no longer reads `.npmrc`/`package.json#pnpm`). Within a major, bumping the field is enough — `pnpm/action-setup` call-sites auto-detect from it and pnpm self-switches locally — plus one `pnpm dedupe`, since a minor can still re-key peer variants. ⚠ **Across a major it is NOT enough**: `action-setup` must itself support the new pnpm major (pnpm 12 needs `@v6.1.0`, and the floating `v6` tag does not point there), so the action pin moves FIRST. Derive the `+sha512.<hex>` by decoding `pnpm view pnpm@<v> dist.integrity`, never by pasting the base64 SRI |
| TypeScript      | 6.0                                              | root `devDependencies` (pinned exact, `save-exact`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Vitest          | 4.1                                              | root `devDependencies`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| tsdown          | 0.23                                             | root `devDependencies`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Turbo           | 2.10                                             | root `devDependencies`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ESLint          | 10.7                                             | root `devDependencies` (flat config)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| @changesets/cli | 3.0                                              | root `devDependencies`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
**Grouped Dependabot PRs carry what `/bump-dep` cannot reach.** The npm entry in `.github/dependabot.yml` is a single `directory: "/"`, and pnpm workspace resolution carries it into every member — so Dependabot, not `/bump-dep`, owns the majority of this repo's dependency surface (`/bump-dep` refuses anything absent from the ROOT manifest; the split and the command that re-derives it live in IMPLEMENTATION_NOTES "Most of the dependency surface is outside `/bump-dep`'s root gate"). Delivery works. What a grouped PR cannot do is **analyse**, so four things a reviewer checks by hand because the diff does not show them:

- ⚠ **A grouped bump can move a dependency BACKWARDS.** Dependabot opens against the base it branched from, so anything `master` has moved past since comes back as a downgrade — #2166 proposed `tinybench` 6.1.4 while `master` was on 6.1.6. Compare every row against the current root manifest before merging.
- ⚠ **`playwright` needs an out-of-band re-seed.** The pinned Chromium is pre-provisioned on the self-hosted runner ([benchmarks/CLAUDE.md](benchmarks/CLAUDE.md): _"Re-seed on a playwright bump"_), and neither the PR nor CI says so.
- ⚠ **Four vite plugins build the CodSpeed bundles.** `@analogjs/vite-plugin-angular`, `@sveltejs/vite-plugin-svelte`, `@vitejs/plugin-react` and `vite-plugin-solid` are imported by `benchmarks/adapter-bench/vite/*.mts`, so bumping them can move benchmark numbers without touching a line of measured code.
- ⚠ **The `packages/` build tooling ships.** `ng-packagr`, `rollup`, `rollup-plugin-dts` and `@sveltejs/vite-plugin-svelte` produce published artifacts, and #2155 measured how long a defect there survives when nothing looks.
...
- A peer-dependent takes a **`patch`**, and only when the new version leaves its declared range. Changesets 3 has no "peer bump ⇒ major" rule and no `onlyUpdatePeerDependentsWhenOutOfRange` flag (changesets#2090); `.changeset/config.json` carries neither. ⚠ The count of internal peers is deliberately not repeated here — `grep -l '@real-router/core' packages/*/package.json` and read their `peerDependencies`. Each declares `workspace:>=0.1.0`, a range a core release cannot leave, so a core bump reaches them through `dependencies` or not at all
