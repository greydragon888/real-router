...
export function getRouteFromEvent(
  ...
  if (raw !== null && typeof raw === "object") {
    ...
    try {
      snapshot = {
        name: entry.name,
        // ⚠ SHAPE-PRESERVING, which is why this is `adoptChannel` and not a
        // spread. `isStateStrict` screens both bags by VALUE, so the guard walks
        // the caller's object and `makeState` walks it again; a key inside
        // either could answer one thing to the verdict and another to the
        // commit. An unconditional copy would also turn every shape the guard
        // exists to refuse into an acceptable one — the laundering the
        // registration walk met in #2139. `restore-nested-read-once-2141` owns
        // the row per refused shape.
        params: adoptChannel(entry.params as Record<string, unknown>),
        search: adoptChannel(entry.search as Record<string, unknown>),
        path: entry.path,
      };
    } catch {
      snapshot = undefined;
