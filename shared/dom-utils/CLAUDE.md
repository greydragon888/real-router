...
| `navigateWithHash(router, name, params, search?, hash, opts?)` | все `Link` компоненты в click handler                                                                                                                                                                      | frozen (**query-канал `search` на позиции 4 — RFC-4 M2 / #1548**; hash сдвинут на 5, opts на 6; `search` → `router.navigate` slot 3; same-route hash-bypass с #1555 **делегирует** вопрос «та же ли это локация?» в `router.isActiveRoute(name, params, search, true, false)` вместо самодельного `shallowEqual` — предикат несёт и канальное правило, и provenance-толерантное сравнение #1554, поэтому `<Link routeSearch={{page:"2"}}>` совпадает с состоянием, распарсенным из `?page=2` в ЧИСЛО. Следствие для тестов: хелперу нужен `isActiveRoute` в контракте — фейковые роутеры обязаны его отдавать. Ранее #532)                                                                                       |
| `resolveLinkTarget(to, routeName, routeParams?, routeSearch?)` | все `Link` компоненты + Angular directive                                                                                                                                                                  | frozen (**RFC-4 M2 B2 / #1548** — коллапсирует две формы пропсов `<Link>` в один триплет `{ name, params, search }`: дескриптор `to={NavigationTarget}` **побеждает** канальные пропсы, `console.warn` при конфликте. Единый рантайм-бэкстоп взаимоисключения для всех 6 адаптеров; react/preact/solid дополнительно enforce'ят union в типе, svelte/vue/angular — только рантайм)                                                                                                                                                                                                                                                                                                                               |
