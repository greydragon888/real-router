...
2. **Правильная headline-метрика на сценарий — НЕ первая попавшаяся** (карта deep-analysis §2.3):
   ...
   - alloc (GC-pressure): **`allocKBPerNav`** (gross, `includeObjectsCollectedByGC` #1417). table-heap/nav-churn/cold-start → retained (force-GC) `jsHeapMB`.
