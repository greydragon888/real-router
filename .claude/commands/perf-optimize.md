...
- **Сьют вообще выполнился?** «Skipped» у CodSpeed значит «в этом прогоне их не было», а НЕ «не затронуто»: core-сьют падал всю ветку на непроведённой фикстуре, отчёт печатал «122 skipped … baseline results were used instead», и несколько ходов ушло на разбор адаптерных чисел, пока по ядру данных не было вообще. Проверяй `conclusion` воркфлоу (`gh run list --workflow codspeed.yml`), а не только комментарий на PR. Заодно: в отчёте одного сьюта весь ДРУГОЙ сьют всегда «пропущен» (два отдельных воркфлоу), плюс осиротевшие URI от переименованных бенчей — само число «skipped» ничего не говорит.
- **База снята на ТЕКУЩЕЙ среде?** Спор «это среда или код?» закрывается не рассуждением, а **A/A-контролем**: прогнать baseline-коммит СЕГОДНЯ, на том же раннере (`gh workflow run codspeed.yml --ref master`). На #1589 база была 8-дневной, за это время на self-hosted сменились build_id `libc`/`ld-linux`, и CodSpeed помечал этим все 59 бенчей — один dispatch (~2.5 мин) дал 58/59 неподвижных и снял вопрос целиком. Дельта против базы, снятой в другой среде, не является измерением.
