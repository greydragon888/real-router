...
| back-forward | **`totalMs`** (script + Blink) + `allocKBPerNav` | popstate path (browser back/forward) — НЕ pushState-вперёд. blink-**DOMINATED** (~1.4 ms = ~5× forward, тяжёлый history-traversal) → **менее sub-ms-хрупкий**, ранг стабильнее forward per-nav. **post-#1353** rr фаerr **1 `updateForSameDocumentNavigation`/нав** (skip no-op popstate replaceState, #1353 MERGED bd26591a; было 2). Пост-фикс: rr WIN react+vue, near-tie solid+svelte, #2 angular (+44%, вдвое уже). Blink-dominated → менее sub-ms-хрупкий |
...
- **Методологические caveat'ы (проговаривай, иначе читатель обманется):** `script`-ratio завышает total; nav-churn читать heap+CPU не navsPerSec; sub-ms абсолюты кросс-сессионно ненадёжны; `_baseline` = floor «router overhead over bare framework» для cold-start/heap/link-build, но НЕ для per-nav script (naive baseline — ручной `pushState` + full re-render — ТЯЖЕЛЕЕ реактивных роутеров в vue/svelte/angular, потому #1326 убрал per-nav из over-baseline таблиц; «конкурент ниже baseline» — свойство naive baseline, НЕ аномалия).
...
- **Stale-блёрбы:** блёрб-числа/ранги расходятся с авто-таблицей (пример: svelte search-param блёрб «sv wins @50», а rr выигрывает; nav-latency блёрб «0.52 ms», results 0.84) → сверить каждое несущее блёрб-число с `results/`, поправить `report.mjs` + регенерить. **НО сверяй с ЧИСТОЙ n=30 ячейкой:** если сама ячейка загрязнена (n<30 из Фазы 0 — A/B/смоук перезаписал её) — staleness на ней **недоказуема** (и блёрб старый, и results кривой), бери другую когорту/сценарий с чистым n=30. Осторожно: блёрбы часто рефрешат ЧАСТИЧНО (одни сценарии обновлены под свежий n, другие отстали) — проверяй КАЖДЫЙ сценарий, не экстраполируй с одного.
  - **⚠️ КЛАСС МЕТРИКИ решает: sync абсолютов ИЛИ де-хардкод.** Прежде чем «синхронизировать блёрб к прогону», раздели: **(a) стабильные** (matcher `wide@1000` · память `table-heap`/`nav-churn heap` · `link-build` · `alloc` — big-signal, кросс-сессионный дрейф ~5%, нагрузка почти не бьёт) → **sync абсолютов ок**; **(b) sub-ms per-nav** (`nav-latency`/`param-nav`/`active-links`/`nested-switch` + vue-всё — крошечные числа) → **НЕ синхронизируй абсолютами**, они дрейфуют ~2× между сессиями И могут быть **load-инфлированы в самом прогоне**. Пиши sub-ms **отношениями/качественно** («rr ~1.3× floor конкурента, sub-ms, session/load-dependent»), а не хардкодом ms — иначе блёрб врёт каждый прогон. Подтверждено 07-05: `bench-cross-router.sh` (с readiness-gate!) выдал nav-latency rr 0.98, а same-session перемер тут же — 0.67 (прогон был загружен ~47%). **Диагноз sub-ms дрейфа/флипа — same-session перемер** (backup ячейки → `run.mjs <sc> <eng> <fw> 12` → сравни → restore): вернулось к старому → прогон был загружен, абсолют не запекать; ранг внутри когорты ratio-fair (движки подряд), но кросс-сессионный sub-ms флип (parity→#3) НЕ регрессия без same-session A/B (кода не менялось → механизма нет). **С #1261 этот перемер автоматизирован:** `bench-cross-router.sh` после каждой когорты гоняет `harness/sanity-remeasure.mjs` (nav-latency × real-router, малый n, БЕЗ записи в results/; флаг при |сдвиге| totalMs-медианы > 20%, оба направления) + межкогортный heavy-process recheck — флаги в summary прогона. Если когорта помечена «sub-ms sanity flagged» — её sub-ms абсолюты в results/ load-tainted: не освежай по ним блёрбы, стабильные классы целы.
