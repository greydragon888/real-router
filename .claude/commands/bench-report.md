...
- **⚠️ sweep `scriptMs@N` — ложная flat-кривая async (R2, #1451):** V8-only script слеп к microtask, поэтому async-движки (@solidjs/router/vue-router/tanstack) показывают почти-flat `scriptMs@N` (solid-router deep@90 0.035) при реальном `navMsTask@N` 0.838 (24× недосчёт) — если блёрб/анализ говорит «конкурент flat vs rr O(depth/N)» по script, это АРТЕФАКТ; смотри `navMsTask@N`. **Run-watch:** `navMsWall@N` в sweep single-nav ~100µs clamp-квантован → может дать высокий RME на малых размерах (@10/@3); если `rme-gate` фейлит на нём — РЕШАТЬ (noisy-класс vs сигнал), а `navMsTask@N` (не квантован) — надёжнее.
