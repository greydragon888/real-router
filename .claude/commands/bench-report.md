...
Каталог: `benchmarks/cross-router/`. Репорты **симметричны**: `REPORT-{react,vue,solid,svelte,angular}.md` (react нормализован в `REPORT-react.md` 2026-07-05 — раньше был `REPORT.md`). Сырьё: `results/<cohort>/<scenario>/<engine>.json` (`.metrics[key].{median,rme,p95,n}`).
...
**⚠️ МЕТРИКИ ПЕРЕИМЕНОВАНЫ (harness-аудит 07-12, #1451/#1452, landed master):** аддитивный **`totalMs`/`totalMs@N`/`totalMsPerNav` РЕТАЙРНУТ везде** — если видишь его в блёрбе/анализе, это стейл. Новый per-nav headline (все per-nav + sweep-сценарии) = **`navMsWall`** (perf.now click→DOM-settle, felt) + **`navMsTask`** (ΔTaskDuration twin), оба microtask-inclusive. `scriptDurationMs`/`scriptMs@N` низведены до **⚠-ДИАГНОСТИКИ** (V8-only, слепы к promise-microtask → систематически недосчитывают async-движки vue-router/@solidjs/router/tanstack на 22-41×; НЕ ранжируй по ним, особенно deep-config). link-build `scriptMs`→**`mountMs`** (wall). cold-start `jsHeapMB` теперь **retained (post-GC)** + `jsHeapPreGcMB` диаг. nav-churn `navsPerSec` теперь **REAL** (settle-timed). **В sweep `navMsTask@N` ЛИДИРУЕТ над `navMsWall@N`** (single-nav perf.now ~100µs clamp-квантован; per-nav суммирует N навов → там wall чист). ⚠ Пока не прошёл ре-baseline (#1450) — sweep-таблицы в results/ на СТАРЫХ ключах → рендерятся «—»; sweep competitive-числа недоступны до прогона.

| сценарий | headline-метрика | почему |
| --- | --- | --- |
| cold-start | `scriptDurationMs` (boot) + `jsHeapMB` **(retained, post-GC #1454)** | rr тяжелее грузится (#1106, eager core). jsHeapMB теперь **force-GC retained**; raw pre-GC = `jsHeapPreGcMB` (ДИАГ, boot-мусор 13-28% движко-зависим → angular retained-**паритет** 2.66=2.66, старое «rr тяжелее по heap» был garbage-differential) |
| nav-latency · param-nav · nested-switch · active-links | **`navMsWall`** (felt, click→DOM-settle) + **`navMsTask`** (ΔTaskDuration twin) | headline #1451/#1452: аддитивный `totalMs=script+blink` **РЕТАЙРНУТ** (blink был paced-inflated ×5-15, script microtask-blind). `scriptDurationMs` = ⚠-ДИАГ (V8-only, недосчитывает async-движки 22-41×), `blinkMs` = ДИАГ (gap=0). navMsWall чист (суммирует N навов → clamp усредняется) |
| back-forward | **`navMsWall`** + **`navMsTask`** + `allocKBPerNav` | popstate (browser back/fwd), НЕ pushState-вперёд. navMsWall ловит blink-history (rr `replaceState`-on-popstate было 2 события/нав → **post-#1353** 1×/нав, MERGED bd26591a). Пост-фикс: rr WIN react+vue, near-tie solid+svelte, #2 angular (+44%). `scriptDurationMs`/`blinkMs` = ДИАГ |
| wide-config | **`navMsTask@1000`** (`navMsWall@1000` = felt, но ~100µs clamp-noisy на sweep single-nav) | matcher-crossover: flat=trie, rising=O(N)-scan. `scriptMs@1000` = ⚠-ДИАГ (V8-blind, см. deep) |
| deep-config | **`navMsTask@90`** | tanstack-solid **N/A** (error 60+); rr solid #1094 RouteView. **⚠ НЕ ранжируй по `scriptMs@90`** (R2): V8-blind давал ЛОЖНУЮ flat-кривую async — solid-router script 0.035 «flat» vs navMsTask 0.838 = **24× недосчёт**, честный task ставит solid-router ВЫШЕ rr |
| search-param-scaling | **`navMsTask@50`** + **наклон** | flat=eager immutable · rising=lazy-reactive · tanstack взрывается O(count). `scriptMs@50` = ⚠-ДИАГ (V8-blind) |
| table-heap | `jsHeapMB@10000` | память route-таблицы (force-GC retained, стабильна RME<0.01%) |
| nav-churn | **`heapDeltaKB`** (retained/leak) + **`navMsWall`** (CPU/nav) | **navsPerSec БОЛЬШЕ НЕ «confounded»** — post-#1451 он REAL (settle-timed, НЕ frame-capped ~121), `navMsWall = 1000/navsPerSec` по построению. Читать heapDeltaKB (leak) + navMsWall (CPU) |
| link-build | **`mountMs`** (wall-clock, mount 1000 links) | rr WIN vs компонент-конкурентов, LAST vs plain-`<a>` (svelte). Было `scriptMs` → **`mountMs`** (#1418 un-blinding: script V8-only недооценивал mount-стоимость) |
| `allocKBPerNav` (nav-lat/param-nav/search-param/back-forward) | KB/nav | GC-pressure = transient-аллокация/nav; memory-counterpart к CPU. **Показывай ОБЕ строки: nav-lat (общий per-nav floor) + search@50 (eager-vs-lazy ПИК — там виден налог validate-pipeline'ов: tanstack ~5–11× rr, когорта-зависимо). alloc-ранг МОЖЕТ расходиться с CPU-рангом (vue: CPU 🔴 но alloc 🟢 — eager платит CPU-floor, НЕ memory; ссылки, не копии) — это ГЛАВНЫЙ инсайт оси, НЕ сворачивай в одну строку.** Частична до полного прогона; пустые ячейки ≠ аномалия |

## Фаза 0 — Ground truth (сначала `results/`, потом блёрбы)

1. **Полнота + n:** сколько ячеек, какой `runs`. Ожидаемое множество ячеек считай от `SCENARIOS` в `run-all.mjs`, НЕ от `ls results/` — gitignored-каталог копит сирот удалённых сценариев (пример: `results/*/param-scaling/` живёт с 07-04 после удаления сценария; выдаёт себя старым mtime и меньшим `runs`). Дыры = failed-cell ИЛИ documented `KNOWN_NA` (не путать — Фаза 4).
2. **RME-gate:** `node harness/rme-gate.mjs 15 40 [cohort]` — stable ≤15% · noisy(blink/latency/fcp) ≤40%. PASS/offenders.
3. **Ранжируй rr** по headline-метрике (карта выше) для каждого (сценарий × когорта): median + ранг + margin к лидеру. `node harness/status-tables.mjs <cohort>` даёт быстрый flat-view — зови ПО-КОГОРТНО (без аргумента выводит не все когорты), и СВЕРЬ метрику (он может брать не тот ключ).
...
- **Методологические caveat'ы (проговаривай, иначе читатель обманется):** `scriptDurationMs`/`scriptMs@N` = ⚠-диагностика (V8-only, слеп к microtask → недосчёт async-движков 22-41×) — ранжируй по `navMsWall`/`navMsTask`, НЕ по script (особенно deep-config, где script даёт ложную flat-кривую async); nav-churn читать heap+`navMsWall` (navsPerSec теперь real, но CPU/leak — headline); sub-ms абсолюты кросс-сессионно ненадёжны; `_baseline` = floor «router overhead over bare framework» для cold-start/heap/link-build, но НЕ для per-nav (naive baseline — ручной `pushState` + full re-render — ТЯЖЕЛЕЕ реактивных роутеров в vue/svelte/angular, потому #1326 убрал per-nav из over-baseline таблиц; «конкурент ниже baseline» — свойство naive baseline, НЕ аномалия).
...
- **Regression-режим (прогон ПОСЛЕ волны мержей — «сдвинула ли перф?»):** частый реальный вопрос, отдельный от same-session A/B одной оптимизации. Сравнивай **СТАБИЛЬНЫЕ** метрики (`table-heap@10k` · `wide@1000` · `cold-start` · `link-build` · `alloc` — big-signal, cross-session сравнимы, дрейф ~5%) свежего `results/` со snapshot прошлого прогона: **committed REPORT ДО `--refresh`** — но сначала выясни, ГДЕ лежит последний refresh: `git log --all -1 -- benchmarks/cross-router/REPORT-react.md` (HEAD может быть на ветке/резете БЕЗ него — тогда `git show HEAD:REPORT-*` подсунет прогон N−1 под видом N; сверь дату в `**Run:**`-заголовке с mtime `results/`). Свежие блёрбы бери из ref последнего refresh (`git show <ref>:…REPORT-<fw>.md`), а более старая worktree/HEAD-версия — готовый снапшот N−1 для сравнения. Регенерация затирает старые числа, так что сними их ПЕРЕД refresh. Сдвиг **>~10% на СТАБИЛЬНОЙ метрике = реальная регрессия/выигрыш** (копай: какой мерж). **sub-ms (nav-latency/param-nav/active-links/nested/vue-всё) cross-session НЕ сравнивай** — floor-bound + load/термал-дрейф топят сигнал; их сдвиг между прогонами ≠ регрессия. Correctness-волна (path-matcher/reachability/codec-фиксы) → **ждёшь 0 сдвига**, и это сам по себе ценный вывод («N мержей, 0 перф-регрессий — мержить было безопасно»), не «нечего сказать».
...
- **Известные артефакты (НЕ баг):** nav-churn `navsPerSec` — **post-#1451 БОЛЬШЕ НЕ frame-capped** (settle заменил rAF-poll → real throughput, `navMsWall=1000/navsPerSec` по построению); старое «rAF-capped ~121 vs solid rr ~15k/s» УСТАРЕЛО. Читать nav-churn по `heapDeltaKB` (leak) + `navMsWall` (CPU). Проверяй RME: стабильный outlier (rme<1%) = артефакт метрики, не глюк.
- **⚠️ sweep `scriptMs@N` — ложная flat-кривая async (R2, #1451):** V8-only script слеп к microtask, поэтому async-движки (@solidjs/router/vue-router/tanstack) показывают почти-flat `scriptMs@N` (solid-router deep@90 0.035) при реальном `navMsTask@N` 0.838 (24× недосчёт) — если блёрб/анализ говорит «конкурент flat vs rr O(depth/N)» по script, это АРТЕФАКТ; смотри `navMsTask@N`. **Clamp-квантование navMsWall@N (РЕШЕНО):** single-nav `perf.now` ~100µs clamp → высокий RME на быстрых точках (rr wide@1000≈0.1ms=1 ступень) по КВАНТОВАНИЮ, не нестабильности. Фикс: `navMsWall@N` эмитится **ТОЛЬКО на endpoint** (совпадает с report-строкой) + `rme-gate` классит `/^navMsWall@/` **noisy(40%)** (per-nav `navMsWall` без `@` = stable; `navMsTask@N` unclamped = stable-гейт, headline). Ранжируй по `navMsTask@N`, НЕ navMsWall@N.
...
- **Stale-блёрбы:** блёрб-числа/ранги расходятся с авто-таблицей (пример: svelte search-param блёрб «sv wins @50», а rr выигрывает; nav-latency блёрб «0.52 ms», results 0.84) → сверить каждое несущее блёрб-число с `results/`, поправить `report.mjs` + регенерить. **НО сверяй с ЧИСТОЙ n=30 ячейкой:** если сама ячейка загрязнена (n<30 из Фазы 0 — A/B/смоук перезаписал её) — staleness на ней **недоказуема** (и блёрб старый, и results кривой), бери другую когорту/сценарий с чистым n=30. Осторожно: блёрбы часто рефрешат ЧАСТИЧНО (одни сценарии обновлены под свежий n, другие отстали) — проверяй КАЖДЫЙ сценарий, не экстраполируй с одного.
  - **⚠️ КЛАСС МЕТРИКИ решает: sync абсолютов ИЛИ де-хардкод.** Прежде чем «синхронизировать блёрб к прогону», раздели: **(a) стабильные** (matcher `wide@1000` · память `table-heap`/`nav-churn heap` · `link-build` · `alloc` — big-signal, кросс-сессионный дрейф ~5%, нагрузка почти не бьёт) → **sync абсолютов ок**; **(b) sub-ms per-nav** (`nav-latency`/`param-nav`/`active-links`/`nested-switch` + vue-всё — крошечные числа) → **НЕ синхронизируй абсолютами**, они дрейфуют ~2× между сессиями И могут быть **load-инфлированы в самом прогоне**. Пиши sub-ms **отношениями/качественно** («rr ~1.3× floor конкурента, sub-ms, session/load-dependent»), а не хардкодом ms — иначе блёрб врёт каждый прогон. Подтверждено 07-05: `bench-cross-router.sh` (с readiness-gate!) выдал nav-latency rr 0.98, а same-session перемер тут же — 0.67 (прогон был загружен ~47%). **Диагноз sub-ms дрейфа/флипа — same-session перемер** (backup ячейки → `run.mjs <sc> <eng> <fw> 12` → сравни → restore): вернулось к старому → прогон был загружен, абсолют не запекать; ранг внутри когорты ratio-fair (движки подряд), но кросс-сессионный sub-ms флип (parity→#3) НЕ регрессия без same-session A/B (кода не менялось → механизма нет). **С #1261 этот перемер автоматизирован:** `bench-cross-router.sh` после каждой когорты гоняет `harness/sanity-remeasure.mjs` (nav-latency × real-router, малый n, БЕЗ записи в results/; флаг при |сдвиге| `navMsWall`-медианы > 20%, оба направления; R1 07-12: раньше ключ был `totalMs`, ретайрнут → guard молча дох на exit-2, пока не переключили на navMsWall) + межкогортный heavy-process recheck — флаги в summary прогона. Если когорта помечена «sub-ms sanity flagged» — её sub-ms абсолюты в results/ load-tainted: не освежай по ним блёрбы, стабильные классы целы.
...
- Ранжируй по ПРАВИЛЬНОЙ headline-метрике (карта: per-nav + sweep = `navMsWall`/`navMsTask@N`, НЕ `scriptMs`/`totalMs`) + правильному endpoint (`@1000`/`@90`/`@50`/`@10k`). `scriptMs`/`scriptMs@N` = ⚠-диаг (V8-blind, недосчитывает async → ложная flat-кривая deep-config); nav-churn — `heapDeltaKB`+`navMsWall`.
- Sub-ms «улучшилось?» — ТОЛЬКО same-session A/B. Кросс-сессионный абсолют = apples/oranges (audit «hardware-bound»).
- Не объявляй ячейку «сломанной», не отсеяв known-артефакт (navsPerSec теперь real/settle-timed; sweep `scriptMs@N` ложно-flat для async-движков) и `KNOWN_NA` (competitor-limit ИЛИ semantics-inexpressible — mateo nested-switch, #1456).
- Perf + capability вместе: не выдавай «конкурент X легче» без «потому что делает меньше».
- репорты симметричны: `REPORT-<fw>.md` для КАЖДОЙ когорты, react = `REPORT-react.md` (переименован из `REPORT.md` 2026-07-05).
