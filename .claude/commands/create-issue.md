...
## Шаг 0 — Триаж: нужен ли issue вообще? (гейт самокритики)

**Прежде чем что-либо оформлять, отнесись к запросу критически — формулировка пользователя это гипотеза, а не истина.** Это гейт-«закоротка»: он может прекратить работу ДО создания issue. Не заводи тикет «по тексту».

- **Баг?** Подтверди, что это баг, а не ожидаемое поведение или неверное использование — по возможности эмпирически: загляни в код (`packages/<pkg>/src/…`), тесты, доки/вики. Если на текущем HEAD не воспроизводится или поведение задокументировано как намеренное — **остановись, сообщи находку и спроси**, а не фиксируй «как сказано».
- **Фича / задача?** Проверь, не покрыто ли уже публичным API (`extendRouter` / `addInterceptor` / `claimContextNamespace` / существующие плагины) и не противоречит ли границам слоёв и архитектуре. Нет реального gap (нет user-driven запроса, есть обходной путь, нет аналога у standalone-конкурентов) → **не плоди issue** — это шум в трекере.
- **Предложенное пользователем решение** (если есть) — тоже гипотеза: не вшивай его в issue как данность. Сначала сам найди корень, затем оцени предложение; если твоё точнее — опиши его, а исходное упомяни как рассмотренную альтернативу с причиной отказа.

Триаж не прошёл — остановись и сообщи, не создавай issue впустую.

## Шаг 1 — Классифицируй тип
...
## Шаг 2 — Определи area и priority

**area** (→ label, см. Шаг 6) — какой пакет/слой затронут: core, react, vue, svelte, solid, plugins, ci/cd, build, npm, tests, benchmarks.
...
## Шаг 3 — Проверь дубликаты (гейт-«закоротка», ОБЯЗАТЕЛЬНО до создания)
...
gh search issues --repo greydragon888/real-router "<keywords>" --state all --limit 20
```

- Проверяй **и closed тоже**: закрытый как `wontfix`/`duplicate` означает, что вопрос уже решён — не воскрешай его молча.
- Нашёл вероятный дубль/родственника → **покажи его (номер, заголовок, state) и СПРОСИ**, прежде чем создавать. Часто верный ход — не новый issue, а комментарий к существующему или ссылка `parallel to #NNN` / `follow-up of #NNN` в теле нового (так делают реальные issue репо). Как и Шаг 0, этот гейт может закоротить работу в «комментарий вместо тикета».

## Шаг 4 — Собери заголовок (реальная конвенция)
...
## Шаг 5 — Собери тело по структуре шаблона
...
## Шаг 6 — Подбери labels (сверяйся с реальным списком!)
...
## Шаг 7 — Свежий взгляд перед публикацией (гейт самокритики)

Issue собран — вернись к нему **свежим взглядом, как ревьюер чужого тикета**, ПРЕЖДЕ чем показывать превью:

- **Заголовок** минимально-конкретен: симптом → следствие, без воды; понятен из списка `gh issue list` без открытия.
- **Тело самодостаточно**: repro/контекст/ссылки на код на месте; человек со стороны поймёт без переспроса.
- **Скоуп не раздут (YAGNI)**: один issue — одна проблема; разнородное разнеси на родственные тикеты с `parallel to #NNN`.
- **Нет переобещаний**: не заявляй статус/решение/«сделаю», если это не подтверждено.
- **Дедуп ещё валиден**: если по ходу сборки всплыли новые ключевые слова/символы — догони дедуп (Шаг 3) по ним; родственник мог не найтись по исходным словам.

Нашёл изъян — поправь здесь, не показывай сырое на Шаге 8.
...
- [ ] Триаж (Шаг 0) пройден: проблема реальна (баг ≠ expected, фича ≠ уже покрыто), не шум
...
- [ ] Прошёл свежим взглядом (Шаг 7): скоуп не раздут, нет переобещаний, дедуп догнан по новым словам
