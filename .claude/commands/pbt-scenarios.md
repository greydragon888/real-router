...
- **Stateful — command-sequence.** Генерируй не значения, а последовательности (`start→navigate(from)→navigate(to)`), и **параметризуй маршруты** (route с `/:id`), иначе param-сравнивающий код мёртв и мутации в нём невидимы. **Ассерт-скоуп, когда шаг сам пере-триггерит наблюдаемое:** если cancel/mutate-шаг последовательности — сам операция, ПЕРЕ-фаерящая наблюдаемый callback (супер-седящий `navigate` пере-фаерит те же leave-listeners для СВОЕЙ фазы), общий буфер (captured signals / порядок / счётчик) копится по ВСЕМ шагам — ассерть на СЛАЙС целевой операции (`signals.slice(0, n)`), не на total (иначе `toHaveLength(n)` ложно падает на 2n — стоило итерации пробы на item-7).
...
3. `git diff` затрагивает ТОЛЬКО `*.properties.ts` (+ при нужде `INVARIANTS.md`/`helpers.ts`) — никаких `src/` (кроме отдельно-веденного реального фикса с changeset). **Точечный `eslint --fix` + `prettier --write` по изменённым `*.properties.ts` ПЕРЕД финалом** — новый property-файл почти всегда триггерит `@stylistic/padding-line-between-statements` + `prettier/prettier` + `vitest/padding-*` (тесты лендятся под тем же lint-гейтом, что src); после `--fix` **перепрогони файл** (fix мог переписать `if→switch` через `unicorn/prefer-switch` — прогон доказывает, что логика цела).
...
3. **Разовое ≠ паттерн.** Специфику конкретного пакета (разовую) → в отчёт/`INVARIANTS.md`/память, НЕ в скил. В скил идёт только повторяющийся класс, полезный СЛЕДУЮЩЕМУ прогону на ДРУГОМ пакете.
