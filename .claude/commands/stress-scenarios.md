...
- **Arg-limit спреда:** `arr.push(...big)` / `fn.apply(null, big)` / `Math.max(...big)` бросают `RangeError: Maximum call stack size exceeded` на достаточно широком входе. Любой spread от данных неизвестного размера — кандидат. ⚠ **Порог НЕ константа — это остаток стека на момент вызова, поэтому мерь его на СВОЁМ сайте, а не бери из соседнего пина.** `get-static-paths-scale` записывает ~124k для своего сайта; на `vue/RouteView` `collectElements` замерено между 400k и 500k, с дрейфом между прогонами на одной машине и одном Node 24. Ширина, взятая из чужого пина, даёт ЛОЖНО-ЗЕЛЁНЫЙ тест: он проходит и на сломанном коде. (Находки: `getStaticPaths` #920, `collectElements` #2203.)
...
5. **Предлагай, не применяй молча.** Выведи кандидатов таблицей (раздел | инцидент | предлагаемый текст), спроси разрешения. Применил по согласию → `.claude/commands/` это infra: changeset не нужен, PR нужен (апрувов ноль, CLAUDE.md).
