...
// Wait for the framework to COMMIT the initial render before tearing down.
// Mounting before commit leaves subscribe/cleanup half-run and the router
// transiently uncollectable — a benchmark-harness race, not a router leak
// (TanStack's setup waits onRendered).
...
export function setup(appModulePath = "./dist/app.mjs") {
  ...
  async function tick() {
    ...
    // Wait for the initial commit before tearing down (see waitForRender).
