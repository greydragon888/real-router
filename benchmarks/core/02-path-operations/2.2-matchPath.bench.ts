// benchmarks/core/02-path-operations/2.2-matchPath.bench.ts

import { bench, do_not_optimize } from "mitata";

import { createRouter, IS_REAL_ROUTER, getPluginApi } from "../helpers";

import type { Route } from "../helpers";

/**
 * Unified matchPath — router5/router6 have it on the instance,
 * real-router exposes it via getPluginApi().
 */
function createMatchPath(
  router: ReturnType<typeof createRouter>,
): (path: string) => unknown {
  if (IS_REAL_ROUTER) {
    const api = getPluginApi!(router);

    return (path: string) => api.matchPath(path);
  }

  // router5/router6: direct method
  return (path: string) =>
    (router as unknown as { matchPath: (p: string) => unknown }).matchPath(
      path,
    );
}

/**
 * Batch size for stable measurements.
 * matchPath is slower than buildPath, so smaller batch is sufficient.
 */
const BATCH = 50;

const routes: Route[] = [
  { name: "home", path: "/" },
  { name: "about", path: "/about" },
  { name: "user", path: "/users/:id" },
  {
    name: "users",
    path: "/users",
    // real-router: allows any child name (user.profile works even though parent is "users")
    // router5: requires relative child names ("profile"), automatically builds full name "users.profile"
    children: [
      {
        name: "profile",
        path: "/:id/profile",
      },
    ],
  },
];

// 2.2.1 Matching simple path
{
  const router = createRouter(routes);
  const matchPath = createMatchPath(router);

  bench(`2.2.1 Matching simple path (×${BATCH})`, () => {
    for (let i = 0; i < BATCH; i++) {
      do_not_optimize(matchPath("/about"));
    }
  }).gc("inner");
}

// 2.2.2 Matching path with parameters
{
  const router = createRouter(routes);
  const matchPath = createMatchPath(router);

  bench(`2.2.2 Matching path with parameters (×${BATCH})`, () => {
    for (let i = 0; i < BATCH; i++) {
      do_not_optimize(matchPath("/users/123"));
    }
  }).gc("inner");
}

// 2.2.3 Matching path with query parameters
{
  const router = createRouter(routes);
  const matchPath = createMatchPath(router);

  bench(`2.2.3 Matching path with query parameters (×${BATCH})`, () => {
    for (let i = 0; i < BATCH; i++) {
      do_not_optimize(matchPath("/about?search=test&page=1"));
    }
  }).gc("inner");
}

// 2.2.4 Matching nested path
{
  const router = createRouter(routes);
  const matchPath = createMatchPath(router);

  bench(`2.2.4 Matching nested path (×${BATCH})`, () => {
    for (let i = 0; i < BATCH; i++) {
      do_not_optimize(matchPath("/users/123/profile"));
    }
  }).gc("inner");
}

// 2.2.5 Matching with parameter decoding
{
  const router = createRouter(routes, {
    urlParamsEncoding: "uriComponent",
  });
  const matchPath = createMatchPath(router);

  bench(`2.2.5 Matching with parameter decoding (×${BATCH})`, () => {
    for (let i = 0; i < BATCH; i++) {
      do_not_optimize(matchPath("/users/test%40example.com"));
    }
  }).gc("inner");
}

// 2.2.6 Matching with custom decoder
{
  const router = createRouter([
    {
      name: "custom",
      path: "/custom/:id",
      decodeParams: (params) => params,
    },
  ]);
  const matchPath = createMatchPath(router);

  bench(`2.2.6 Matching with custom decoder (×${BATCH})`, () => {
    for (let i = 0; i < BATCH; i++) {
      do_not_optimize(matchPath("/custom/123"));
    }
  }).gc("inner");
}

// 2.2.7 Matching with allowNotFound mode
{
  const router = createRouter(routes, {
    allowNotFound: true,
  });
  const matchPath = createMatchPath(router);

  bench(`2.2.7 Matching with allowNotFound mode (×${BATCH})`, () => {
    for (let i = 0; i < BATCH; i++) {
      do_not_optimize(matchPath("/nonexistent"));
    }
  }).gc("inner");
}

// 2.2.9 Matching with trailing slash
{
  const router = createRouter(routes, {
    trailingSlash: "always",
  });
  const matchPath = createMatchPath(router);

  bench(`2.2.9 Matching with trailing slash (×${BATCH})`, () => {
    for (let i = 0; i < BATCH; i++) {
      do_not_optimize(matchPath("/about/"));
    }
  }).gc("inner");
}
