 ...
 * Run: BENCH_ROUTER=real-router npx tsx core/isolated-anomalies.ts
...
async function main(): Promise<void> {
  ...
  // ── 2.1.1 buildPath simple (×100) ──
  console.log("── 2.1.1 buildPath simple ──");
  {
    ...
    await isolatedMeasure("2.1.1 buildPath simple (×1)", () => {
      do_not_optimize(router.buildPath("about"));
    });
  }

  // ── 2.1.2 buildPath with params (×100) ──
  console.log("\n── 2.1.2 buildPath with params ──");
  {
    const router = createRouter(routes);
    ...
    await isolatedMeasure("2.1.2 buildPath with params (×1)", () => {
      do_not_optimize(router.buildPath("user", { id: "123" }));
    });
  }

  // ── 2.1.3 buildPath nested (×100) ──
  console.log("\n── 2.1.3 buildPath nested ──");
  {
    const router = createRouter(routes);
    ...
    await isolatedMeasure("2.1.3 buildPath nested (×1)", () => {
      do_not_optimize(router.buildPath("users.profile", { id: "123" }));
    });
  }

  // ── 2.2.1 matchPath simple (×50) ──
  console.log("\n── 2.2.1 matchPath simple ──");
  {
    const router = createRouter(routes);
    ...
    await isolatedMeasure("2.2.1 matchPath simple (×1)", () => {
      do_not_optimize(matchPath("/about"));
    });
  }

  // ── 2.2.4 matchPath nested (×50) ──
  console.log("\n── 2.2.4 matchPath nested ──");
  {
    const router = createRouter(routes);
    const matchPath = getMatchPath(router);
    ...
    await isolatedMeasure("2.2.4 matchPath nested (×1)", () => {
      do_not_optimize(matchPath("/users/123/profile"));
    });
  }

  // ── 2.2.2 matchPath with parameters (×50) ──
  console.log("\n── 2.2.2 matchPath with parameters ──");
  {
    const router = createRouter(routes);
    const matchPath = getMatchPath(router);

    await isolatedMeasure(
      "2.2.2 matchPath with params (×50)",
      () => {
        for (let i = 0; i < 50; i++) {
    ...
    await isolatedMeasure("2.2.2 matchPath with params (×1)", () => {
      do_not_optimize(matchPath("/users/123"));
    });
  }

  // ── 2.2.3 matchPath with query parameters (×50) ──
  console.log("\n── 2.2.3 matchPath with query parameters ──");
  {
    const router = createRouter(routes);
    const matchPath = getMatchPath(router);

    await isolatedMeasure(
      "2.2.3 matchPath with query params (×50)",
      () => {
        for (let i = 0; i < 50; i++) {
    ...
    await isolatedMeasure("2.2.3 matchPath with query params (×1)", () => {
      do_not_optimize(matchPath("/about?search=test&page=1"));
    });
  }

  // ── 2.2.5 matchPath with parameter decoding (×50) ──
  console.log("\n── 2.2.5 matchPath with parameter decoding ──");
  {
    const router = createRouter(routes, {
    ...
    await isolatedMeasure(
      "2.2.5 matchPath with param decoding (×50)",
      () => {
        for (let i = 0; i < 50; i++) {
    ...
    await isolatedMeasure("2.2.5 matchPath with param decoding (×1)", () => {
      do_not_optimize(matchPath("/users/test%40example.com"));
    });
  }

  // ── 2.2.9 matchPath with trailing slash (×50) ──
  console.log("\n── 2.2.9 matchPath with trailing slash ──");
  {
    const router = createRouter(routes, {
    ...
    await isolatedMeasure("2.2.9 matchPath with trailing slash (×1)", () => {
      do_not_optimize(matchPath("/about/"));
    });
  }

  // ── 2.4.8 matchPath with duplicate query params (×50) ──
  console.log("\n── 2.4.8 matchPath with duplicate query params ──");
  {
    ...
    await isolatedMeasure(
      "2.4.8 matchPath duplicate query params (×50)",
      () => {
        for (let i = 0; i < 50; i++) {
    ...
    await isolatedMeasure("2.4.8 matchPath duplicate query params (×1)", () => {
      do_not_optimize(matchPath("/?tag=1&tag=2&tag=3"));
    });
  }

  // ── 2.4.12 matchPath without optional parameters (×50) ──
  console.log("\n── 2.4.12 matchPath without optional parameters ──");
  {
    const optionalRoutes: Route[] = [
      { name: "home", path: "/" },
      { name: "user", path: "/users/:id" },
    ...
    await isolatedMeasure(
      "2.4.12 matchPath without optional params (×50)",
      () => {
        for (let i = 0; i < 50; i++) {
    ...
    await isolatedMeasure(
      "2.4.12 matchPath without optional params (×1)",
      () => {
        do_not_optimize(matchPath("/articles"));
      },
    );
  ...
  // ── 3.1.1 areStatesEqual mixed (×100) ──
  console.log("\n── 3.1.1 areStatesEqual mixed ──");
  {
  ...
  // ── 3.5.4 areStatesEqual identical params different order (×100) ──
  console.log("\n── 3.5.4 areStatesEqual identical params ──");
  {
    const router = createSimpleRouter();
    ...
    await isolatedMeasure(
      "3.5.4 areStatesEqual identical params diff order (×100)",
      () => {
        for (let i = 0; i < 100; i++) {
  ...
  // ── 3.5.10 areStatesEqual null params (×100) ──
  console.log("\n── 3.5.10 areStatesEqual null params ──");
  {
    const router = createSimpleRouter();
    ...
    await isolatedMeasure(
      "3.5.10 areStatesEqual null vs undefined (×100)",
      () => {
        for (let i = 0; i < 100; i++) {
          do_not_optimize(router.areStatesEqual(state1, state2));
        }
  ...
  // ── 3.1.2 areStatesEqual with ignoreQueryParams (×100) ──
  console.log("\n── 3.1.2 areStatesEqual with ignoreQueryParams ──");
  {
    const router = createSimpleRouter();
    ...
    await isolatedMeasure(
      "3.1.2 areStatesEqual ignoreQueryParams (×100)",
      () => {
        for (let i = 0; i < 100; i++) {
    ...
    await isolatedMeasure("3.1.2 areStatesEqual ignoreQueryParams (×1)", () => {
      do_not_optimize(router.areStatesEqual(state1, state2, true));
    });
  }
