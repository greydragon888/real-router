// benchmarks/core/04-stress-testing/4.1-high-load-sequential.bench.ts
...
// 4.1.1 Thousand sequential navigations between two routes
{
  const router = createSimpleRouter();

  router.start("/");

  bench("4.1.1 Thousand sequential navigations between two routes", () => {
...
// 4.1.2 Ten thousand sequential navigations between five routes
{
  const router = createSimpleRouter();
...
// 4.1.4 Thousand navigations through deeply nested routes
// Note: createNestedRouter(5) creates routes: root, root.level1, ..., root.level1.level2.level3.level4.level5
{
  ...
  bench("4.1.4 Thousand navigations through deeply nested routes", () => {
    for (let i = 0; i < 1000; i++) {
...
// 4.1.5 Five thousand navigations with query parameters
{
  const router = createSimpleRouter();

  router.start("/");

  bench("4.1.5 Five thousand navigations with query parameters", () => {
...
// 4.1.6 Thousand navigate -> stop -> navigate cycles
{
  const router = createSimpleRouter();

  router.start("/");

  bench("4.1.6 Thousand navigate -> stop -> navigate cycles", () => {
    for (let i = 0; i < 1000; i++) {
...
// 4.1.7 Thousand navigations alternating reload flag
// Dynamic reload calculation prevents LICM optimization
{
  ...
  bench("4.1.7 Thousand navigations alternating reload flag", () => {
    for (let i = 0; i < 1000; i++) {
...
// 4.1.8 Five thousand navigations to random routes
{
  const router = createSimpleRouter();
  ...
  bench("4.1.8 Five thousand navigations to random routes", () => {
    for (let i = 0; i < 5000; i++) {
