// benchmarks/core/03-current-state/3.4-forward.bench.ts
...
// 3.4.1 Creating forward state
{
  const router = createSimpleRouter();
  const forwardState = getForwardState(router);

  bench("3.4.1 Creating forward state", () => {
    for (let i = 0; i < 100; i++) {
...
// 3.4.2 Forward state with parameters
{
  const router = createSimpleRouter();
  const forwardState = getForwardState(router);
  const params = { id: "123" };

  bench("3.4.2 Forward state with parameters", () => {
    for (let i = 0; i < 100; i++) {
