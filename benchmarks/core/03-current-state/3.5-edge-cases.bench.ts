// benchmarks/core/03-current-state/3.5-edge-cases.bench.ts
...
// 3.5.1 State with large number of parameters
{
  ...
  bench("3.5.1 State with large number of parameters", () => {
    for (let i = 0; i < 100; i++) {
...
// 3.5.4 Comparing states with identical parameters but different structure
{
  const router = createSimpleRouter();
  const makeState = getMakeState(router);
...
// 3.5.6 Building state for non-existent route
{
  const router = createSimpleRouter();
  const buildState = getBuildState(router);

  bench("3.5.6 Building state for non-existent route", () => {
    for (let i = 0; i < 100; i++) {
...
// 3.5.9 Sequential state ID generation
{
  const router = createSimpleRouter();
  const makeState = getMakeState(router);

  bench("3.5.9 Sequential state ID generation", () => {
    for (let i = 0; i < 10; i++) {
...
// 3.5.10 Comparing states with null parameters
{
  const router = createSimpleRouter();
  const makeState = getMakeState(router);
  ...
  bench("3.5.10 Comparing states with null parameters", () => {
    for (let i = 0; i < 100; i++) {
      do_not_optimize(router.areStatesEqual(state1, state2));
    }
  }).gc("inner");
