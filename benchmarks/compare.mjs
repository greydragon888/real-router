...
const RESULTS_DIR = join(__dirname, "core", ".bench-results");
const BENCH_JSON_DIR = join(__dirname, "core", ".bench");
...
function compareThreeBenchmarks(router5File, router6File, realRouterFile) {
  ...
  // Load RME data from JSON files
  const rmeData = loadRmeData();
  const router5RmeMap = rmeData.get("router5") || new Map();
  const router6RmeMap = rmeData.get("router6") || new Map();
  const realRouterRmeMap = rmeData.get("real-router") || new Map();
