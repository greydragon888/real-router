...
│   │   #  react cohort: real-router · react-router (v8 Data mode) · tanstack — wouter EXCLUDED (minimalist, different class; REPORT-react.md Scope) · _baseline = bare React floor · real-router-full = REFERENCE variant (base app + production plugin stack: browser+persistent-params+search-schema+ssr-data) quantifying the interceptor-depth measurement note — NOT in run-all/REPORT rosters (apples-to-oranges vs bare competitors); run manually: run.mjs {nav-latency,param-nav} real-router-full react 50
