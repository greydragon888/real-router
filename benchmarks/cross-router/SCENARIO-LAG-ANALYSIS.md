...
|---|---|---|---|---|---|
| all | cold-start | +8.5%…+123% (react +8.5%, vue +76%, solid 2.2×, svelte +52%, angular 2.9×; rr +3.4–4.4 ms over bare) | `EAGER-CORE` | code-traced (#1106) | `STRUCTURAL` / `v2-CANDIDATE` |
...
| svelte | nested-switch | ~~+42%~~ → @1 +13% (0.107 vs 0.095); **@≥8 rr WINS** (@32 1.8×: 0.113 vs 0.203) | `<Link>-COMPONENT` active-flip @shallow; sv O(depth) re-render @deep | **A/B-proven** (07-16; old row from retired sweep) | `FEATURE-COST` @1 → **rr WIN at depth** |
...
| svelte | table-heap | +19% @100 (2.18 vs 1.83; 2.6× @10k retired — SIZES=[100]) | `SCALE-FLOOR` (**98% route-tree+trie**, ~650 B/route) | **measured (heap-decomp)** | `STRUCTURAL` |
...
| angular | cold-start | 2.9× (6.18 vs 2.11) | `EAGER-CORE` | code-traced | `STRUCTURAL` — table-heap +70% was @10k-retired; **@100 rr WINS −3.7%** (2.44 vs 2.53) |
| react, vue, solid | deep-config (matcher, isolated) | ~~🟡 rr #2 to TanStack~~ → **🟢 rr WINS ~25×** (every cohort) | rr O(depth) **lightest** trie; TanStack also O(depth) (~70 µs @90 — the "O(1) flat" was a matcher-bench bug) | **measured (isolated · bug-fixed 07-16)** | ✅ resolved — **no real deep-matcher lag**; react-router #15249 = **6–9 ms** |
