...
|---|---|---|---|---|---|
| all | cold-start | +8.5%…+123% (react +8.5%, vue +76%, solid 2.2×, svelte +52%, angular 2.9×; rr +3.4–4.4 ms over bare) — **boot-CPU axis**; felt `fcpMs@10` (same cells): angular 28 ≈ 28 & react 36 ≈ 36 = WASH, vue/solid/svelte 1.2–1.4× (audit 07-18 K2) | `EAGER-CORE` | code-traced (#1106) | `STRUCTURAL` / `v2-CANDIDATE` |
| svelte | link-build | 🔴 3.2× (3.2 vs 1.0 ms mount @256; cap 256 `dadff6f7`) | `<Link>-COMPONENT` | code-traced (#1099/#1253; #1247 closed not-planned) | `FEATURE-COST` (use:link = fast path) |
| svelte | nested-switch | ~~+42%~~ → @1 +13% (0.107 vs 0.095); **@≥8 rr WINS** (@32 1.8×: 0.113 vs 0.203); @1 margin is first-point-artifact-inflated — bound-corrected ≈ wash (audit 07-18 K10) | `<Link>-COMPONENT` active-flip @shallow; sv O(depth) re-render @deep | **A/B-proven** (07-16; old row from retired sweep) | `FEATURE-COST` @1 → **rr WIN at depth** |
| svelte | param-nav | +14–19% wall (0.095 vs 0.080) | `<Link>-COMPONENT` + `IMMUTABLE-STATE` | **A/B-proven** | `STRUCTURAL` (use:link → +0–6% parity, no flip) |
| svelte | table-heap | +19% @100 (2.18 vs 1.83; 2.6× @10k retired — SIZES=[100]) | `SCALE-FLOOR` (**98% route-tree+trie**, ~650 B/route — the @10k decomposition; at the shown @100 point the table is ~29% of router heap, audit 07-18 K1) | **measured (heap-decomp)** | `STRUCTURAL` |
| svelte | nav-churn | +54% (381 vs 247 KB Δ@200-nav churn — NOT per-nav, warmup-dom. #1462) | `IMMUTABLE-STATE` (Δ-causation inferred) | code-traced | `STRUCTURAL` / `v2-CANDIDATE` |
| svelte | deep-config | +61% browser (3.35 vs 2.08 @90) — **RENDER, not matcher** | `FRAMEWORK-NATIVE` render (isolated matcher: rr **WINS 74.6×**, sv-router 202 vs 2.8 µs) | **measured (isolated)** | `STRUCTURAL` (compiled render) |
| svelte | back-forward | +4% (0.210 vs 0.202, ~8µs wall) | core back/forward path | inferred | `FRAMEWORK-NATIVE` (tiny; ~floor) |
| solid | nav-churn | +28% (310 vs 242 KB Δ@200-nav churn — not per-nav) | `IMMUTABLE-STATE` | code-traced | `STRUCTURAL` / `v2-CANDIDATE` |
| solid | table-heap | +17% @100 (2.09 vs 1.78; +3% @10k retired); vs tanstack rr lighter −7% | `SCALE-FLOOR` (route-tree+trie) | **measured (heap-decomp)** | `STRUCTURAL` (small) |
...
| angular | cold-start | 2.9× (6.18 vs 2.11) — boot-CPU; felt `fcpMs@10` **28 ≈ 28, a WASH** (audit 07-18 K2) | `EAGER-CORE` | code-traced | `STRUCTURAL` — table-heap +70% was @10k-retired; **@100 rr WINS −3.7%** (2.44 vs 2.53) |
| react, vue, solid | deep-config (matcher, isolated) | ~~🟡 rr #2 to TanStack~~ → **🟢 rr WINS ~25×** (every cohort) | rr O(depth) **lightest** trie; TanStack also O(depth) (~70 µs @90 — the "O(1) flat" was a matcher-bench bug) | **measured (isolated · bug-fixed 07-16)** | ✅ resolved — **no real deep-matcher lag**; react-router #15249 = **6–9 ms** (per public call — 96–98% `flattenAndRankRoutes` construct; amortized rescan ~0.1–3.4 ms, audit 07-18 K6) |
| react | search-param-scaling | 🔴 +48% (0.53 vs 0.36 ms @256, both sub-ms; task-axis — the single-nav `navMsTask@N` carries an unamortized out-of-window excess, felt wall@256 ≈ +33%; audit 07-18 K9) | `IMMUTABLE-STATE` (eager O(count) materialize; react-router 8.2.0 = one native `URLSearchParams`) | measured n=50; **code-traced** (magnitude inferred) | `STRUCTURAL` (rr does more per pass) |
...
| react, vue | table-heap | +5.4% / +9.5% @100 (rr 2.39/2.29 vs 2.26/2.09) | `SCALE-FLOOR` → **⚠ corrected (audit 07-18 K1): fixed engine statics, NOT per-route growth** — Δ(table@100−cold@10, same app): rr **46 KB**/+90 routes vs react-router 54 / vue-router 110 (rr's per-route growth is the lightest of the three; the abs lag sits in the O(1) floor) | measured (Ф3-audit 07-16) **+ corroborated** (cold-start heap@10: +6.0%/+13.2%, same sign, 2.7–4.7× > ±2% band) | `STRUCTURAL` (small) |
...
- **Fix:** `RouteView` + `injectRoute`/`injectRouteNode` call a local `detectChanges()` from the source callback (fires outside Angular CD, so it's safe — mirrors `RealLink`'s direct-DOM write). Same-session A/B: nav-latency 0.97 → 0.07 (~13×). n=50 @`622b27be`/`3e2a642e` (branch commits, squash-merged to master as `6e6dbf88` via PR #1469; re-verified @`bf8a189f` n=50): real-router leads `@angular/router` on all six per-nav wall axes (2.0–21×; wall≈task, max gap +0.09 ms back-forward).
- **Lesson:** a deferred-commit lag *is* addressable — look for a scheduler hop between the source notify and the view commit.
- **Residual (audit 07-18, K11 — floor cell only):** the same class survives UNFIXED in `apps/angular/_baseline` — its nav is `pushState` + `signal.set()` under zoneless with **no sync flush**, so its `navMsWall` (0.988 ms) times the CD-scheduler cadence (~1 ms macrotask hop, `scheduleCallbackWithRafRace` = race(setTimeout(0), rAF)), 12.4× its own task (0.080) — the only wall≫task outlier in 129 paired cells; the "bare framework floor" reads 13× slower than the router, physically absurd for a strict subset of work. Until the app mirrors the #1466 sync-commit (`cdr.detectChanges()` in `go()`), angular "router overhead over bare" reads on the **wall** axis are invalid (the task/script/alloc floor cells stay valid — they measure work, not waiting). code-traced (Angular 22 scheduler source) + signature-proven; deck/GRID unaffected (`_baseline` is never rendered).
...
- **A/B (same-session, drift-cancelled ≤1%, n=20, navMsTask@256):**
  - **vue:** rr-current **1.19** (slope 11×, O(N)) → rr-fixed **0.128** (slope 1.6×, **O(changed)**) vs vue-router **0.55** (slope 3.4×, O(N)). Fixed rr **BEATS vue-router 4.3–4.6×** (re-proven serial 07-16: OLD −2.18× → NEW +4.54×) — and is O(changed) where vue-router is O(N). Fix gain 9.3×.
...
### svelte nested-switch — `<Link>`-COMPONENT (active-class flip) → **FEATURE-COST** · *(historical — the numbers below are from the retired sweep; superseded 07-16, see the Summary row: @1 +13%, @≥8 rr WINS; the @1 margin itself is first-point-artifact-inflated ≈ wash — audit 07-18 K10/K17. The active-flip mechanism and the A/B below remain valid @1.)*
...
- **Verdict:** `STRUCTURAL`. Not winnable even with plain-`<a>`; same axes as the rest of the svelte cohort. The immutable-state residual is a `v2-CANDIDATE` (RFC-6 snapshot-store — defined only in the gitignored/local `.claude/core-v2-architecture-review-*.md`, not a tracked doc).
...
- **Root cause of the react lag:** rr eagerly materializes all 256 params into fresh immutable state each nav (O(count)); react-router does less per pass — **code-traced (07-16, Ф3-audit):** react-router **8.2.0** `useSearchParams` = one memoized native `new URLSearchParams` (`dom.js:57-71`) vs rr's JS parse + merge + spread + freeze (`SegmentMatcher.ts`'s param decode plus `StateNamespace`'s freeze); the ~0.17 ms **magnitude** attribution stays inferred. Same `IMMUTABLE-STATE` family as nav-churn / param-nav. rr **WINS the other 4 cohorts** (vue 1.35×, solid 17×, svelte 1.74×, angular 8.6× vs best rival) — the lazy routers explode (TanStack 6–12 ms, solid-router 15 ms); react-router is the lone competitor that also reads eagerly and stays light.
- **New observation (NOT a lag — rr wins):** svelte is now rr's **steepest** search-param cohort (@256 0.566 ms, 3.58× floor, **monotonic**, rme 3.2%) yet rr still WINS svelte (sv-router 0.985, mateo 1.237). **Bench-app equality code-confirmed (2026-07-16):** react/solid/svelte leaves all run the identical `_shared/search-param-spec` `readSearch(Object.entries(params))` over the same 9-`<Link>` nav (not 256) — no svelte-specific extra work. The steeper slope is the **per-param constant of Svelte's `$derived` recompute** — O(count) contribution ~0.40 ms @256 vs react VDOM ~0.35, solid fine-grained ~0.21 (solid's floor dilutes its ratio to 1.4×; svelte's low floor + high slope reads as a late spike). Framework reactivity, not app or router-core. Sub-ms, structural, no issue. **(NB audit 07-18 K9: sweep `navMsTask@N` absolutes carry a +0.06–1.2 ms unamortized out-of-window excess — the 0.566 / 3.58×-floor multiple is inflated on both ends; slope reads survive, felt-axis multiples are lower.)**
- **gc-per-nav side-effect:** one-realm shifted rr's measured `allocKBPerNav` up ~20% every cohort (warm steady-state; rr code unchanged — react 217→237, vue 92→113, solid 140→160, svelte 91→112, angular 86→106 KB) — **NB: the pre-fix baselines (217/92/140/91/86) are now historically unreproducible** (cells overwritten, run-logs lack the alloc pass). Story intact (rr lightest in vue/solid/svelte/angular; react-router edges rr 87 vs 237). Deck blurbs synced.
...
Core eagerly imports + parses its full dependency graph (path-matcher + route-tree + fsm + event-emitter; `sources` is pulled eagerly by each **adapter** one layer up, not the core graph), builds the trie, and runs the initial match + transition at boot — ~3.9 ms over the bare framework (measured 3.4–4.4 ms, all cohorts), the largest cross-cohort boot gap (#1106). Competitors tree-shake / lazy-load. One-time cost, not per-nav. **v2-CANDIDATE:** the v2 package collapse (9 foundation units → 3–4) + sources absorption would reduce the parsed graph. **Felt-rider (audit 07-18, K2):** the boot-CPU Δ is real but does not move first paint on the bench machine — `fcpMs@10` (emitted into the same cells): angular 28 ≈ 28 and react 36 ≈ 36 (WASH at script-Δ up to 4.06 ms), vue 28 vs 20, solid 24 vs 18, svelte 24 vs 20 — felt 1.2–1.4× where script says 1.6–2.3×. The deck's cold-start card currently shows `scriptDurationMs@10` under a "first load + first paint" caption (audit K2 — caption-or-axis fix pending); the felt axis softens this v2-CANDIDATE's motivation.

### svelte link-build — `<Link>`-COMPONENT → `FEATURE-COST`

rr 3.2 ms vs sv-router 1.0 ms to mount @256 links (mateo 1.9, bare 0.8; sweep capped at 256 `dadff6f7` — the historical "12.1 vs 3.2 @~1000" is retired). sv-router uses plain-`<a>`; rr's `<Link>` instantiates a Svelte component per link. Already litigated: #1099 (`<Link>` render overhead), #1253 (per-link listeners → event-delegation) — both closed; **#1247 (per-link `createSubscriber` → shared active-name) closed as not-planned**. **`use:link` is the documented fast path** (README recommends it for link-heavy/paginated pages; ~2× cheaper to mount, and — per the param-nav/nested-switch A/Bs — also cheaper on *update*, not just mount). `<Link>` stays the idiomatic default; the component cost buys ergonomics (auto active-state, hash support).

### nav-churn (solid, svelte) — `IMMUTABLE-STATE` → `STRUCTURAL` / `v2-CANDIDATE`

real-router shows a larger forced-GC retained Δ over the **200-nav churn window** (svelte +54%, solid +28%; RME 0.0% — deterministic). **NB (unit):** `heapDeltaKB` is a Δ over 200 navs in ONE app, warmup-dominated (#1462) — **NOT per-nav retention nor a create→navigate→dispose cycle**. The per-nav allocation mechanism (a fresh, frozen `State` per navigation; solid-router/sv-router mutate a signal in place) is code-traced; the Δ-causation is inferred. This is the eager-immutable identity (explicitly kept in the v2 review). **v2-CANDIDATE:** RFC-6 (versioned snapshot-store + total freeze) targets exactly this floor.
...
- **react-router #15249 is a REAL, catastrophic MATCHER cost, not a render quirk.** The isolated matcher re-flattens + re-ranks the whole 90-deep config every call: a **parabola peaking at depth 60 (9.6 ms), ~4600× real-router's matcher** — same parabola shape (peak @60) as the browser card, a matcher-algorithm signature (render would be monotonic in depth). **The browser "react-router wins deep @90" (1.5 ms) was render**: its matcher @90 is 5953 µs, ~2000× heavier than real-router (2.85 µs). The old `COMPETITOR-ARTIFACT` / `ARTIFACT` verdict was wrong — it is a real, severe react-router matcher cost. (**Caveat:** the isolated `matchRoutes` is an **upper bound** on the public-API re-flatten — the browser data-router amortizes it across navigations, so the browser full-nav @90 1.5 ms sits below the isolated 5.95 ms; but the parabola / #15249 is real, the shape matches.) **Quantified (audit 07-18, K6/G1p):** the isolated per-call absolute @90 is **96–98% `flattenAndRankRoutes` construct**; the bench app is Data mode (`createBrowserRouter`), which **does** amortize it (dist `router.js:74` — `#branches` computed once; every per-nav match calls `matchRoutesImpl` with precomputed branches), so the amortized rescan is **~0.11 ms @90 ≈ 40× real-router** (mid-tree peak ~3.4 ms ≈ 1.5–2k×). The parabola survives on the amortized axis and is the **scan of branches below the match** (flatten itself is depth-constant) — "re-flattens every call → parabola" misattributes the shape. The "wins deep @90 was render" framing is also wrong: browser full-nav 1.445 ms sits below the isolated 5.84 ms *because of amortization* — no render compensation is needed. Cite "~4600×/~2000×" only as per-public-call (Declarative-mode) numbers, alongside the amortized ~40× endpoint.
- **real-router's matcher is O(depth) but the LIGHTEST — it WINS the deep matcher in every cohort** (0.6 → 2.8 µs at depth 90; the trie walks d levels). TanStack is **also O(depth)** (~5 → 70 µs @90) — the earlier "TanStack O(1) flat ~1 µs" was a **matcher-bench bug**: `tanstack buildDeep` closed over a mutated `let parent` (late-binding → all 90 levels parented to the last), so the tree never built and `matchRoutes` fuzzy-matched to `/deep` (2 matches) at any depth → a false O(1). Found by the independent audit (C9); fixed 07-16 (per-iteration `const` capture + a depth-assert gate rejecting a match that doesn't reach depth d). Corrected: real-router (2.8 µs) beats TanStack (~70 µs) **~25×** at depth 90 → **no real deep-matcher lag exists** — real-router has the best measurable deep matcher of all.
...
- **table-heap @100 (react/vue small-N)** — the +5.4% / +9.5% @100 margins are **cross-scenario corroborated** (Ф3-audit 07-16): cold-start `jsHeapMB@10` shows the same-sign per-route overhead (react +6.0%, vue +13.2%) at an independent load point, 2.7–4.7× above the ±2% cross-session heap band — so the small-N table-heap lag is a real per-route footprint, not measurement noise. Same `SCALE-FLOOR` mechanism as the @10k decomposition, one order of magnitude down. **⚠ Corrected (audit 07-18, K1): NOT a per-route footprint** — the corroborated same-sign margin is real as an *absolute*, but on the growth axis (Δ table@100 − cold-start@10, same app, same post-GC read) rr adds **46 KB**/+90 routes vs react-router **54** / vue-router **110** — rr's per-route growth is the lightest of the three; the @100 lag lives in the **fixed engine statics** (the O(1) machinery the @10k decomposition prices at 0.16 MB), not in per-route cost. At @100 the route table is ~29% of rr's router heap (98% is the @10k asymptote), and ~95–99% of the deck's shown absolute is framework shell + engine statics (the deck's "just to hold its routes" caption is flagged — audit K1).
...
- **First-point bump on per-point-reload sweeps (wide @4 / nested @1 / active @4) —
  NOT cross-engine-uniform (audit 07-18, K10).** The residual first-executed-point
  cold tail survives `WARMUP_NAVS=6` and is engine-dependent: spread ×1.00–2.20 and
  in BOTH directions — nested: solid-router 1.30 (larger than any rr point),
  rr 1.08–1.24, tanstack 1.12–1.13; wide: sv-router **2.20** (the dataset max — a
  competitor), tanstack-react 1.48, rr 1.21–1.69; vue-router ~1.0. rr merely shows
  it consistently (5/5 cohorts) because its curves are flat. Artifact-hood proven
  without a live run: each engine violates its own monotone bound (rr nested @1
  0.108 > @2 0.093 on a strictly lighter workload), and the one-realm search-param
  control (p0/p1 0.83–1.16 for ALL engines) shows removing the per-point reload
  collapses the class. Consequences: svelte nested @1's near-r closeness is
  artifact-inflated (bound-corrected ≈ wash — the deck's "one crossover in the
  suite" blurb is unsupported); svelte active @4 y may hide a real g
  (bound-corrected ≈ 1.118 at the 1.12 threshold); browser wide p0/p1 must not be
  read as behavior; all other cohorts' classes hold with 1.6–4.9× margin. The
  in-code comment "accepted, cross-engine-uniform artifact"
  (`search-param-scaling.mjs:23-25`) is empirically false. Fix: sacrificial first
  episode per sweep (recipe in the 07-18 audit report); do NOT carry the "biased
  against rr" framing — in solid nested the artifact *flatters* rr.
