...
const REACT_BLURBS = {
  ...
  "nav-churn": "200-nav stress; per-nav total (script + Blink) + heap. **real-router lightest CPU/nav (0.94 total)**, tanstack 1.16, react-router 1.30 — **and retains the least heap (684 KB)**. navsPerSec frame-capped.",
  "active-links": "Per-nav total recompute across 100 links (steady-state toggle). **real-router lightest by a wide margin — 0.82 ms vs ~1.5–1.7** (~2× lighter; cached active-source per link). Blink ~equal.",
...
const SOLID_BLURBS = {
  ...
  "nav-churn": "200-nav stress; CPU/nav + retained heap. **real-router leanest CPU/nav — 0.324 total / 0.061 script per nav** (< tanstack 0.445 < @solidjs/router 0.533); **@solidjs/router retains the least heap (235 KB), real-router 299, tanstack 638.** `navsPerSec` is NOT comparable here: real-router navigates synchronously (churns 200 navs without yielding to frames → ~15k/s) while @solidjs/router + tanstack defer to the frame cadence (~121/s) — read CPU/nav + heap.",
  "active-links": "Per-nav total recompute across 100 links (steady-state toggle). **All three tight — tanstack 0.345 < real-router 0.375 < @solidjs/router 0.439** (~1.3× spread, unlike the React cohort's wide spread). @solidjs/router's `<A>` is inherently active-aware; real-router recomputes a cached active-source per link.",
...
const FW = {
  ...
  solid: {
    ...
    scopeNote: "**Scope — three full routers, like-for-like** (no minimalist exclusion needed). The honest picture, DIFFERENT from the React/Vue cohorts: **`@solidjs/router` is a remarkably lean router** and leads the scale floor (wide / param-scaling), table-heap memory, cold-start, nav-churn heap, and active-links. **real-router leads the per-nav *totals*** — nav-latency (0.347, the leanest here), param-nav, nested-switch, link-build — where its lean script + single `pushState` beat @solidjs/router's 2× Blink history; and it brings the full pipeline (guards, validated search, data, scroll). Two adapter soft-spots surface: real-router's heaviest per-nav FLOOR at scale (transition pipeline + browser-plugin History), and **`@real-router/solid`'s deep-nesting `RouteView` cost (#1094 — now ~linear after a bench-app fix removed an O(depth²) getter-chain artifact: 1.07 ms @90)** — an adapter issue, not the (framework-agnostic, flat-capable) core. **`@tanstack/solid-router` is generally heaviest** and cannot render 60+-deep nested routes (errors).",
