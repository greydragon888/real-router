...
const ANGULAR_BLURBS = {
  ...
  "nav-latency": (h) => `Per-nav **wall** (click→DOM settle, felt) + **task** (ΔTaskDuration, CPU). **A split verdict: @angular/router wins the felt wall (${h.m("navMsWall", "angular-router")} vs real-router ${h.m("navMsWall", "real-router")}), but real-router wins CPU (task ${h.m("navMsTask", "real-router")} vs ${h.m("navMsTask", "angular-router")}, ${h.x("navMsTask", "real-router", "angular-router")}) and allocation (~${h.kb("allocKBPerNav", "real-router")} vs ${h.kb("allocKBPerNav", "angular-router")} KB/nav, ${h.x("allocKBPerNav", "real-router", "angular-router")}).** The ~0.87 ms wall gap is idle time — the \`@real-router/angular\` adapter commits the route DOM a task later than the click (async change-detection settle), where @angular/router commits within the click task (**#1466, under investigation**). So real-router does far less CPU/GC work per nav yet *feels* slower on plain-link navs. *(Sub-ms task; wall carries the adapter's async settle — read both rows.)*`,
  "param-nav": (h) => `Per-nav wall + task changing :id (steady-state). **real-router wins CPU — task ${h.m("navMsTask", "real-router")} vs @angular/router ${h.m("navMsTask", "angular-router")} (${h.x("navMsTask", "real-router", "angular-router")}) and alloc ${h.x("allocKBPerNav", "real-router", "angular-router")} leaner (${h.kb("allocKBPerNav", "real-router")} vs ${h.kb("allocKBPerNav", "angular-router")} KB)**; felt wall ~parity (${h.m("navMsWall", "real-router")} vs ${h.m("navMsWall", "angular-router")}, both carry Angular's async settle). *(Sub-ms — session/load-dependent.)*`,
