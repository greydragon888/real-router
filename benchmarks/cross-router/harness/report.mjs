...
const VUE_BLURBS = {
  ...
  "wide-config": "Navigate into a flat 1000-route table — **the matcher crossover.** vue-router is lightest at small N (lean Vue floor) but **degrades @1000** (O(N) matcher); **real-router stays flat and WINS @1000 — task 0.275 vs vue-router 0.546, tanstack 0.72** (~2×). The trie's structural win at scale survives the honest metric.",
  "table-heap": "Retained JS heap holding 1 / 1000 / 10000 routes (forced GC). **real-router is the lightest at 10k — 6.07 MB vs vue-router 13.85 (~2.3×) and tanstack 11.31** (the #1009/#1010-optimized trie). At scale real-router wins both CPU (wide) and memory.",
  ...
  "nested-switch": "Sibling switch a↔b under a shared layout (steady-state) — reuse the parent. **real-router WINS — wall 0.1 vs vue-router 0.22, tanstack 0.32** (~2.2× lighter). *(Sub-ms — session/load-dependent.)*",
...
const SOLID_BLURBS = {
  ...
  "link-build": "CPU to mount 1000 links, each building its href. **real-router leanest (8.7 ms); @solidjs/router 10.1; tanstack ~3× (27.2).** Over the bare-`<a>` floor real-router adds the least reverse-matcher cost.",
  "nested-switch": "Sibling switch a↔b under a shared layout (steady-state) — reuse the parent. **real-router leanest — wall 0.08 vs @solidjs/router 0.12, tanstack 0.515** (~1.5× under @solidjs/router), consistent with its nav-latency / param-nav wins here. *(Sub-ms — session/load-dependent.)*",
...
const SVELTE_BLURBS = {
  "cold-start": "App init + parse/exec to first route painted. **sv-router is the lightest to boot** (script 3.58 ms, heap 1.85 MB); mateo-router 4.0 ms / 1.89 MB; real-router heaviest (5.16 ms / 2.20 MB). Over the bare-Svelte floor real-router adds the most boot cost (#1106).",
  ...
  "param-nav": "Per-nav wall + task changing :id (steady-state). **sv-router leanest (wall 0.08); real-router second (0.093, +16%); mateo-router third (0.14)** — sv-router's minimal reactive model sits just below real-router's eager pipeline. *(Sub-ms — session/load-dependent.)*",
  "wide-config": "Navigate into a flat 1000-route table — **the matcher crossover, and real-router's clearest win in this cohort.** real-router's segment trie stays FLAT (task ~0.23 across @10/@100/@1000) while **sv-router degrades O(N)** (0.475 @1000 — it sorts + scans its route-key list per nav, NOT a segment trie) and mateo-router carries a high floor (~0.52). **real-router WINS @1000** — task 0.233 vs sv-router 0.475 (~2×), mateo 0.519 — the structural trie advantage holds in Svelte (as in React/Vue/Solid).",
  ...
  "nav-churn": "200-nav stress; per-nav **task** (CPU) + retained **heap Δ**. **CPU/nav a 3-way tie (~0.08 task).** sv-router retains the least heap (247 KB), mateo-router 303, real-router 396 (heaviest here). `navsPerSec` is settle-timed ~14k/s for all three — read CPU/nav + heap (heap Δ warmup-dominated, not a leak #1462).",
...
const ANGULAR_BLURBS = {
  ...
  "param-nav": "Per-nav wall + task changing :id (steady-state). **real-router wins CPU — task 0.14 vs @angular/router 0.345 (~2.5×) and alloc ~16× leaner (9 vs 146 KB)**; felt wall ~parity (0.752 vs 0.807, both carry Angular's async settle). *(Sub-ms — session/load-dependent.)*",
  "wide-config": "Navigate into a flat 1000-route table. **real-router stays flat and WINS @1000 by a wide margin — task 0.22 vs @angular/router 2.834 (~12.9×)** — @angular/router's matcher degrades sharply with N (it is *not* flat at scale), so real-router's segment trie is decisive here (as in React/Vue/Solid/Svelte). Its official router is not a trie.",
  "deep-config": "Navigate into a 90-level nested chain. **real-router WINS decisively — task 0.49 → 1.67 @90 vs @angular/router 1.14 → 5.50 (~3.3× @90)** — both rise O(depth) but @angular/router far steeper. Real apps rarely nest past ~10, where real-router already leads.",
  ...
  "nested-switch": "Sibling switch a↔b under a shared layout (steady-state) — reuse the parent. **real-router wins CPU — task 0.151 vs @angular/router 0.392 (~2.6×)**; felt wall ~parity (0.838 vs 0.875, both carry the async settle). *(Sub-ms task — session/load-dependent.)*",
  "back-forward": "Browser **back/forward** (popstate) steady-state. **real-router WINS — wall 0.207 vs @angular/router 0.395 (~2×), task 0.116 vs 0.35.** **#1353** (skip no-op popstate `replaceState`) removed real-router's redundant second history event. real-router the leaner **allocator** too (~8 KB/nav vs @angular/router 128, ~17×). *(n=50.)*",
};
