<script lang="ts">
  // useRoute() in a CHILD of RouterProvider. real-router merges the declared query
  // params (searchDecl → `?k1&k2&...`) into the active route's params object; the
  ...
  const stats = $derived(readSearch(Object.entries(route.current.params)));
</script>
