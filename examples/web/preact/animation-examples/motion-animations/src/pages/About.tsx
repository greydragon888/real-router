  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>useRouteExit</code> from <code>@real-router/preact</code>{" "}
      ...
      <p>
        ...
        from <code>@real-router/preact</code>. Encapsulated, simpler mental
      ...
      <p>
        ...
        <strong>library-native</strong> — what{" "}
        <code>route-animations/</code> implements in ~340 LOC of manual{" "}
        <code>getBoundingClientRect</code> + WAAPI bookkeeping is a single
        prop here.
      </p>
      ...
      <ul>
        ...
        <li>
          ...
          list FLIP with ghosts, full router coordination? →{" "}
        ...
        <li>
          Cross-browser, hero morph, list FLIP, gesture support, willing to
          pull in a library, want library-native primitives over hand-rolled
      ...
      <p>
        ...
        <code>&lt;motion.li layout&gt;</code>) are declarative props on
        inner elements, not procedural code in a coordinator module. The
        router stays blocked on the page-level exit Promise (resolved by{" "}
        <code>onExitComplete</code>), so URL and UI stay in lock-step —
        same semantics as <code>route-animations/</code> and{" "}
        <code>page-animations/</code>, but the choreography is expressed
        through library primitives instead of hand-rolled FLIP / ghost
        cloning code.
      </p>
      ...
      <p>
        ...
        <code>useRouteExit</code> from <code>@real-router/preact</code>{" "}
        with a handler that bumps an <code>exitToken</code> (triggering
      ...
      <p>
        ...
        Preact&apos;s standard <code>preact/compat</code> alias (configured
        in <code>vite.config.ts</code> via{" "}
        <code>@preact/preset-vite</code>), which redirects{" "}
        <code>react</code> / <code>react-dom</code> imports to{" "}
