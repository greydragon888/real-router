  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>@real-router/preact</code> with its own DOM recipe; pages stay
        declarative — they only mark <code>[data-route-root]</code> /{" "}
        <code>[data-flip-key]</code> attributes. Cross-route coordination (hero
        rects, list FLIP, ghost clones) lives in module-level refs inside the
        hooks.
      </p>
      ...
      <p>
        ...
        page calls the hook in its own component; the hook itself is built on{" "}
        <code>useRouteExit</code> + <code>useRouteEnter</code> from{" "}
        <code>@real-router/preact</code>. Encapsulated, simpler mental model,
        less boilerplate per new page once the hook exists. Cross-page
        coordination needs shared state because each component&apos;s hook only
      ...
      <p>
        ...
        The Promise the router awaits resolves on <code>onExitComplete</code> —
        URL and UI stay in lock-step like the other three examples. Hero morph
        through <code>layoutId</code> and list reorder through{" "}
        <code>&lt;motion.li layout&gt;</code> are{" "}
      ...
      <p>
        ...
        <code>&lt;AnimatePresence onExitComplete&gt;</code>: it calls{" "}
        <code>useRouteExit</code> from <code>@real-router/preact</code> with a
        handler that bumps an <code>exitToken</code> (triggering
        AnimatePresence&apos;s exit on the cached old subtree) and returns a
        Promise resolved when motion fires <code>onExitComplete</code>. The
        abort signal from <code>useRouteExit</code> resolves the same Promise on
        rapid navigation, so the router pipeline drains cleanly — no
        exit-token-counter bookkeeping. Entry animations are declarative — the{" "}
      ...
      <h2>Note on Preact + motion</h2>
      <p>
        <code>motion</code> is a React-only library. This example uses
        Preact&apos;s standard <code>preact/compat</code> alias (configured in{" "}
        <code>vite.config.ts</code> via <code>@preact/preset-vite</code>), which
        redirects <code>react</code> / <code>react-dom</code> imports to{" "}
        <code>preact/compat</code>. Most React libraries — including motion —
        work transparently through this alias.
      </p>
    </div>
  );
}
