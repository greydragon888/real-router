    ...
    <div>
      ...
      <ol>
        <li>
          ...
          <code>exit=&#123;{`{ opacity: 0, x: -20 }`}&#125;</code>), the new
          one slides in. <code>mode=&quot;wait&quot;</code> guarantees
        ...
        <li>
          ...
          <code> AnimatePresence</code> suppresses entry animation for the
          first mount.
        </li>
        <li>
          <strong>Skip same-route</strong>: click <em>Home</em> while on
          Home — the router rejects with <code>SAME_STATES</code> before
          re-render; <code>route.name</code> stays the same so
          AnimatePresence is not triggered.
        </li>
        <li>
          <strong>Hero morph (free, via layoutId)</strong>: open{" "}
          <em>Products</em>, click any card. The thumbnail&apos;s rect is
          paired with the cover via{" "}
          <code>layoutId=&quot;product-&#123;id&#125;&quot;</code>; library
          handles the FLIP automatically — no{" "}
          <code>getBoundingClientRect</code> or WAAPI bookkeeping in our
          code.
        </li>
        <li>
          <strong>List reorder (free, via layout)</strong>: open{" "}
          <em>Products</em> and switch sort. Each <code>motion.li</code> with{" "}
          <code>layout</code> animates from old to new position; library
          reads positions before / after re-render and FLIP&apos;s the diff.
        </li>
        <li>
          <strong>Filter (Query demo)</strong>: switch a filter — survivors
          glide via <code>layout</code>, newcomers fade in via{" "}
          <code>initial</code> / <code>animate</code>, removed items fade
          out via the inner{" "}
        ...
        <li>
          ...
          animations. Slide-x collapses to opacity-only fade; layoutId pairs
          and motion.li layout become instant snaps.
        </li>
        <li>
