...
export function About(): JSX.Element {
  return (
    <div>
      <h1>About</h1>
      <p>
        This example uses <code>@real-router/preact</code> with the{" "}
      ...
      <ol>
        ...
        <li>
          ...
          &mdash; mandatory before <code>updateCallback</code> runs), then
          invokes the callback. Inside the callback we call{" "}
          <code>resolveLeave()</code> — the router unblocks and proceeds through
          activation guards and <code>setState</code>.
        </li>
        <li>
        ...
        <li>
          Utility resolves the deferred via <code>setTimeout(0)</code>. The
          Preact scheduler queues commits as microtasks/tasks earlier than our
          timer, so the DOM is committed by the time our callback runs. Browser
          then captures the new snapshot and plays{" "}
      ...
      <p>
        <strong>Why clicks feel blocked during long animations</strong>
      </p>
      <p>
      ...
      <p>
        ...
        ms fade, the entire viewport is captured — there is nothing underneath
        to hit, so the overlay-transparency trick cannot rescue interactivity.
      </p>
      <p>
