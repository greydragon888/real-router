import { useNavigator, useRouteExit } from "@real-router/preact";
import { useEffect, useRef } from "preact/hooks";

interface FlipSnapshot {
  rects: Map<string, DOMRect>;
  // Detached clones for items that may unmount on rerender (filter
  // narrowed). We cannot animate exit on the original — Preact removes
 ...
 * page-level hook (`usePageAnimator`) already skips these via
 ...
 * exit decisions are out of scope of `useRouteExit` — the hook only
 * handles router-side coordination (abort signal, same-route
 * detection).
 *
 *   1. On exit (same-route only), capture the bounding rect of every
 *      `[data-flip-key]` element AND clone it (clones are needed
 *      because Preact unmounts removed items synchronously before
 *      `subscribe` fires).
 *   2. Pre-hide newly-mounted items with `opacity: 0` via a
 *      `MutationObserver` running during Preact's commit phase.
 ...
 */
export function useListFlip(): void {
  const navigator = useNavigator();
  const pendingFlipsRef = useRef<FlipSnapshot | null>(null);
  const flipObserverRef = useRef<MutationObserver | null>(null);

  useRouteExit(
    ({ route, nextRoute }) => {
      // List-FLIP only fires on same-route navigations (sort / filter
      // on the same page). Cross-route is handled by usePageAnimator.
      if (route.name !== nextRoute.name) {
        pendingFlipsRef.current = null;

        return;
      }

      pendingFlipsRef.current = captureFlipSnapshot();

      const snapshot = pendingFlipsRef.current;

      if (snapshot?.parent) {
        flipObserverRef.current?.disconnect();
        flipObserverRef.current = preHideNewItems(
          snapshot.parent,
          new Set(snapshot.rects.keys()),
        );
      }
    },
    { skipSameRoute: false },
