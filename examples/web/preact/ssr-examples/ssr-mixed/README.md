...
serving four routes from a single `entry-server.tsx`:
...
`entry-server.tsx` reads `getSsrDataMode(state)` and branches:

- `"full"` — `renderToStringAsync(<App/>)` (Preact's async-aware single-shot SSR).
