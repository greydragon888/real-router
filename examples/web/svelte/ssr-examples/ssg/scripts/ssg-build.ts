...
const { renderPage: renderEntry, getStaticPaths } = (await import(
  path.resolve(root, "dist/server/entry-server.js")
)) as {
  renderPage: (url: string) => Promise<{
    html: string;
    head: string;
    ssrJson: string;
    statusCode: number;
...
for (const url of paths) {
  try {
    const result = await renderEntry(url);

    const html = buildHtml(result, { withSsrState: true });
...
const notFound = await renderEntry("/__nonexistent");
const notFoundHtml = buildHtml(notFound, { withSsrState: false });
