...
| **Where loaders run** | Server at boot/SSG; on the client `hydrateRouter` reuses pre-resolved `state.context.data` via the `@real-router/ssr-utils` scratchpad (#596) — loader skipped on first paint; CSR navigations require `lifecycle-plugin.onNavigate` by design | n/a |
