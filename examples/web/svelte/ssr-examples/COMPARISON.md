...
> Authoritative source for Real-Router capabilities: [`README.md`](./README.md) (synthesised from the three child READMEs in [`ssr/`](./ssr), [`ssr-streaming/`](./ssr-streaming), [`ssg/`](./ssg)).
...
`svelte-spa-router` is a small, focused **hash-based SPA router**: routes are an object/Map mapping path patterns to components, lifecycle callbacks fire on load, and the wire format is `#/page`. It does **not** ship per-route loaders, SSR, SSG, hydration, typed errors → HTTP, or any server-side concept — and that is its design. **Real-Router on Svelte 5 ships three dogfooded delivery shapes** (`render` for SSR with strong sha256 ETag, deferred-data SSR via `{#await}` / `<svelte:boundary>`, and build-time SSG with `getStaticPaths` leaf-only), wires `ssr-data-plugin` loaders into all three, codifies a typed-error → HTTP envelope, threads `AbortController` into loader DI, and keeps the same plugin contract that ports across React/Vue/Solid/Preact/Angular. **`svelte-spa-router` "wins" only by being smaller in scope** (one file, no plugins, no SSR concerns); on every SSR/SSG axis it is silent because it does not address the problem space. **Real-Router wins on every SSR/SSG axis by default**, but readers who only need a CSR-only hash router for a static deployment will find `svelte-spa-router` denser per byte. Svelte 5 framework limits (single buffered HTML body, no chunked HTTP streaming, no out-of-order placeholders, atomic `hydrate()`) constrain Real-Router on Svelte equally — `ssr-streaming/` is **deferred-data SSR**, not progressive HTTP flush, and the README documents this falsifiably with a `node:http` reproducer (Scenario 12: 1 TCP frame, ~0 ms span).
...
| **Race / cancellation** | `AbortController` threaded into loader DI in `ssr/` (the `slow` loader pulls via `getDep("abortSignal")` and clears its `setTimeout`); `withTimeout()` races a `LoaderTimeout` against the original promise (does not cancel underlying work — pair with `AbortController`) | n/a — no fetch layer at the router level |
