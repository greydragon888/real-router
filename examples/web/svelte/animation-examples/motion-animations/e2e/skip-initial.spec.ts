...
test.describe("Skip initial load", () => {
  test("first-load suppresses entry animation via initial={false}", async ({
    page,
  }) => {
    await page.goto("/");
    // Heading should be visible immediately — AnimatePresence's
    // initial={false} prevents the page-level motion.div from animating
    // on first mount.
    await expect(
      page.getByRole("heading", { name: "Motion Animations" }),
    ).toBeVisible();

    // Library may still register an animation briefly during hydration,
