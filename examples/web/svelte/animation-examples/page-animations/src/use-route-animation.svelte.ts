import { useRouteEnter, useRouteExit } from "@real-router/svelte";
 ...
 * composable with a getter for its outer wrapper element.
 *
 * Svelte pattern: pages declare `let el: HTMLDivElement | undefined`,
 * bind it via `bind:this={el}`, and pass `() => el` to the composable.
 ...
 * abstraction needed — Svelte's `let` + `bind:this` does the same job.)
 *
 ...
 * init, so `entryClass` / `exitClass` / `skipSameRoute` are captured
 * at hook-call time. This is intended — pages declare their animation
