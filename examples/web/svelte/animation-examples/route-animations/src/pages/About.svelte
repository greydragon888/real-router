<div data-route-root data-route-anim="fade">
  ...
  <ul>
    ...
    <li>
      ...
      pixel-level crossfade between snapshots, so identical regions (e.g.
      a header that does not change) are visually static. With CSS
      recipes, everything inside the leaving root fades — there is no
      concept of "identical region stays static" because we never compare
  ...
  <p>
    ...
    <code>useRouteExit</code> from <code>@real-router/svelte</code> once
    with its own recipe:
  </p>
  <ul>
    <li>
  ...
  <p>
    ...
    is delegated entirely to the <code>useRouteExit</code> composable. The
    composables themselves only own the DOM-side recipes (style flush,
    element-scoped <code>getAnimations()</code>, reduced-motion fast-path).
  </p>
  <p>
    <strong>Svelte handler-reactivity:</strong> composables run once at
    component init, so handler bodies are captured in closure.
    Cross-handler state (<code>useHeroMorph</code>'s
    <code>pendingHero</code>, <code>useListFlip</code>'s
    <code>pendingFlips</code>) lives as plain <code>let</code> variables
    in the composable body — equivalent role to React's
