 ...
 *      Web Animations API.
 *
 * The two phases share `pendingHero` — exit captures, subscribe applies
 * and clears. Because Svelte composables run once at component init,
 * a plain `let` variable is the natural equivalent of React's `useRef`
 * (closure-captured, mutable across handler invocations).
 */
export function useHeroMorph(): void {
  ...
  const off = navigator.subscribe(() => {
    ...
    // setTimeout(0) defers past Svelte's commit so the new page's
