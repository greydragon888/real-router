...
interface FlipSnapshot {
  ...
  // narrowed). We cannot animate exit on the original — Svelte removes
 ...
 *      because Svelte unmounts removed items synchronously before
 *      `subscribe` fires).
 *   2. Pre-hide newly-mounted items with `opacity: 0` via a
 *      `MutationObserver` running during Svelte's commit phase.
 ...
 * component init, so `pendingFlips` and `flipObserver` live in the
 * closure as plain `let` variables (equivalent role to React's
 * `useRef`).
 */
