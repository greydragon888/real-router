 ...
 *
 * The VT utility in `@real-router/solid` (via `viewTransitions` prop)
 ...
 * `setTimeout(0)` (new DOM, after Solid commits). A single CSS rule
...
export function installViewTransitionPolicy(router: Router): () => void {
  ...
  const offSuccess = router.subscribe(() => {
    // After Solid commits the new DOM, find the target element on the
    // destination page and mark it. Solid's fine-grained reactivity
    // commits synchronously inside subscribe, but setTimeout(0) is kept
    // for symmetry with other adapters and runs before the VT utility's
