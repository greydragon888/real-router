  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>useHeroMorph</code>, <code>useListFlip</code>) called once
        at the top of <code>App</code>. Each hook wraps{" "}
        <code>useRouteExit</code> from <code>@real-router/solid</code>{" "}
        with its own DOM recipe; pages stay declarative — they only mark{" "}
        <code>[data-route-root]</code> / <code>[data-flip-key]</code>{" "}
        attributes. Cross-route coordination (hero rects, list FLIP,
        ghost clones) lives in plain <code>let</code> variables inside
        the hooks.
      </p>

      <h2>page-animations/</h2>
      <p>
        Distributed via <code>useRouteAnimation(() ={">"}{" "}
        ref, …)</code> per page. Each page calls the hook in its own
        component; the hook itself is built on{" "}
        <code>useRouteExit</code> + <code>useRouteEnter</code> from{" "}
        <code>@real-router/solid</code>. Encapsulated, simpler mental
      ...
      <p>
        ...
        — Solid bindings around{" "}
        <a href="https://motion.dev">Motion One</a>.{" "}
        <code>&lt;Presence exitBeforeEnter&gt;</code> wraps a single
        page-level <code>&lt;Motion.div&gt;</code> keyed by an{" "}
        <code>exitToken</code> counter bumped inside{" "}
        <code>useRouteExit</code>. The Promise the router awaits
        resolves on the exiting element's <code>onMotionComplete</code>{" "}
      ...
      <p>
        ...
        animations. Motion One — and therefore solid-motionone — does
        not bundle these layout primitives. For those scenarios in
        Solid, see <code>route-animations/</code> for the hand-rolled
        WAAPI recipes (<code>useHeroMorph</code> ≈ 110 LOC,{" "}
      ...
      <ul>
        ...
        <li>
          Cross-browser, declarative library-driven page transitions,
          willing to add a small dependency, comfortable falling back
          to manual recipes for hero morph / list FLIP? →{" "}
      ...
      <p>
        <strong>Library-driven entry/exit with router-coordinated
        semantics.</strong> The router stays blocked on the page-level
        exit Promise (resolved by <code>onMotionComplete</code>), so URL
        and UI stay in lock-step — same semantics as{" "}
        <code>route-animations/</code> and <code>page-animations/</code>,
        but the entry / exit choreography is expressed through library
        primitives (declarative <code>animate</code> /{" "}
        <code>exit</code> props on Motion components) instead of
        hand-rolled CSS keyframes + <code>animationend</code>{" "}
        bookkeeping.
      </p>
      ...
      <p>
        ...
        with <code>&lt;Presence&gt;</code>: it calls{" "}
        <code>useRouteExit</code> from <code>@real-router/solid</code>{" "}
        with a handler that bumps an <code>exitToken</code> (driving the{" "}
        keyed <code>&lt;Show&gt;</code> remount, which Presence runs
        exit on) and returns a Promise resolved when Motion fires{" "}
        <code>onMotionComplete</code>. The abort signal from{" "}
        <code>useRouteExit</code> resolves the same Promise on rapid
        navigation, so the router pipeline drains cleanly. Entry
        animations are declarative — the <code>initial</code> /{" "}
        <code>animate</code> props on <code>Motion.div</code> in{" "}
        <code>App.tsx</code>; no hook involvement needed because
        Motion One handles mount-time animation natively.
      </p>
    </div>
  );
}
