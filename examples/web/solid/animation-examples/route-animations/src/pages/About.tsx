    ...
    <div data-route-root data-route-anim="fade">
      ...
      <ul>
        ...
        <li>
          ...
          DOM. To approximate persistence, you would have to mount the
      ...
      <p>
        ...
        <code>useRouteExit</code> from <code>@real-router/solid</code> once with
        its own recipe:
      </p>
      <ul>
        <li>
      ...
      <p>
        Routing-side coordination — abort signal pre-check, same-route skip — is
        delegated entirely to the <code>useRouteExit</code> hook. The hooks
        themselves only own the DOM-side recipes (style flush, element-scoped{" "}
        <code>getAnimations()</code>, reduced-motion fast-path).
      </p>
      <p>
        <strong>Solid handler-reactivity:</strong> components run once at mount,
        so handler bodies are captured in closure. Cross-handler state (
        <code>useHeroMorph</code>'s <code>pendingHero</code>,{" "}
        <code>useListFlip</code>'s <code>pendingFlips</code>) lives as plain{" "}
        <code>let</code> variables in the hook body — equivalent role to React's{" "}
