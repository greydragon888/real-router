...
export function About(): JSX.Element {
  return (
    <div data-route-root data-route-anim="fade">
      ...
      <ul>
        <li>
          <strong>Cross-browser</strong>: works in every browser that runs
          CSS animations. View Transitions silently degrades to no animation
          in older Firefox / Safari; this recipe does not.
        </li>
        <li>
          <strong>Sequential by default</strong>: the router blocks on the
          Promise returned from <code>subscribeLeave</code>, so the exit
          animation completes before the next page mounts. VT always gives
          you parallel crossfade.
        </li>
        <li>
        ...
        <li>
          ...
          during playback (real DOM is unhittable, clicks land on the
          overlay). CSS animations leave the DOM live throughout.
        </li>
      </ul>
      ...
      <ul>
        <li>
          ...
          the old and new snapshots automatically. The recipe approximates
          this by measuring <code>getBoundingClientRect()</code> on leave +
          applying an inverse-FLIP transform via the Web Animations API
          after the destination mounts. It works (see <em>Products</em> →{" "}
        ...
        <li>
          ...
          CSS recipes, everything inside the leaving root fades — there is
          no concept of "identical region stays static" because we never
          compare old and new DOM. To approximate persistence, you would
          have to mount the persistent content above the animation root,
          but cross-route shared content is then outside the leave-window
          entirely.
        </li>
        <li>
          <strong>Crossfade between routes</strong>: the recipe is
          sequential. A true crossfade (old fading out while new fades in,
          both visible mid-animation) needs both DOM trees mounted
      ...
      <p>
        ...
        <code>useRouteExit</code> from <code>@real-router/solid</code> once
        with its own recipe:
      </p>
      <ul>
        <li>
        ...
        <li>
          ...
          exits. Captures rects + clones unmount-bound items, replays
          inverse translates on survivors, fades clones for removed items.
        </li>
      </ul>
      <p>
        <code>createDirectionTracker(router)</code> from{" "}
        <code>shared/dom-utils</code> is wired in <code>main.tsx</code>{" "}
      ...
      <p>
        ...
        — is delegated entirely to the <code>useRouteExit</code> hook. The
        hooks themselves only own the DOM-side recipes (style flush,
        element-scoped <code>getAnimations()</code>, reduced-motion
        fast-path).
      </p>
      <p>
        <strong>Solid handler-reactivity:</strong> components run once at
        mount, so handler bodies are captured in closure. Cross-handler
        state (<code>useHeroMorph</code>'s <code>pendingHero</code>,{" "}
        <code>useListFlip</code>'s <code>pendingFlips</code>) lives as
        plain <code>let</code> variables in the hook body — equivalent
        role to React's <code>useRef</code>, simpler ergonomics.
      </p>
    </div>
  );
}
