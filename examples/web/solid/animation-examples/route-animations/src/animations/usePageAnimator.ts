 ...
 * to the reader of the example. `useRouteExit` from `@real-router/solid`
 * handles router-side coordination (abort pre-check, same-route skip,
 * subscription lifecycle via `onCleanup`).
 *
 ...
 *
 * Solid handler-reactivity caveat: components run once at mount and the
 * handler is captured in closure. The recipe above is closure-stable —
 * it reads from `document` synchronously, no captured state to go stale.
 */
