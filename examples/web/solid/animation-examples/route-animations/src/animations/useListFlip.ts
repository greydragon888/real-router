import { useNavigator, useRouteExit } from "@real-router/solid";
import { onCleanup } from "solid-js";

interface FlipSnapshot {
  rects: Map<string, DOMRect>;
  // Detached clones for items that may unmount on rerender (filter
  // narrowed). We cannot animate exit on the original — Solid removes
 ...
 *      because Solid unmounts removed items synchronously before
 *      `subscribe` fires).
 *   2. Pre-hide newly-mounted items with `opacity: 0` via a
 *      `MutationObserver` running during Solid's commit phase.
 ...
 * `pendingFlips` and `flipObserver` live in the closure as plain `let`
 * variables (equivalent role to React's `useRef`).
 */
