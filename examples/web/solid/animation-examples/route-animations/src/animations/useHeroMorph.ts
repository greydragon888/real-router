 ...
 * and clears. Because Solid components run once, a plain `let`
 * variable in the hook body is the natural equivalent of React's
...
export function useHeroMorph(): void {
  ...
  const off = navigator.subscribe(() => {
    ...
    // setTimeout(0) defers past Solid's commit so the new page's
