  ...
  return (
    <div ref={ref}>
      ...
      <p>
        ...
        <code>@real-router/solid</code> with its own DOM recipe. Cross-route
        coordination (hero rect capture, list FLIP, ghost clones) works because
        state lives in plain <code>let</code> variables (closure-captured)
      ...
      <p>
        ...
        central module. Encapsulated, simpler mental model, less boilerplate per
        new page after the hook is in place. The trade-off: each component
        instance only sees its own lifecycle. Hero morph between routes needs
        shared state because the source rect is captured in page A but the
        destination is in page B. Same for list FLIP with ghost exits —
      ...
      <p>
        Router-coordinated via <code>solid-motionone</code> (Solid bindings
        around Motion One — the framework-agnostic engine powering Framer
        Motion). <code>&lt;Presence&gt;</code> with an <code>exitToken</code>{" "}
        counter bumped inside <code>useRouteExit</code>; the Promise the router
        awaits resolves on <code>onExitComplete</code>. Same URL-and-UI
        lock-step semantics as the other three. Hero morph through{" "}
        <code>Motion.div layout-id</code> and list reorder through{" "}
        <code>Motion.li layout</code> are <strong>library-native</strong> —
        declarative props rather than procedural code. Trade-off: ~30 KB extra
        in bundle.
      </p>
      ...
      <ul>
        ...
        <li>
          ...
          to add a Motion One dependency for declarative ergonomics? →{" "}
      ...
      <p>
        ...
        <code>useRouteExit</code> from <code>@real-router/solid</code> wraps{" "}
        ...
        <code>useRouteEnter</code> from the same package — skip-initial built
        in.
      </p>
      <p>
