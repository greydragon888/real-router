  ...
  return (
    <div ref={ref}>
      ...
      <p>
        ...
        hooks. Pages stay declarative — they only mark{" "}
      ...
      <p>
        ...
        page. Each page subscribes to the router for its own lifetime;
        no central module. Encapsulated, simpler mental model, less
        boilerplate per new page after the hook is in place. The
      ...
      <p>
        Router-coordinated via <code>solid-motionone</code> (Solid
        bindings around Motion One — the framework-agnostic engine
        powering Framer Motion).{" "}
        <code>&lt;Presence&gt;</code> with an{" "}
        <code>exitToken</code> counter bumped inside{" "}
        <code>useRouteExit</code>; the Promise the router awaits resolves
        on <code>onExitComplete</code>. Same URL-and-UI lock-step
        semantics as the other three. Hero morph through{" "}
        <code>Motion.div layout-id</code> and list reorder through{" "}
        <code>Motion.li layout</code> are <strong>library-native</strong>{" "}
        — declarative props rather than procedural code. Trade-off: ~30
        KB extra in bundle.
      </p>
      ...
      <ul>
        ...
        <li>
          ...
          willing to add a Motion One dependency for declarative
          ergonomics? → <code>motion-animations/</code>.
        </li>
      </ul>
      ...
      <p>
        ...
        <code>{`ref={ref}`}</code>, and pass <code>{`() => ref`}</code> to
        the hook. The getter is read inside the handler at exit / enter
        time — no <code>useRef</code> abstraction needed.
      </p>
    </div>
  );
}
