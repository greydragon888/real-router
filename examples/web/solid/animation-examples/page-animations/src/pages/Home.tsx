  ...
  return (
    <div ref={ref}>
      ...
      <p>
        ...
        The hook is built on <code>useRouteExit</code> +{" "}
        <code>useRouteEnter</code> from <code>@real-router/solid</code>,
      ...
      <ol>
        ...
        <li>
          ...
          fades. The hook is route-agnostic — pages choose semantics.
        </li>
        <li>
          <strong>Sequential timing</strong>: same as the centralised
          recipe — <code>useRouteExit</code> returns a Promise, so the
          router blocks until the exit animation completes before the next
          page mounts.
        </li>
        <li>
        ...
        <li>
          ...
          mounts do not trigger entry animations. Click <em>About</em> and
          back to <em>Home</em>: now the fade-in plays.
        </li>
        <li>
          ...
          <code>Promise.allSettled([])</code> in the exit handler
      ...
      <p>
        ...
        The short version: distributed per-page hooks cover entry + exit
        cleanly, but cross-page coordination (hero morph between routes,
        list FLIP with ghost exits) needs shared state — module-level
        variables or a custom event bus — because each page's hook only
