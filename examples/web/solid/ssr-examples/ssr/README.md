...
- **Top-level `<Show>` for UNKNOWN_ROUTE.** vite-plugin-solid 2.11.x emits divergent hydration-key counters when `<RouteView.NotFound>` sits beside multiple `<RouteView.Match>` siblings. The example sidesteps the mismatch with an app-level `<Show when={isKnown}>` guard — see `App.tsx` for the comment chain. Same workaround in `../ssg/` and `../ssr-streaming/`.
