import { useNavigator, useRouteExit } from "@real-router/vue";
import { onScopeDispose } from "vue";

interface FlipSnapshot {
  rects: Map<string, DOMRect>;
  // Detached clones for items that may unmount on rerender (filter
  // narrowed). We cannot animate exit on the original — Vue removes
 ...
 * page-level composable (`usePageAnimator`) already skips these via
 ...
 * exit decisions are out of scope of `useRouteExit` — the composable
 ...
 *      because Vue unmounts removed items synchronously before
 *      `subscribe` fires).
 *   2. Pre-hide newly-mounted items with `opacity: 0` via a
 *      `MutationObserver` running during Vue's commit phase.
 ...
 *
 * Vue handler-reactivity caveat: the composable runs once at `setup()`,
 * so `pendingFlips` and `flipObserver` live in the closure as plain
 * `let` variables (equivalent role to React's `useRef`).
 */
