 ...
 * `data-product-id` matches), so the recipe lives in this composable
 ...
 * applies and clears. Because Vue composables run once at `setup()`,
 * a plain `let` variable is the natural equivalent of React's
 * `useRef` (closure-captured, mutable across handler invocations).
 */
export function useHeroMorph(): void {
  ...
  const off = navigator.subscribe(() => {
    ...
    // setTimeout(0) defers past Vue's commit so the new page's
