<template>
  <div data-route-root data-route-anim="fade">
    ...
    <ul>
      ...
      <li>
        Want library-free, language-native page transitions via Vue's
        built-in <code>&lt;Transition&gt;</code> component? Use
    ...
    <p>
      ...
      calling <code>useRouteExit</code> from
      <code>@real-router/vue</code> once with its own recipe:
    </p>
    <ul>
      <li>
        <code>usePageAnimator</code> (~50 LOC) — page-level fade/slide
      ...
      <li>
        <code>useHeroMorph</code> (~110 LOC) — captures the source
        thumb rect on <code>useRouteExit</code>, plays an inverse-FLIP
        transform on the destination cover after commit (via
        <code>navigator.subscribe</code>) using the Web Animations API.
      </li>
      <li>
        <code>useListFlip</code> (~230 LOC) — same-route reorder +
    ...
    <p>
      ...
      <code>pendingFlips</code>) lives as plain <code>let</code>
