import { useRouteEnter, useRouteExit } from "@real-router/vue";
 ...
 * composable with a template ref to its outer wrapper.
 *
 * Vue pattern: pages declare `const ref = useTemplateRef<HTMLElement>("name")`
 * (or the older `ref<HTMLElement>()` + `ref="name"` template binding),
 * and pass the ref directly. The composable reads `ref.value` inside
 * the handler at exit/enter time, after the component has mounted and
 * the element is in the DOM.
 *
 * Router-side coordination is fully delegated to two composables from
 * `@real-router/vue`:
 *
 ...
 *
 * Vue handler-reactivity caveat: composables run once at `setup()`, so
 * `entryClass` / `exitClass` / `skipSameRoute` are captured at
 * hook-call time. This is intended — pages declare their animation
