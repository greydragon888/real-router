...
test.describe("Skip initial load", () => {
  test("first-load suppresses entry animation via :appear=\"false\"", async ({
    ...
    await page.goto("/");
    // Heading should be visible immediately — `:appear="false"` on
    // <Transition> prevents the keyed page-level <div> from animating
    // on first mount.
    await expect(
      page.getByRole("heading", { name: "Motion Animations" }),
    ).toBeVisible();

    // Vue may still register an animation briefly during hydration,
