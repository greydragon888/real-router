<template>
  <div>
    ...
    <p>
      ...
      top of <code>App</code>. Each composable wraps
      <code>useRouteExit</code> from <code>@real-router/vue</code> with its own
      DOM recipe; pages stay declarative — they only mark
      <code>[data-route-root]</code> / <code>[data-flip-key]</code> attributes.
      Cross-route coordination (hero rects, list FLIP, ghost clones) lives in
      plain <code>let</code> variables inside the composables.
    </p>

    <h2>page-animations/</h2>
    <p>
      Distributed via <code>useRouteAnimation(ref, …)</code> per page. Each page
      calls the composable in its own component; the composable itself is built
      on <code>useRouteExit</code> + <code>useRouteEnter</code> from
      <code>@real-router/vue</code>. Encapsulated, simpler mental model, less
      boilerplate per new page once the composable exists. Cross-page
      coordination needs shared state because each component's composable only
      sees its own lifecycle.
    </p>
    ...
    <p>
      ...
      <code>mode="out-in"</code> prop sequence the leave hooks before the enter
      hooks; <code>:appear="false"</code> suppresses the initial mount
      animation. The Promise the router awaits resolves on the
      <code>@after-leave</code> event — URL and UI stay in lock-step like the
    ...
    <p>
      ...
      animations. Vue's <code>&lt;Transition&gt;</code> is per-element
      entry/exit only. (<code>&lt;TransitionGroup&gt;</code> handles in-list
      FLIP reorder, but doesn't pair elements across route boundaries.) For
      cross-route hero morph in Vue, see <code>route-animations/</code> for the
    ...
    <ul>
      ...
      <li>
        Cross-browser, library-free declarative page transitions via Vue's
        built-in <code>&lt;Transition&gt;</code>? →
    ...
    <p>
      <strong>
        Library-free, router-coordinated entry/exit using Vue's first-class
        language features.
      </strong>
      The router stays blocked on the page-level exit Promise (resolved by
      <code>@after-leave</code>), so URL and UI stay in lock-step — same
      semantics as <code>route-animations/</code> and
      <code>page-animations/</code>, but the entry / exit choreography is
      expressed through Vue's declarative
      <code>&lt;Transition&gt;</code> component (with CSS class hooks) instead
      of hand-rolled CSS keyframes + <code>animationend</code> bookkeeping or an
    ...
    <p>
      ...
      <code>&lt;Transition&gt;</code>: it calls <code>useRouteExit</code> from
      <code>@real-router/vue</code> with a handler that bumps an
      <code>exitToken</code> ref (driving Vue's keyed remount, which fires the
      leave hooks) and returns a Promise resolved when
      <code>@after-leave</code> fires. The abort signal from
      ...
      the <code>.page-enter-from</code> / <code>.page-enter-to</code> CSS
      classes in <code>App.vue</code>'s <code>&lt;style&gt;</code> block.
    </p>
  </div>
</template>
