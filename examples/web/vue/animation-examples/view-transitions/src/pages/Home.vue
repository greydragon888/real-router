<template>
  <div>
    ...
    <ol>
      ...
      <li>
        <strong>Query-only scope</strong>: open <em>Products</em> and switch the
        sort (A → Z / Z → A), or open <em>Query demo</em> and change the filter.
        <code>html.vt-query-only</code> kills the root curtain so only the local
        list container fades in place.
      </li>
      <li>
        <strong>Hero morph</strong>: open <em>Products</em>, click a product
        card — only that thumb keeps its <code>view-transition-name</code> (set
        via <code>html.vt-hero-morph[data-vt-hero-id]</code>), then FLIP-morphs
        into the detail cover. Root softens to a plain fade so the morph is
        visible.
      </li>
      <li>
      ...
      <li>
        ...
        <code>::view-transition-*</code> duration collapses to zero, the swap is
        instant but the pipeline still runs.
      </li>
      <li>
    ...
    <p>
      ...
      classes on <code>&lt;html&gt;</code> from <code>subscribeLeave</code> and
      lets CSS gate <code>view-transition-name</code> accordingly.
    </p>
    <p>
      ...
      hit-testable, so clicks will feel blocked until the animation ends — this
      is the CSS VT spec behaviour, not a router bug. See the
      <em>About</em> page for details. In production keep animations under ~400
      ms so the block is imperceptible.
    </p>
  </div>
</template>
