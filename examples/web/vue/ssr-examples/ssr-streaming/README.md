...
- **Why no `defer()` from `@real-router/ssr-data-plugin` here?** Vue's `<Suspense>` is **chunked-blocking** in SSR — the renderer awaits every `async setup()` *before* emitting that chunk, so by the time HTML hits the wire the value is already inline. Adding `<script>__rrDefer__("key", json)</script>` settle scripts on top of Vue's body chunks trips the hydration walker (`Hydration completed but contains mismatches`). The cross-adapter `defer()` API is available at `@real-router/vue/ssr` (`useDeferred`/`<Await>`/`<Streamed>`), but **this example uses Vue-native `async setup()` with per-component fetchers**. See `packages/ssr-data-plugin/CLAUDE.md` ("Adapters that intentionally don't dogfood `defer()`") for the full rationale
