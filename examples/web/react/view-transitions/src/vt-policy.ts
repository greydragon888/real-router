 ...
 *
 * The VT utility in `@real-router/react` (via `viewTransitions` prop)
 ...
 * `setTimeout(0)` (new DOM, after React commits). A single CSS rule
...
export function installViewTransitionPolicy(router: Router): () => void {
  ...
  const offSuccess = router.subscribe(() => {
    // After React commits the new DOM, find the target element on the
    // destination page and mark it. setTimeout(0) defers past React's
    // commit (MessageChannel task) and runs before the VT utility's own
    // setTimeout — so the class is in place when the browser captures
    // the new snapshot.
