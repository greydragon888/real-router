...
async function getSsrState(body) {
    const match = body.match(/<script>window\.__SSR_STATE__=(?<json>.+?)<\/script>/);
    if (!match?.groups)
        throw new Error("__SSR_STATE__ not found in response");
    return JSON.parse(match.groups.json);
}
test.describe("ssr-mixed: per-route SSR mode", () => {
    test("home — full SSR (mode=full, HTML rendered, data present)", async ({ request, }) => {
    ...
    test("admin.dashboard — client-only (mode=client-only, no data, shell HTML)", async ({ request, }) => {
    ...
    test("users.profile — data-only (mode=data-only, data present, shell HTML)", async ({ request, }) => {
    ...
    test("docs.detail — function form: ?format=html → full, ?format=pdf → client-only", async ({ request, }) => {
