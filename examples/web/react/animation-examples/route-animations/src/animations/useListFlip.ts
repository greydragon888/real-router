...
interface FlipSnapshot {
  ...
  // narrowed). We cannot animate exit on the original — React removes
 ...
 *      because React unmounts removed items synchronously before
 *      `subscribe` fires).
 *   2. Pre-hide newly-mounted items with `opacity: 0` via a
 *      `MutationObserver` running during React's commit phase.
