...
- **Loader runs again on the client during hydration.** Same caveat as the runtime SSR example: `ssr-data-plugin` registers on both build pipeline and client, and Angular's `provideRealRouterFactory` calls `router.start(path)` directly from `provideAppInitializer` (not via `hydrateRouter()`), so the post-hydration loader-skip mechanism (#596) is bypassed — the `RouterInternals.hydrationState` scratchpad is never populated. For the in-memory `database.ts` it is invisible; for a real `fetch()` it is one extra roundtrip per route. **No `__SSG_STATE__` blob is serialized into the static HTML** — adding one is application-level work, not a router feature. See [`ssr/README.md`](../ssr/README.md) → "No SSR-State Serialization" → "Why doesn't #596 help here?" for the full explanation and a `TransferState` mitigation sketch.
