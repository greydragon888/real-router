...
- **Post-hydration loader skip via TransferState bridge (#599).** Build-time SSG prerender writes the resolved router state into Angular's `TransferState` (the SSG pipeline boots `server.mjs` in-process and `fetch`-es each URL through `AngularNodeAppEngine` — same `provideRealRouterFactory` server pass as runtime SSR). Each prerendered HTML file carries `<script id="ng-state" type="application/json">…</script>` with the seed. On client hydration, `provideRealRouterFactory` consumes the seed via `hydrateRouter(...)` and `ssr-data-plugin` reuses the build-time `state.context.data` — no client-side loader call on first paint. Verified end-to-end via `e2e/ssg.spec.ts` `post-hydration loader skip (#599)` (asserts `window.__LOADER_CALLS__` is empty after deep-link to a prerendered URL).
