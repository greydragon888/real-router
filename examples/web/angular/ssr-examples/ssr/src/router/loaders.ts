...
export const loaders: DataLoaderFactoryMap = {
  ...
  // client-side aborts (server.ts). The composed AbortSignal from
  // withTimeout (#598) fires on the 250 ms deadline OR on client
  // disconnect (upstreamSignal from per-request deps), and
  // `fetch(..., { signal })` propagates the abort to the network layer
  // — the e2e test asserts that the bench counter ticks up when the
  // deadline elapses, proving fetch is actually cancelled.
  slow: (_router, getDep) => () => {
