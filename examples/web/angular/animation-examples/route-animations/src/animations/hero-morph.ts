 ...
 * `data-product-id` matches), so the recipe lives in this factory
 ...
 *   2. After commit (via `navigator.subscribe`), measure the
 *      destination's rect and play an inverse-FLIP transform via the
 *      Web Animations API.
 *
 * The two phases share `pendingHero` — exit captures, subscribe
 * applies and clears. Because Angular `inject*` runs once at component
 * construction, a plain `let` variable is the natural equivalent of
 * React's `useRef` (closure-captured, mutable across handler
 * invocations).
 */
export function installHeroMorph(): void {
  const navigator = injectNavigator();
  const destroyRef = inject(DestroyRef);
  let pendingHero: PendingHero | null = null;

  injectRouteExit(({ route, nextRoute }) => {
  ...
  const off = navigator.subscribe(() => {
    ...
    // setTimeout(0) defers past Angular's commit so the new page's
    // product cover is in the DOM when we measure it.
    setTimeout(() => {
