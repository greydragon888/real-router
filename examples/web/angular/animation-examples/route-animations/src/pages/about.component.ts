  ...
  template: `
    <div data-route-root data-route-anim="fade">
      ...
      <ul>
        ...
        <li>
          Want library-free, language-native page transitions via Angular's
          <code>&#64;angular/animations</code> trigger system? Use
      ...
      <p>
        Three thin factories own the app's animation behavior, each calling
        <code>injectRouteExit</code> from
        <code>&#64;real-router/angular</code> once with its own recipe:
      </p>
      <ul>
        <li>
          <code>installPageAnimator</code> (~50 LOC) — page-level fade/slide on
        ...
        <li>
          <code>installHeroMorph</code> (~110 LOC) — captures the source thumb
          rect on <code>injectRouteExit</code>, plays an inverse-FLIP transform
          on the destination cover after commit (via
          <code>navigator.subscribe</code>) using the Web Animations API.
        </li>
        <li>
          <code>installListFlip</code> (~230 LOC) — same-route reorder + ghost
      ...
      <p>
        <strong>Angular handler-reactivity:</strong>
        <code>inject*</code> functions run once at component construction, so
        handler bodies are captured in closure. Cross-handler state
        (<code>installHeroMorph</code>'s <code>pendingHero</code>,
        <code>installListFlip</code>'s <code>pendingFlips</code>) lives as plain
        <code>let</code> variables in the factory body — equivalent role to
        React's <code>useRef</code>, simpler ergonomics.
      </p>
    </div>
  `,
})
