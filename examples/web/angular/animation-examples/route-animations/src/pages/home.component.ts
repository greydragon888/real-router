...
@Component({
  ...
  template: `
    <div data-route-root data-route-anim="fade">
      ...
      <p>
        This example animates route changes via CSS
        <code>&#64;keyframes</code> and the
        <code>injectRouteExit</code> function from
        <code>&#64;real-router/angular</code> — no View Transitions API. Three
        thin factories own the animation behavior (page-level fade/slide, hero
      ...
      <ol>
        ...
        <li>
          ...
          completes <em>before</em> the next page mounts — no crossfade, no
          overlap. View Transitions gives you crossfade for free; the recipe
        ...
        <li>
          ...
          back button (back — slide-from-left).
          <code>data-nav-direction</code> on <code>&lt;html&gt;</code> is
          flipped by <code>createDirectionTracker(router)</code> (installed in
        ...
        <li>
          ...
          <code>injectRouteExit</code>'s default
        ...
        <li>
          ...
          does not fade (<code>installPageAnimator</code> skips same-route
          navigations by default), but <code>installListFlip</code> opts in via
          ...
          <code>cloneNode</code> ghosts pinned at the captured rect (Angular
          unmounts the originals before subscribe fires).
        </li>
        <li>
          <strong>Hero morph (manual FLIP)</strong>: open <em>Products</em>,
          click a product card. <code>installHeroMorph</code> captures the
          thumbnail's rect on <code>injectRouteExit</code>; after the detail
          page mounts, <code>navigator.subscribe</code> fires an inverse-FLIP
          transform via the Web Animations API. Costs ~110 lines of factory code
        ...
        <li>
          ...
          <code>AbortSignal</code> exposed by
          <code>injectRouteExit</code> triggers the abort-listener cleanup that
          strips the <code>.leaving</code> class from cancelled exits — no
          animation leaks.
        </li>
        <li>
          ...
          <code>installPageAnimator</code> resolves synchronously — the router
          never blocks.
        </li>
        <li>
