import { effect, type ElementRef, type Signal } from "@angular/core";
...
/**
 * Per-component list FLIP factory with full enter / exit / move
 * coverage.
 *
 * One `effect()` runs after every `routeState` signal change and
 * computes three coordinated WAAPI animations against a snapshot of the
 * previous commit:
 *
 ...
 *
 * Why an `effect()` and no `MutationObserver`:
 *
 *   - **Move != remove for MutationObserver**: when Angular's `@for`
 ...
 *     moved item on a sort change. The `effect()` callback's
 ...
 * static cards with `<a realLink>` children this is fine —
 * `pointer-events: none` on the ghost prevents clicks from reaching
 * the cloned anchor anyway.
 *
 * Trigger: this factory reads `routeState()` inside the `effect()`,
 * which makes Angular's signal system rerun the effect on every route
 * commit — including same-route navigations like sort / filter param
 * changes. `effect()` runs after Angular's render commit, so
 * `querySelectorAll` sees the post-update DOM.
 */
export function installListFlip<T extends HTMLElement>(
  containerSignal: Signal<ElementRef<T> | undefined>,
): void {
  const route = injectRoute();
  const previousMap = new Map<string, Snapshot>();

  effect(() => {
    // Read both signals so Angular tracks them and reruns this effect
    // on every route commit (and on viewChild populate).
    route.routeState();

    const container = containerSignal()?.nativeElement;
