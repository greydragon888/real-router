...
@Component({
  ...
  template: `
    ...
    <p>
      ...
      constructor. The factory is built on <code>injectRouteExit</code> +
      <code>injectRouteEnter</code> from <code>&#64;real-router/angular</code>,
      both of which subscribe to the router for the page's lifetime via
      <code>DestroyRef</code>. No centralised policy module — animation logic
      lives next to the page that owns it.
    </p>
    ...
    <ol>
      <li>
        ...
        <code>fade-out</code> class runs, the factory's Promise resolves when
      ...
      <li>
        ...
        factory is route-agnostic — pages choose semantics.
      </li>
      <li>
        <strong>Sequential timing</strong>: same as the centralised recipe —
        <code>injectRouteExit</code> returns a Promise, so the router blocks
      ...
      <li>
        ...
        filter. <code>injectRouteExit</code>'s default
      ...
      <li>
        ...
        not play. <code>injectRouteEnter</code> requires a
    ...
    <p>
      ...
      short version: distributed per-page factories cover entry + exit cleanly,
      but cross-page coordination (hero morph between routes, list FLIP with
      ghost exits) needs shared state — module-level variables or a custom event
      bus — because each page's factory only sees its own lifecycle.
    </p>
  `,
})
