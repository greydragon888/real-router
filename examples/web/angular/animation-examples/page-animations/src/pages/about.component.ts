...
@Component({
  ...
  template: `
    ...
    <p>
      ...
      <code>&#64;real-router/angular</code> with its own DOM recipe. Cross-route
      coordination (hero rect capture, list FLIP, ghost clones) works because
      state lives in plain <code>let</code> variables (closure-captured) inside
      the factories. Pages stay declarative — they only mark
      <code>[data-route-root]</code> / <code>[data-flip-key]</code> attributes;
      the factories find them via DOM queries.
    </p>
    ...
    <p>
      ...
      the factory is in place. The trade-off: each component instance only sees
      ...
      Same for list FLIP with ghost exits — survivors live in the new page's
      DOM, ghosts live nowhere unless something orchestrates them centrally.
    </p>
    ...
    <p>
      ...
      framework). Same URL-and-UI lock-step semantics as the other three
      examples, but the entry / exit choreography is expressed through Angular's
      declarative trigger DSL.
    </p>
    ...
    <p>
      ...
      <code>allSettled([])</code> resolves synchronously). Entry plays on
      nav-driven mount via <code>injectRouteEnter</code>
    ...
    <p>
      ...
      <code>hostRef.nativeElement</code> inside the handler at exit / entry
      time. The host element <em>is</em> the animated wrapper — no inner div
      needed.
    </p>
  `,
})
