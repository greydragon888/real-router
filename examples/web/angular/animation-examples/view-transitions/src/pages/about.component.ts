...
@Component({
  ...
  template: `
    <div>
      <h1>About</h1>
      <p>
        This example uses <code>@real-router/angular</code> with the
        <code>viewTransitions: true</code> option on
        <code>provideRealRouter</code>. The utility lives in
        <code>shared/dom-utils/view-transitions.ts</code> and uses only the
        public <code>subscribeLeave</code> and <code>subscribe</code> router
        APIs — no private internals.
      </p>
