...
// motion-animations/'s page-level wrapper has no per-list
// layout-animation primitive — the products list re-renders in the new
// order without inverse-FLIP per item (route-animations/ and
// page-animations/ ship installListFlip for in-list animations, but
// motion-animations uses a single page-level wrapper). This test
// verifies the sort change causes a DOM re-order — items end up at
// their new positions; for cross-route / in-list FLIP, see
// route-animations/'s installListFlip factory.
test.describe("List reorder (DOM order swap, no per-item FLIP)", () => {
  test("sort change re-renders the product cards in reversed order", async ({
    ...
    // After sort change DOM order should be reversed. Angular re-renders
