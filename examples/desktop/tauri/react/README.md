...
`app:build` requires a Rust toolchain and per-platform deps (`webkit2gtk-4.1` on Linux, Xcode Command Line Tools on macOS, etc.).
...
If you need the exclusive history methods (visited-routes list, visit counts, `peekBack` / `peekForward`, `traverseToLast`), use [`examples/desktop/tauri/react-navigation`](../react-navigation) and accept its OS compatibility trade-offs.

## See also

- [`examples/desktop/tauri/react-navigation`](../react-navigation) — navigation-plugin in Tauri, with OS compatibility notes
- [`examples/desktop/electron/react`](../../electron/react) — same plugin in Electron with a custom `app://` protocol
