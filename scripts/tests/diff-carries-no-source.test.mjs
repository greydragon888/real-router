...
/**
 * Whether this checkout carries the history the three repository cells read.
 *
 * `actions/checkout` defaults to `fetch-depth: 1`, and in such a clone the only
 * commit has no parent to diff against — `git diff <sha>^ <sha>` fails outright.
 * A cell that answered on no data would pass while asserting nothing, so they
 * skip visibly instead. `ci.yml`'s `repo-lints` job checks out full history so
 * they run there; pre-push runs in a developer's clone, which always has it.
 */
const hasHistory = () =>
  git("rev-parse", "--is-shallow-repository").trim() === "false";

const NO_HISTORY = "shallow checkout: no commit history to read";
...
test("every release commit on master is accepted", (t) => {
  if (!hasHistory()) return t.skip(NO_HISTORY);
...
test("every commit that touches source is refused", (t) => {
  if (!hasHistory()) return t.skip(NO_HISTORY);
...
test("a dependency bump is accepted too, and that is the point", (t) => {
  if (!hasHistory()) return t.skip(NO_HISTORY);
